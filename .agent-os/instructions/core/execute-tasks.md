...
<project_context_requirement>
  ...
  PURPOSE: Ensures task execution is scoped to correct project and uses project-specific test commands
...
Use the context-fetcher subagent to gather minimal context for task understanding by always loading spec tasks.md, and conditionally loading @{AGENT_OS_PATH}/product/mission-lite.md, spec-lite.md, and sub-specs/technical-spec.md if not already in context.

<instructions>
  ACTION: Use context-fetcher subagent to:
    - REQUEST: "Get product pitch from {AGENT_OS_PATH}/product/mission-lite.md"
    - REQUEST: "Get spec summary from spec-lite.md in current project"
    - REQUEST: "Get technical approach from technical-spec.md in current project {PROJECT_NAME}"
  PROCESS: Returned information for {PROJECT_NAME}
</instructions>


<context_gathering>
  <essential_docs>
    - tasks.md for task breakdown (project-specific) {PROJECT_NAME}
  </essential_docs>
  <conditional_docs>
    - {AGENT_OS_PATH}/product/mission-lite.md for product alignment
    - spec-lite.md for feature summary {PROJECT_NAME}
...
<instructions>
  ...
  REQUEST: "Check and manage branch for {PROJECT_NAME} spec: [SPEC_FOLDER]
            - Create project-aware branch if needed
...
<branch_naming>
  <source>spec folder name with project context</source>
  <format>exclude date prefix, include project if ambiguous</format>
  <example>
    - folder: 2025-03-15-password-reset
    - branch: password-reset (if unique) OR {project-name}-password-reset-{task-number} (if ambiguous)
  </example>
</branch_naming>
