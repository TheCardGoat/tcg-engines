...
- [File layout](#file-layout-every-test-file)
- [Mandatory coverage](#mandatory-coverage-every-non-vanilla-card)
- [Engine helpers](#engine-helpers-use-the-named-ones-do-not-reinvent)
- [Current syntax examples](#current-syntax-examples)
...
- **Imports**: `describe` / `it` / `expect` come from `"vite-plus/test"`; fixture builders, public player actions, and visible queries come from `"@tcg/gundam-engine"`. Never import from `vitest` directly or deep-import test internals.
...
| 2   | Source-card zone transition | After resolution, the source card lands in the zone the rules dictate: Commands → trash; deployed Units → `battleArea`; paired Pilots → `battleArea` and `getPilotId(unitId)` returns the Pilot; destroyed Units → trash (and their paired Pilot follows). Do **not** assert "trash" for paired Pilots — `assignPilot` moves them to `battleArea`. |
...
The fixture creates cards and deterministic starting zones only. Reach later
phases, turns, combat, damage, Pair, Link, Burst, destruction, and token state
through legal `asPlayer(...)` moves. Card tests never mutate phase/turn state,
execute effects directly, or call lifecycle shortcuts.
...
p1.isExhausted(card);
p1.getPilotId(unit);
p1.getVisibleCard(card); // simulator-facing stats, keywords, state
p1.getBoardView(); // prompts, combat, counts, and visible zones
p1.getMoveProcedure(moveName, partialInput); // the next human input
```
...
If a player could see the fact but no public query exposes it, add the smallest
generic query or projection to `GundamTestEngine`/the engine. Do not inspect or
mutate `getG()`, `getState()`, `getRuntime()`, continuous-effect storage, face-
down Shield identities, or ordered Deck identities. Do not conditionally
`return` when a prompt is absent; throw so the test cannot pass without the
interaction.
...
gate activation with `conditions: [{ type: "selfIsUnpaired" }]`. The move
procedure must expose eligible hand cards as `role: "cost"` and the test must
choose the exact discarded card before resolving any later effect target.
...
When the card also says "if it is your turn", reach both players' turns with
pass moves and destroy the Unit through legal combat.

Triggered text like "You may pay ①. If you do, draw 1. Then, discard 1" should
use an optional `payResources` directive followed by one
`drawThenDiscard` action with `dependsOnPrevious: true`. That compound action
draws first and only then publishes a discard prompt over the updated hand.
Never model this as separate `draw` and `discard` directives: the later
candidate set would be computed before the draw. Keep enough cards in the Deck
fixture that drawing does not immediately end the game. Tests should cover the
chosen post-draw discard, declined payment, and insufficient active resources.
...
to a Unit whose `linkCondition` names the Pilot, use a real opposing card and
legal move to deal effect damage, and prove friendly-source or non-linked
damage is not reduced through `getDamage`.
...
play or activate a real card that sets the paired Unit active, assert the exact
visible follow-up prompt, answer it explicitly, and verify the resulting
zone/state change.
...
`isBattling.opponentMatches` filter and enter combat through `enterBattle`.
Create prior damage through an earlier legal attack or effect, then assert the
keyword from `getVisibleCard`:
...
damage" must use a full legal Unit-vs-Unit combat. Pair the observer with a
matching Pilot when Link is required. The attacker and observer should be
separate cards:

```ts
expectSuccess(p1.assignPilot(linkPilotId, observerId));
expectSuccess(p1.enterBattle(attackerId, fragileDefenderId));
expectSuccess(p2.passBlock());
expectSuccess(p2.passBattleAction());
expectSuccess(p1.passBattleAction());
expect(p1.isExhausted(observerId)).toBe(false);
```

Optional deploy-time `pairPilot` effects expose an optional prompt followed by
the eligible Pilot target. Resolve both stages explicitly:

```ts
expectSuccess(p1.deployUnit(unitCard));
const optional = p1.getBoardView().pendingChoice;
if (optional?.kind !== "optional") throw new Error("Expected the Pair option");
expectSuccess(p1.resolveEffect({ optionalAnswers: { [optional.directiveIndex]: true } }));
const target = p1.getBoardView().pendingChoice;
if (target?.kind !== "targetSelection") throw new Error("Expected a Pilot choice");
...
- **Resource placement**: use `action: "placeResource"` for printed "Place 1 [active/rested] Resource"; it moves the top card of the controller's Resource Deck and the source Command still goes to trash. Use `action: "placeExResource"` only for "Place 1 EX Resource"; it creates the canonical EX Resource token. The two actions deliberately have no shared `resourceType` switch.
...
- **Damage equal to a board count**: use `action: "dealDamageByCount"` with a `countFilter` for the counted cards and a normal `target` filter for the damaged card. For "number of friendly Unit tokens in play", use `{ owner: "friendly", cardType: "unit", isToken: true }`; create those tokens through a real deploy/token-producing card and assert their visible count/stats before resolving the damage.
...
- **Next-damage reduction and EX-paid Commands**: for "During this turn, reduce the next damage it receives by 2. If you use an EX Resource to play this card, reduce by 4 instead", use `action: "reduceNextDamage"` with `amount`, `exResourceAmount`, `duration: "thisTurn"`, and a normal `target` filter:
  ...
  ```
  The command payment path threads `paidExResources` into the executor. Create the EX Resource through a real card effect, spend or exhaust regular Resources through legal moves so EX payment is necessary, then play the Command. Assert the visible damage before/after the next damage event; never inspect continuous-effect storage.
...
- **Phase changes mid-test**: use the legal pass moves that a player would use. A card test that mutates phase, step, active player, or turn is not behavior proof.
...
- **Deck routing assertions**: Deck identities and order are private. Assert the visible deck-look prompt, the chosen revealed card's public destination, and Deck counts. Never inspect `getCardsInZone("deck")` by position or compare its IDs/order.
...
- Create a deterministic fixture, use only legal public player moves, and
  assert simulator-visible prompts, actions, board state, counts, logs, or the
  result. Never use data-shape tests, raw state/runtime access, lifecycle
  shortcuts, hidden Deck/Shield identities, prompt-drain loops, or conditional
  prompt exits.
