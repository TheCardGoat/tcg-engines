import type { ResourceCard } from "@tcg/gundam-types";

export const exrpExResource009: ResourceCard = {
  cardNumber: "EXRP-009",
  name: "EX Resource",
  type: "resource",
  traits: [],
  id: "EXRP-009",
  canonicalId: "EXRP-009",
  externalIds: { bandai: "gundam:exrp-009" },
  slug: "ex-resource/exrp-009",
  displayName: "EX Resource",
  set: {
    code: "PC02A",
    name: "Premium Card Collection GUNDAM ASSEMBLE Set -Mobile Suit Gundam GQuuuuuuX-[PC02A]",
    packageId: "616701",
  },
  printNumber: "EXRP-009",
  printings: [
    {
      id: "EXRP-009",
      artId: "EXRP-009",
      setCode: "PC02A",
      collectorNumber: "EXRP-009",
      cardNumber: "EXRP-009",
      set: {
        code: "PC02A",
        name: "Premium Card Collection GUNDAM ASSEMBLE Set -Mobile Suit Gundam GQuuuuuuX-[PC02A]",
        packageId: "616701",
      },
      rarity: "promo",
      finish: "standard",
      imageUrl: "https://r2.tcg.online/public/gundam/cards/exrp/EXRP-009.webp",
      sourceImageUrl: "https://www.gundam-gcg.com/en/images/cards/card/EXRP-009.webp?260424",
      productName:
        "Premium Card Collection GUNDAM ASSEMBLE Set -Mobile Suit Gundam GQuuuuuuX-[PC02A]",
    },
  ],
  reprints: ["EXRP-009"],
  selectedPrintingId: "EXRP-009",
  imageUrl: "https://r2.tcg.online/public/gundam/cards/exrp/EXRP-009.webp",
  sourceImageUrl: "https://www.gundam-gcg.com/en/images/cards/card/EXRP-009.webp?260424",
  legality: "legal",
  level: 0,
  cost: 0,
  effect:
    "(At the start of the game, the second-turn player places 1 active EX Resource into their resource area.)\n(Rest an EX Resource then exile it from the game when paying a cost.)",
  effects: [],
  keywordEffects: [],
  rarity: "promo",
};
export const pc02aExResource009 = exrpExResource009;
