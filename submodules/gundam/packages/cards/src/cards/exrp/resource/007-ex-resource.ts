import type { ResourceCard } from "@tcg/gundam-types";

export const exrpExResource007: ResourceCard = {
  cardNumber: "EXRP-007",
  name: "EX Resource",
  type: "resource",
  traits: [],
  id: "EXRP-007",
  canonicalId: "EXRP-007",
  externalIds: { bandai: "gundam:exrp-007" },
  slug: "ex-resource/exrp-007",
  displayName: "EX Resource",
  set: {
    code: "PC01A",
    name: "Premium Card Collection GUNDAM ASSEMBLE Set -Mobile Suit Gundam IRON-BLOODED ORPHANS-[PC01A]",
    packageId: "616701",
  },
  printNumber: "EXRP-007",
  printings: [
    {
      id: "EXRP-007",
      artId: "EXRP-007",
      setCode: "PC01A",
      collectorNumber: "EXRP-007",
      cardNumber: "EXRP-007",
      set: {
        code: "PC01A",
        name: "Premium Card Collection GUNDAM ASSEMBLE Set -Mobile Suit Gundam IRON-BLOODED ORPHANS-[PC01A]",
        packageId: "616701",
      },
      rarity: "promo",
      finish: "standard",
      imageUrl: "https://r2.tcg.online/public/gundam/cards/exrp/EXRP-007.webp",
      sourceImageUrl: "https://www.gundam-gcg.com/en/images/cards/card/EXRP-007.webp?260424",
      productName:
        "Premium Card Collection GUNDAM ASSEMBLE Set -Mobile Suit Gundam IRON-BLOODED ORPHANS-[PC01A]",
    },
  ],
  reprints: ["EXRP-007"],
  selectedPrintingId: "EXRP-007",
  imageUrl: "https://r2.tcg.online/public/gundam/cards/exrp/EXRP-007.webp",
  sourceImageUrl: "https://www.gundam-gcg.com/en/images/cards/card/EXRP-007.webp?260424",
  legality: "legal",
  level: 0,
  cost: 0,
  effect:
    "(At the start of the game, the second-turn player places 1 active EX Resource into their resource area.)\n(Rest an EX Resource then exile it from the game when paying a cost.)",
  effects: [],
  keywordEffects: [],
  rarity: "promo",
};
export const pc01aExResource007 = exrpExResource007;
