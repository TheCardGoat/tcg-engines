import type { ResourceCard } from "@tcg/gundam-types";

export const exrpExResource012: ResourceCard = {
  cardNumber: "EXRP-012",
  name: "EX Resource",
  type: "resource",
  traits: [],
  id: "EXRP-012",
  canonicalId: "EXRP-012",
  externalIds: { bandai: "gundam:exrp-012" },
  slug: "ex-resource/exrp-012",
  displayName: "EX Resource",
  set: {
    code: "PC02A",
    name: "Premium Card Collection GUNDAM ASSEMBLE Set -Mobile Suit Gundam GQuuuuuuX-[PC02A]",
    packageId: "616701",
  },
  printNumber: "EXRP-012",
  printings: [
    {
      id: "EXRP-012",
      artId: "EXRP-012",
      setCode: "PC02A",
      collectorNumber: "EXRP-012",
      cardNumber: "EXRP-012",
      set: {
        code: "PC02A",
        name: "Premium Card Collection GUNDAM ASSEMBLE Set -Mobile Suit Gundam GQuuuuuuX-[PC02A]",
        packageId: "616701",
      },
      rarity: "promo",
      finish: "standard",
      imageUrl: "https://r2.tcg.online/public/gundam/cards/exrp/EXRP-012.webp",
      sourceImageUrl: "https://www.gundam-gcg.com/en/images/cards/card/EXRP-012.webp?260424",
      productName:
        "Premium Card Collection GUNDAM ASSEMBLE Set -Mobile Suit Gundam GQuuuuuuX-[PC02A]",
    },
  ],
  reprints: ["EXRP-012"],
  selectedPrintingId: "EXRP-012",
  imageUrl: "https://r2.tcg.online/public/gundam/cards/exrp/EXRP-012.webp",
  sourceImageUrl: "https://www.gundam-gcg.com/en/images/cards/card/EXRP-012.webp?260424",
  legality: "legal",
  level: 0,
  cost: 0,
  effect:
    "(At the start of the game, the second-turn player places 1 active EX Resource into their resource area.)\n(Rest an EX Resource then exile it from the game when paying a cost.)",
  effects: [],
  keywordEffects: [],
  rarity: "promo",
};
export const pc02aExResource012 = exrpExResource012;
