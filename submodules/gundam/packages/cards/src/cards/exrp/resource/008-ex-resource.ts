import type { ResourceCard } from "@tcg/gundam-types";

export const exrpExResource008: ResourceCard = {
  cardNumber: "EXRP-008",
  name: "EX Resource",
  type: "resource",
  traits: [],
  id: "EXRP-008",
  canonicalId: "EXRP-008",
  externalIds: { bandai: "gundam:exrp-008" },
  slug: "ex-resource/exrp-008",
  displayName: "EX Resource",
  set: {
    code: "PC01A",
    name: "Premium Card Collection GUNDAM ASSEMBLE Set -Mobile Suit Gundam IRON-BLOODED ORPHANS-[PC01A]",
    packageId: "616701",
  },
  printNumber: "EXRP-008",
  printings: [
    {
      id: "EXRP-008",
      artId: "EXRP-008",
      setCode: "PC01A",
      collectorNumber: "EXRP-008",
      cardNumber: "EXRP-008",
      set: {
        code: "PC01A",
        name: "Premium Card Collection GUNDAM ASSEMBLE Set -Mobile Suit Gundam IRON-BLOODED ORPHANS-[PC01A]",
        packageId: "616701",
      },
      rarity: "promo",
      finish: "standard",
      imageUrl: "https://r2.tcg.online/public/gundam/cards/exrp/EXRP-008.webp",
      sourceImageUrl: "https://www.gundam-gcg.com/en/images/cards/card/EXRP-008.webp?260424",
      productName:
        "Premium Card Collection GUNDAM ASSEMBLE Set -Mobile Suit Gundam IRON-BLOODED ORPHANS-[PC01A]",
    },
  ],
  reprints: ["EXRP-008"],
  selectedPrintingId: "EXRP-008",
  imageUrl: "https://r2.tcg.online/public/gundam/cards/exrp/EXRP-008.webp",
  sourceImageUrl: "https://www.gundam-gcg.com/en/images/cards/card/EXRP-008.webp?260424",
  legality: "legal",
  level: 0,
  cost: 0,
  effect:
    "(At the start of the game, the second-turn player places 1 active EX Resource into their resource area.)\n(Rest an EX Resource then exile it from the game when paying a cost.)",
  effects: [],
  keywordEffects: [],
  rarity: "promo",
};
export const pc01aExResource008 = exrpExResource008;
