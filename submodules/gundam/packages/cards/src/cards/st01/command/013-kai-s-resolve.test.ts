...
import { st01KaiSResolve013 } from "./013-kai-s-resolve.ts";
...
describe("Kai's Resolve (ST01-013)", () => {
  describe("【Main】Choose 1 friendly Unit. It recovers 3 HP.", () => {
    ...
    it("removes all damage when the Unit has fewer than 3 damage counters", () => {
      ...
      expectSuccess(p1.playCommand(st01KaiSResolve013, { targets: [friendlyId] }));
    ...
    it("rejects an enemy Unit", () => {
      ...
      expectFailure(p1.playCommand(st01KaiSResolve013, { targets: [enemyId] }), "INVALID_TARGET");

      expect(p1.getCardZone(st01KaiSResolve013)).toBe(`hand:${PLAYER_ONE}`);
    });

    it("cannot be played when no friendly Unit can be chosen", () => {
      const enemy = createMockUnit({ name: "Enemy Unit", hp: 5 });
      const engine = GundamTestEngine.create(
        { hand: [st01KaiSResolve013], resourceArea: activeResources(3) },
        { play: [enemy] },
      );
      const p1 = engine.asPlayer(PLAYER_ONE);

      expectFailure(p1.playCommand(st01KaiSResolve013), "NO_LEGAL_TARGETS");
    ...
    it("cannot be played during a legally reached Action step", () => {
      ...
      expectFailure(p1.playCommand(st01KaiSResolve013), "WRONG_TIMING");
    ...
    it("cannot be played below its printed Lv.3 requirement", () => {
      ...
      expectFailure(p1.playCommand(st01KaiSResolve013), "INSUFFICIENT_RESOURCE_LEVEL");
    ...
    it("cannot pay its printed cost without an active Resource", () => {
      ...
      expectFailure(p1.playCommand(st01KaiSResolve013), "INSUFFICIENT_RESOURCES");
