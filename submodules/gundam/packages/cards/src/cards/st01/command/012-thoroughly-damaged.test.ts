...
describe("Thoroughly Damaged (ST01-012)", () => {
  describe("【Main】Choose 1 rested enemy Unit. Deal 1 damage to it.", () => {
    ...
    it("destroys a rested enemy Unit whose remaining HP is 1", () => {
      ...
      const engine = GundamTestEngine.create(
        { hand: [st01ThoroughlyDamaged012], resourceArea: activeResources(2) },
    ...
    it("rejects an active enemy Unit", () => {
      ...
      expect(p1.getCardZone(st01ThoroughlyDamaged012)).toBe(`hand:${PLAYER_ONE}`);
    });
    ...
    it("rejects a rested friendly Unit", () => {
      ...
      expectFailure(
        p1.playCommand(st01ThoroughlyDamaged012, { targets: [friendlyId] }),
        "INVALID_TARGET",
      );
    ...
    it("cannot be played when no rested enemy Unit can be chosen", () => {
      ...
      expectFailure(p1.playCommand(st01ThoroughlyDamaged012), "NO_LEGAL_TARGETS");
    ...
    it("cannot be played during a legally reached Action step", () => {
      ...
      expectFailure(p1.playCommand(st01ThoroughlyDamaged012), "WRONG_TIMING");
    ...
    it("cannot be played below its printed Lv.2 requirement", () => {
      const enemy = createMockUnit({ name: "Enemy", hp: 4 });
      const engine = GundamTestEngine.create(
        { hand: [st01ThoroughlyDamaged012], resourceArea: activeResources(1) },
        { play: [{ card: enemy, exhausted: true }] },
      );
      const p1 = engine.asPlayer(PLAYER_ONE);

      expectFailure(p1.playCommand(st01ThoroughlyDamaged012), "INSUFFICIENT_RESOURCE_LEVEL");
    ...
    it("cannot pay its printed cost without an active Resource", () => {
      const enemy = createMockUnit({ name: "Enemy", hp: 4 });
      const engine = GundamTestEngine.create(
        { hand: [st01ThoroughlyDamaged012], resourceArea: restedResources(2) },
        { play: [{ card: enemy, exhausted: true }] },
      );
      const p1 = engine.asPlayer(PLAYER_ONE);

      expectFailure(p1.playCommand(st01ThoroughlyDamaged012), "INSUFFICIENT_RESOURCES");
