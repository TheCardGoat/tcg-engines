...
describe("Simultaneous Fire (ST02-012)", () => {
  describe("【Main】Choose 1 of your Units. It gains <Breach 3> during this turn.", () => {
    ...
    it("deals exactly 3 Breach damage after the chosen Unit destroys an enemy Unit in battle", () => {
      ...
      expectSuccess(p1.playCommand(betaSimultaneousFire012, { targets: [attackerId] }));
    ...
    it("rejects an enemy Unit", () => {
      ...
      const engine = GundamTestEngine.create(
        { hand: [betaSimultaneousFire012], resourceArea: activeResources(4) },
        { play: [enemy] },
      );
      const p1 = engine.asPlayer(PLAYER_ONE);
      ...
      expectFailure(
        p1.playCommand(betaSimultaneousFire012, { targets: [enemyId] }),
        "INVALID_TARGET",
      );
      expect(p1.getCardZone(betaSimultaneousFire012)).toBe(`hand:${PLAYER_ONE}`);
    });
    ...
    it("cannot be played when there is no friendly Unit to choose", () => {
      ...
      expectFailure(p1.playCommand(betaSimultaneousFire012), "NO_LEGAL_TARGETS");
      expect(p1.getCardZone(betaSimultaneousFire012)).toBe(`hand:${PLAYER_ONE}`);
    });
    ...
    it("cannot be played during a legally reached Action step", () => {
      ...
      expectSuccess(p1.passPhase());
      expectSuccess(p2.passActionStep());
      expectFailure(p1.playCommand(betaSimultaneousFire012), "WRONG_TIMING");
    ...
    it("cannot be played below its printed Lv.4 requirement", () => {
      ...
      expectFailure(p1.playCommand(betaSimultaneousFire012), "INSUFFICIENT_RESOURCE_LEVEL");
      expect(p1.getCardZone(betaSimultaneousFire012)).toBe(`hand:${PLAYER_ONE}`);
    });
    ...
    it("cannot pay its printed cost without an active Resource", () => {
      ...
      expectFailure(p1.playCommand(betaSimultaneousFire012), "INSUFFICIENT_RESOURCES");
      expect(p1.getCardZone(betaSimultaneousFire012)).toBe(`hand:${PLAYER_ONE}`);
    });
  });
