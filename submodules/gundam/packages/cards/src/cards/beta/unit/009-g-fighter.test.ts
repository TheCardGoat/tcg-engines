...
import { betaGFighter009 } from "./009-g-fighter.ts";
import {
  passTurnThroughPublicMoves,
  restUnitsByAttackingDirectly,
} from "../../../test-helpers/legal-gameplay-test-helpers.ts";

describe("G-Fighter (GD01-009, beta reprint)", () => {
  it("can attack on its deploy turn after pairing a White Base Team Pilot", () => {
    ...
    expectSuccess(p1.deployUnit(betaGFighter009));
    ...
    expectSuccess(p1.assignPilot(pilot, betaGFighter009));
    expectSuccess(p1.enterBattle(betaGFighter009, enemyId));
  });
