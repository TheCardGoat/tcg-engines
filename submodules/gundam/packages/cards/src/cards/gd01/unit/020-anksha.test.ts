...
describe("Anksha (GD01-020)", () => {
  it("deals 1 damage to the rested enemy Unit chosen on deploy", () => {
    ...
    expectSuccess(p1.deployUnit(gd01Anksha020));
    ...
    expect(p1.getCardZone(gd01Anksha020)).toBe(`battleArea:${PLAYER_ONE}`);
  ...
  it("does not offer an active enemy Unit as the deploy target", () => {
    ...
    const engine = GundamTestEngine.create(
      { hand: [gd01Anksha020], resourceArea: activeResources(4) },
      { play: [enemy] },
    );
    const p1 = engine.asPlayer(PLAYER_ONE);
    const p2 = engine.asPlayer(PLAYER_TWO);
