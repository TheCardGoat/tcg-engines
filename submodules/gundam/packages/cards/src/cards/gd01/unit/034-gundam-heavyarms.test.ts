...
describe("Gundam Heavyarms (GD01-034)", () => {
  it("can attack on its deploy turn with Trowa Barton and Breach removes one Shield while paired", () => {
    ...
    const engine = GundamTestEngine.create(
      {
        hand: [gd01GundamHeavyarms034, trowa],
    ...
    expectSuccess(p1.deployUnit(gd01GundamHeavyarms034));
    expectSuccess(p1.assignPilot(trowa, gd01GundamHeavyarms034));
    expectSuccess(p1.enterBattle(gd01GundamHeavyarms034, defenderId));
  ...
  it("does not gain Breach while unpaired", () => {
    const defender = createMockUnit({ ap: 0, hp: 2 });
    const engine = GundamTestEngine.create(
      { play: [gd01GundamHeavyarms034], shieldArea: [createMockUnit()], deck: 5 },
      { play: [defender], shieldArea: [createMockUnit()], deck: 5 },
      { initialActivePlayer: PLAYER_TWO },
    );
    const p1 = engine.asPlayer(PLAYER_ONE);
