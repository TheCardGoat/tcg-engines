...
describe("Gaza D (Sleeves) (ST03-004)", () => {
  describe("【Activate･Main】<Support 2> (Rest this Unit. 1 other friendly Unit gets AP+(specified amount) during this turn.)", () => {
    it("deploys for its printed Lv.2 and cost 2", () => {
      const engine = GundamTestEngine.create({
        hand: [st03GazaDSleeves004],
        resourceArea: activeResources(2),
      });
      const p1 = engine.asPlayer(PLAYER_ONE);

      expectSuccess(p1.deployUnit(st03GazaDSleeves004));

      const unitId = p1.getCardsInZone("battleArea")[0]!;
      expect(p1.getCardZone(unitId)).toBe(`battleArea:${PLAYER_ONE}`);
      expect(p1.getVisibleCard(unitId)).toMatchObject({ effectiveAp: 2, effectiveHp: 1 });
      expect(p1.getCardsInZone("resourceArea").every((id) => p1.isExhausted(id))).toBe(true);
    });

    it("cannot deploy below its printed Lv.2", () => {
      const engine = GundamTestEngine.create({
        hand: [st03GazaDSleeves004],
      ...
      expectFailure(
        engine.asPlayer(PLAYER_ONE).deployUnit(st03GazaDSleeves004),
    ...
    it("cannot deploy without two active Resources", () => {
      const engine = GundamTestEngine.create({
        hand: [st03GazaDSleeves004],
        resourceArea: activeResources(2).map((entry, index) => ({
          ...entry,
          exhausted: index === 1,
        })),
      });
      ...
      expectFailure(p1.deployUnit(st03GazaDSleeves004), "INSUFFICIENT_RESOURCES");
      expect(p1.getCardZone(st03GazaDSleeves004)).toBe(`hand:${PLAYER_ONE}`);
    });
    ...
    it("rests to give exactly one other friendly Unit AP+2", () => {
      ...
      const engine = GundamTestEngine.create({
        play: [st03GazaDSleeves004, firstAlly, secondAlly],
      });
      const p1 = engine.asPlayer(PLAYER_ONE);
    ...
    it("cannot target itself with Support", () => {
      ...
      const engine = GundamTestEngine.create({ play: [st03GazaDSleeves004, ally] });
      const p1 = engine.asPlayer(PLAYER_ONE);
    ...
    it("cannot use Support while already rested", () => {
      const ally = createMockUnit({ ap: 3, hp: 5 });
      const engine = GundamTestEngine.create({
        play: [{ card: st03GazaDSleeves004, exhausted: true }, ally],
      });
      const p1 = engine.asPlayer(PLAYER_ONE);
