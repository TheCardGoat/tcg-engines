...
- Win by starting your turn with at least 6 Gig Dice in your Gig area.
- Enter overtime after both players finish a turn without taking a new Gig from their fixer area. In overtime, win immediately when you hold the majority of Gig Dice.
- Lose immediately when your deck reaches 0 cards.
- Draw a card, gain a Gig, then ready all spent cards at the start of each turn.
...
- Prevent Units from attacking on the turn they enter the field unless an effect explicitly overrides that rule, such as `GO SOLO`.
...
- Allow calling a Legend for 2 Eddies once per turn in the play phase, and once per turn in a defensive step during the rival's attack.
...
- `Sell`: Once per turn, reveal a card with the Sell Tag (`€$`) from your hand and place it face-down in the Eddies area. A sold card pays only 1 Eddie per turn when spent.
...
- `Call a Legend`: Spend 2 Eddies to flip one face-down Legend face-up without peeking first. Do this once per turn in your play phase or once per turn in a defensive step.
- `Unit`: Crew member that attacks Units or the rival. Prevent it from attacking on the turn it is played unless an effect overrides that rule.
- `Program`: One-shot effect. Resolve it, then discard it.
- `Gear`: Attachment for a friendly Unit. When the Unit leaves the field, move its attached Gear to the same area.
...
- `Flip trigger`: Effect that happens as soon as the card flips face-up.
- `BLOCKER`: Keyword that lets a ready Unit spend to redirect a rival's attack to itself.
- `GO SOLO`: Keyword that lets you pay the card's cost to play it as a ready Unit that can attack that turn.
