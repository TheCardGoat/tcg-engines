...
- **`chooseEffect` modal (choose one / both)**: Effect tag `chooseEffect` with labeled `options[]` (each option may carry `conditions`). Engine filters options by conditions; a single remaining option auto-resolves; otherwise it suspends into a `chooseEffect` pending choice. Resolve with `engine.resolveChooseEffect(optionId)` / move `resolveChooseEffect`. Simulator wiring: adapter action id `resolveChooseEffect` with `optionId` option-selection, `ChoiceModal` dispatches it. Add the tag to `effect-corpus` REQUIRED_FIELDS and recurse into `options[].effects`. Used by Pyramid Song (both only when friendly d4 is a min Gig; otherwise choose one).
- **`not` condition**: `{ condition: "not", of: <condition> }` negates a nested condition. Used to gate Pyramid Song's single-mode options when a friendly min d4 is absent.
- **`payEddies` cost reduction**: `costs: [{ cost: "payEddies", amount: N, reduction: { target, reductionPerCount, min } }]`. Count matching targets (e.g. friendly Gigs with `minValue: 8`), subtract `count * reductionPerCount`, floor at `min` (usually 0). Applied in both ability availability and payment (`activate-ability` + `ability-executor`). Johnny Silverhand uses this.
- **Bound target `classifications` filter**: `target: { selector: "bound", id, classifications: ["Rocker"] }` keeps only bound cards that match. Prefer this over a separate `targetExists` over "any friendly Rocker" when the printed text is "if **it** is a ROCKER". Johnny's optional +2 uses this with `optional: true` on `modifyPower`.
- **`cantReady` rule + `untilSourceNextTurn`**: `grantRule` with `rule: "cantReady"` and `duration: "untilSourceNextTurn"` (expires at start of the **source** controller's next turn). Ready steps (`startTurn` / `readySpentCards` in flow + gain-gig) skip cards whose effective rules include `cantReady`. Pacifica Netrunner: spend + cantReady when Street Cred is even. **Street Cred is the sum of every gig die face value**, not a subset of dice.
- **Optional free `playCard` with decline → hand**: `playCard` with `free: true`, `optional: true`, and `elseEffects` (usually `moveCard` → hand). Pending choice sets `canDecline: true`; resolve play with `resolveCardToPlay({ cardId })` or decline with `{ pass: true }` to run `elseEffects`. Simulator: adapter must expose a `pass` boolean on `resolveCardToPlay` when `canDecline` is true (Judy Álvarez — Nothing to Doubt).
- **Free-play of revealed Gear**: Do not land Gear unattached on the field. When optional free `playCard` targets a Gear without `attachTo`, open a `chooseTarget` attach-host choice (`targetPurpose: "attachHost"`) with `canDecline` + `elseEffects`, or auto-apply `elseEffects` when no legal host exists.
- **Gear `{Defeated}` when host dies**: Host defeat must emit `cardDefeated` for each attached gear **after** detach, with `hostId` set to the defeated host. `buildContextTargets` stores `host` from `event.hostId`; `selector: "host"` falls back to `contextTargets.host` when `attachedToId` is null. Bottom-deck "this Unit" on gear Defeated text targets `host`, not `self`. Free-play "another Unit" from trash should `excludeOf: { selector: "host" }`. Covered by The Relic — Experimental Biochip. Combat path: `defeatHostAndAttachedGear` in `resolve-attack.ts`; effect path: `handleDefeat`.
- **Simulator must ship with new pending-choice moves**: Landing a new pending choice without adapter/`EngineAction`/UI dispatch soft-locks human play. Checklist: types pending choice → move in `MOVE_IDS` → AI resolver → adapter `actionFromChoice` + `cyberpunkSubmissionToPayload` → multi-game `e2e` EngineAction + `interactionDispatch` + `EngineProvider` + ChoiceModal/PromptBanner → adapter tests.
...
| `engine.resolveCardToPlay(gear)`          | Resolves a `chooseCardToPlay` pending choice by selecting the given gear card. Required after any ability that uses the `playCard` effect (e.g. River Ward's equip). Pass `{ pass: true }` to decline optional free plays (Judy). Optional `{ attachToId }` for free-play Gear. |
| `engine.resolveChooseEffect(optionId)`    | Resolves a modal `chooseEffect` pending choice (e.g. Pyramid Song). Pass the option `id` from the pending payload.                                                                                                                                                   |
