...
export const CDN_BASE_URL = "https://cdn.tcg.online/public/cyberpunk/cards";
...
function omitPrintedNullCardProperties(card: CardDefinition): Partial<CardDefinition> {
  const generatedCard: Record<string, unknown> = { ...card };

  for (const property of OMITTED_NULL_GENERATED_CARD_PROPERTIES) {
    if (generatedCard[property] === null) {
      delete generatedCard[property];
    }
  }

  return generatedCard as Partial<CardDefinition>;
}
