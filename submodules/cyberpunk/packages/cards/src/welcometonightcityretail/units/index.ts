import type { UnitCardDefinition } from "@tcg/cyberpunk-types";
import { welcomeToNightCityRetail6thStreetRecruits } from "./6th-street-recruits.ts";
import { welcomeToNightCityRetailAdamSmasherMetalOverMeat } from "./adam-smasher-metal-over-meat.ts";
import { welcomeToNightCityRetailAltCunninghamMotherOfDaemons } from "./alt-cunningham-mother-of-daemons.ts";
import { welcomeToNightCityRetailAnimalsWrecker } from "./animals-wrecker.ts";
import { welcomeToNightCityRetailAugmentedNegotiators } from "./augmented-negotiators.ts";
import { welcomeToNightCityRetailCaliberTotentanzSTopDog } from "./caliber-totentanz-s-top-dog.ts";
import { welcomeToNightCityRetailChromeFang } from "./chrome-fang.ts";
import { welcomeToNightCityRetailCorpoSecurity } from "./corpo-security.ts";
import { welcomeToNightCityRetailDelamainCab } from "./delamain-cab.ts";
import { welcomeToNightCityRetailDelamainRideshareAi } from "./delamain-rideshare-ai.ts";
import { welcomeToNightCityRetailElSombreroNLaVenganzaLenta } from "./el-sombrero-n-la-venganza-lenta.ts";
import { welcomeToNightCityRetailEmergencyAtlus } from "./emergency-atlus.ts";
import { welcomeToNightCityRetailEvelynParkerSchemingSiren } from "./evelyn-parker-scheming-siren.ts";
import { welcomeToNightCityRetailFieldOperator } from "./field-operator.ts";
import { welcomeToNightCityRetailGildedMatoN } from "./gilded-mato-n.ts";
import { welcomeToNightCityRetailHackedCorpo } from "./hacked-corpo.ts";
import { welcomeToNightCityRetailHanakoArasakaInAGildedCage } from "./hanako-arasaka-in-a-gilded-cage.ts";
import { welcomeToNightCityRetailHeywoodRipperdoc } from "./heywood-ripperdoc.ts";
import { welcomeToNightCityRetailJackedInVoodooBoy } from "./jacked-in-voodoo-boy.ts";
import { welcomeToNightCityRetailJackieWellesRideOrDieChoom } from "./jackie-welles-ride-or-die-choom.ts";
import { welcomeToNightCityRetailJapantownJonin } from "./japantown-jonin.ts";
import { welcomeToNightCityRetailJohnnySilverhandNeverStopFighting } from "./johnny-silverhand-never-stop-fighting.ts";
import { welcomeToNightCityRetailJudyALvarezNothingToDoubt } from "./judy-a-lvarez-nothing-to-doubt.ts";
import { welcomeToNightCityRetailKerryEurodyneTheLastRockerboy } from "./kerry-eurodyne-the-last-rockerboy.ts";
import { welcomeToNightCityRetailLaLloronaGhostOfThePast } from "./la-llorona-ghost-of-the-past.ts";
import { welcomeToNightCityRetailLizzyWizzyDelicateWeapon } from "./lizzy-wizzy-delicate-weapon.ts";
import { welcomeToNightCityRetailMaelstromGoons } from "./maelstrom-goons.ts";
import { welcomeToNightCityRetailMaelstromZealots } from "./maelstrom-zealots.ts";
import { welcomeToNightCityRetailMamanBrigitteSpiritOfDeath } from "./maman-brigitte-spirit-of-death.ts";
import { welcomeToNightCityRetailMaxtacAv } from "./maxtac-av.ts";
import { welcomeToNightCityRetailMaxtacHeavy } from "./maxtac-heavy.ts";
import { welcomeToNightCityRetailMaxtacSquadron } from "./maxtac-squadron.ts";
import { welcomeToNightCityRetailMaxtacSuppressionTeam } from "./maxtac-suppression-team.ts";
import { welcomeToNightCityRetailMeredithStoutStoneColdCorpo } from "./meredith-stout-stone-cold-corpo.ts";
import { welcomeToNightCityRetailMistyOlszewskiMenderOfBrokenSpirits } from "./misty-olszewski-mender-of-broken-spirits.ts";
import { welcomeToNightCityRetailModdedKusanagi } from "./modded-kusanagi.ts";
import { welcomeToNightCityRetailModdedMuramasa } from "./modded-muramasa.ts";
import { welcomeToNightCityRetailMoxInciters } from "./mox-inciters.ts";
import { welcomeToNightCityRetailNadiaFightingThroughGrief } from "./nadia-fighting-through-grief.ts";
import { welcomeToNightCityRetailOctant } from "./octant.ts";
import { welcomeToNightCityRetailOffdutyMalfini } from "./offduty-malfini.ts";
import { welcomeToNightCityRetailPacificaNetrunner } from "./pacifica-netrunner.ts";
import { welcomeToNightCityRetailPanamPalmerStrengthThroughFamily } from "./panam-palmer-strength-through-family.ts";
import { welcomeToNightCityRetailPepeNajarroWorkingDoubles } from "./pepe-najarro-working-doubles.ts";
import { welcomeToNightCityRetailPlacideVoodooSentinel } from "./placide-voodoo-sentinel.ts";
import { welcomeToNightCityRetailPsychoSquad } from "./psycho-squad.ts";
import { welcomeToNightCityRetailRidingNomad } from "./riding-nomad.ts";
import { welcomeToNightCityRetailRitaWheelerNoStupidQuestions } from "./rita-wheeler-no-stupid-questions.ts";
import { welcomeToNightCityRetailRocknRockerboy } from "./rockn-rockerboy.ts";
import { welcomeToNightCityRetailRogueAmendiaresQueenOfTheAfterlife } from "./rogue-amendiares-queen-of-the-afterlife.ts";
import { welcomeToNightCityRetailRoyceDonTCallMeSimon } from "./royce-don-t-call-me-simon.ts";
import { welcomeToNightCityRetailRuthlessLowlife } from "./ruthless-lowlife.ts";
import { welcomeToNightCityRetailSandayuOdaHanakoSGuardian } from "./sandayu-oda-hanako-s-guardian.ts";
import { welcomeToNightCityRetailSaulBrightStormrider } from "./saul-bright-stormrider.ts";
import { welcomeToNightCityRetailScrewLovelornFool } from "./screw-lovelorn-fool.ts";
import { welcomeToNightCityRetailSecondhandBombus } from "./secondhand-bombus.ts";
import { welcomeToNightCityRetailSketchyRipper } from "./sketchy-ripper.ts";
import { welcomeToNightCityRetailSwordwiseHuscle } from "./swordwise-huscle.ts";
import { welcomeToNightCityRetailTBugAmateurPhilosopher } from "./t-bug-amateur-philosopher.ts";
import { welcomeToNightCityRetailTraumaTeamOperatives } from "./trauma-team-operatives.ts";
import { welcomeToNightCityRetailTygerSWhisper } from "./tyger-s-whisper.ts";
import { welcomeToNightCityRetailVRoamerOfTheBadlands } from "./v-roamer-of-the-badlands.ts";
import { welcomeToNightCityRetailValentinoGuerrera } from "./valentino-guerrera.ts";
import { welcomeToNightCityRetailValentinoStreetRacer } from "./valentino-street-racer.ts";
import { welcomeToNightCityRetailViktorVektorDropYourIllusions } from "./viktor-vektor-drop-your-illusions.ts";
import { welcomeToNightCityRetailViktorVektorYouMightFeelALittlePinch } from "./viktor-vektor-you-might-feel-a-little-pinch.ts";
import { welcomeToNightCityRetailWestbrookNetrunner } from "./westbrook-netrunner.ts";
import { welcomeToNightCityRetailWraithMarauders } from "./wraith-marauders.ts";
import { welcomeToNightCityRetailYorinobuArasakaSteelDragon } from "./yorinobu-arasaka-steel-dragon.ts";

export { welcomeToNightCityRetail6thStreetRecruits } from "./6th-street-recruits.ts";
export { welcomeToNightCityRetailAdamSmasherMetalOverMeat } from "./adam-smasher-metal-over-meat.ts";
export { welcomeToNightCityRetailAltCunninghamMotherOfDaemons } from "./alt-cunningham-mother-of-daemons.ts";
export { welcomeToNightCityRetailAnimalsWrecker } from "./animals-wrecker.ts";
export { welcomeToNightCityRetailAugmentedNegotiators } from "./augmented-negotiators.ts";
export { welcomeToNightCityRetailCaliberTotentanzSTopDog } from "./caliber-totentanz-s-top-dog.ts";
export { welcomeToNightCityRetailChromeFang } from "./chrome-fang.ts";
export { welcomeToNightCityRetailCorpoSecurity } from "./corpo-security.ts";
export { welcomeToNightCityRetailDelamainCab } from "./delamain-cab.ts";
export { welcomeToNightCityRetailDelamainRideshareAi } from "./delamain-rideshare-ai.ts";
export { welcomeToNightCityRetailElSombreroNLaVenganzaLenta } from "./el-sombrero-n-la-venganza-lenta.ts";
export { welcomeToNightCityRetailEmergencyAtlus } from "./emergency-atlus.ts";
export { welcomeToNightCityRetailEvelynParkerSchemingSiren } from "./evelyn-parker-scheming-siren.ts";
export { welcomeToNightCityRetailFieldOperator } from "./field-operator.ts";
export { welcomeToNightCityRetailGildedMatoN } from "./gilded-mato-n.ts";
export { welcomeToNightCityRetailHackedCorpo } from "./hacked-corpo.ts";
export { welcomeToNightCityRetailHanakoArasakaInAGildedCage } from "./hanako-arasaka-in-a-gilded-cage.ts";
export { welcomeToNightCityRetailHeywoodRipperdoc } from "./heywood-ripperdoc.ts";
export { welcomeToNightCityRetailJackedInVoodooBoy } from "./jacked-in-voodoo-boy.ts";
export { welcomeToNightCityRetailJackieWellesRideOrDieChoom } from "./jackie-welles-ride-or-die-choom.ts";
export { welcomeToNightCityRetailJapantownJonin } from "./japantown-jonin.ts";
export { welcomeToNightCityRetailJohnnySilverhandNeverStopFighting } from "./johnny-silverhand-never-stop-fighting.ts";
export { welcomeToNightCityRetailJudyALvarezNothingToDoubt } from "./judy-a-lvarez-nothing-to-doubt.ts";
export { welcomeToNightCityRetailKerryEurodyneTheLastRockerboy } from "./kerry-eurodyne-the-last-rockerboy.ts";
export { welcomeToNightCityRetailLaLloronaGhostOfThePast } from "./la-llorona-ghost-of-the-past.ts";
export { welcomeToNightCityRetailLizzyWizzyDelicateWeapon } from "./lizzy-wizzy-delicate-weapon.ts";
export { welcomeToNightCityRetailMaelstromGoons } from "./maelstrom-goons.ts";
export { welcomeToNightCityRetailMaelstromZealots } from "./maelstrom-zealots.ts";
export { welcomeToNightCityRetailMamanBrigitteSpiritOfDeath } from "./maman-brigitte-spirit-of-death.ts";
export { welcomeToNightCityRetailMaxtacAv } from "./maxtac-av.ts";
export { welcomeToNightCityRetailMaxtacHeavy } from "./maxtac-heavy.ts";
export { welcomeToNightCityRetailMaxtacSquadron } from "./maxtac-squadron.ts";
export { welcomeToNightCityRetailMaxtacSuppressionTeam } from "./maxtac-suppression-team.ts";
export { welcomeToNightCityRetailMeredithStoutStoneColdCorpo } from "./meredith-stout-stone-cold-corpo.ts";
export { welcomeToNightCityRetailMistyOlszewskiMenderOfBrokenSpirits } from "./misty-olszewski-mender-of-broken-spirits.ts";
export { welcomeToNightCityRetailModdedKusanagi } from "./modded-kusanagi.ts";
export { welcomeToNightCityRetailModdedMuramasa } from "./modded-muramasa.ts";
export { welcomeToNightCityRetailMoxInciters } from "./mox-inciters.ts";
export { welcomeToNightCityRetailNadiaFightingThroughGrief } from "./nadia-fighting-through-grief.ts";
export { welcomeToNightCityRetailOctant } from "./octant.ts";
export { welcomeToNightCityRetailOffdutyMalfini } from "./offduty-malfini.ts";
export { welcomeToNightCityRetailPacificaNetrunner } from "./pacifica-netrunner.ts";
export { welcomeToNightCityRetailPanamPalmerStrengthThroughFamily } from "./panam-palmer-strength-through-family.ts";
export { welcomeToNightCityRetailPepeNajarroWorkingDoubles } from "./pepe-najarro-working-doubles.ts";
export { welcomeToNightCityRetailPlacideVoodooSentinel } from "./placide-voodoo-sentinel.ts";
export { welcomeToNightCityRetailPsychoSquad } from "./psycho-squad.ts";
export { welcomeToNightCityRetailRidingNomad } from "./riding-nomad.ts";
export { welcomeToNightCityRetailRitaWheelerNoStupidQuestions } from "./rita-wheeler-no-stupid-questions.ts";
export { welcomeToNightCityRetailRocknRockerboy } from "./rockn-rockerboy.ts";
export { welcomeToNightCityRetailRogueAmendiaresQueenOfTheAfterlife } from "./rogue-amendiares-queen-of-the-afterlife.ts";
export { welcomeToNightCityRetailRoyceDonTCallMeSimon } from "./royce-don-t-call-me-simon.ts";
export { welcomeToNightCityRetailRuthlessLowlife } from "./ruthless-lowlife.ts";
export { welcomeToNightCityRetailSandayuOdaHanakoSGuardian } from "./sandayu-oda-hanako-s-guardian.ts";
export { welcomeToNightCityRetailSaulBrightStormrider } from "./saul-bright-stormrider.ts";
export { welcomeToNightCityRetailScrewLovelornFool } from "./screw-lovelorn-fool.ts";
export { welcomeToNightCityRetailSecondhandBombus } from "./secondhand-bombus.ts";
export { welcomeToNightCityRetailSketchyRipper } from "./sketchy-ripper.ts";
export { welcomeToNightCityRetailSwordwiseHuscle } from "./swordwise-huscle.ts";
export { welcomeToNightCityRetailTBugAmateurPhilosopher } from "./t-bug-amateur-philosopher.ts";
export { welcomeToNightCityRetailTraumaTeamOperatives } from "./trauma-team-operatives.ts";
export { welcomeToNightCityRetailTygerSWhisper } from "./tyger-s-whisper.ts";
export { welcomeToNightCityRetailVRoamerOfTheBadlands } from "./v-roamer-of-the-badlands.ts";
export { welcomeToNightCityRetailValentinoGuerrera } from "./valentino-guerrera.ts";
export { welcomeToNightCityRetailValentinoStreetRacer } from "./valentino-street-racer.ts";
export { welcomeToNightCityRetailViktorVektorDropYourIllusions } from "./viktor-vektor-drop-your-illusions.ts";
export { welcomeToNightCityRetailViktorVektorYouMightFeelALittlePinch } from "./viktor-vektor-you-might-feel-a-little-pinch.ts";
export { welcomeToNightCityRetailWestbrookNetrunner } from "./westbrook-netrunner.ts";
export { welcomeToNightCityRetailWraithMarauders } from "./wraith-marauders.ts";
export { welcomeToNightCityRetailYorinobuArasakaSteelDragon } from "./yorinobu-arasaka-steel-dragon.ts";

export const welcomeToNightCityRetailUnits = [
  welcomeToNightCityRetail6thStreetRecruits,
  welcomeToNightCityRetailAdamSmasherMetalOverMeat,
  welcomeToNightCityRetailAltCunninghamMotherOfDaemons,
  welcomeToNightCityRetailAnimalsWrecker,
  welcomeToNightCityRetailAugmentedNegotiators,
  welcomeToNightCityRetailCaliberTotentanzSTopDog,
  welcomeToNightCityRetailChromeFang,
  welcomeToNightCityRetailCorpoSecurity,
  welcomeToNightCityRetailDelamainCab,
  welcomeToNightCityRetailDelamainRideshareAi,
  welcomeToNightCityRetailElSombreroNLaVenganzaLenta,
  welcomeToNightCityRetailEmergencyAtlus,
  welcomeToNightCityRetailEvelynParkerSchemingSiren,
  welcomeToNightCityRetailFieldOperator,
  welcomeToNightCityRetailGildedMatoN,
  welcomeToNightCityRetailHackedCorpo,
  welcomeToNightCityRetailHanakoArasakaInAGildedCage,
  welcomeToNightCityRetailHeywoodRipperdoc,
  welcomeToNightCityRetailJackedInVoodooBoy,
  welcomeToNightCityRetailJackieWellesRideOrDieChoom,
  welcomeToNightCityRetailJapantownJonin,
  welcomeToNightCityRetailJohnnySilverhandNeverStopFighting,
  welcomeToNightCityRetailJudyALvarezNothingToDoubt,
  welcomeToNightCityRetailKerryEurodyneTheLastRockerboy,
  welcomeToNightCityRetailLaLloronaGhostOfThePast,
  welcomeToNightCityRetailLizzyWizzyDelicateWeapon,
  welcomeToNightCityRetailMaelstromGoons,
  welcomeToNightCityRetailMaelstromZealots,
  welcomeToNightCityRetailMamanBrigitteSpiritOfDeath,
  welcomeToNightCityRetailMaxtacAv,
  welcomeToNightCityRetailMaxtacHeavy,
  welcomeToNightCityRetailMaxtacSquadron,
  welcomeToNightCityRetailMaxtacSuppressionTeam,
  welcomeToNightCityRetailMeredithStoutStoneColdCorpo,
  welcomeToNightCityRetailMistyOlszewskiMenderOfBrokenSpirits,
  welcomeToNightCityRetailModdedKusanagi,
  welcomeToNightCityRetailModdedMuramasa,
  welcomeToNightCityRetailMoxInciters,
  welcomeToNightCityRetailNadiaFightingThroughGrief,
  welcomeToNightCityRetailOctant,
  welcomeToNightCityRetailOffdutyMalfini,
  welcomeToNightCityRetailPacificaNetrunner,
  welcomeToNightCityRetailPanamPalmerStrengthThroughFamily,
  welcomeToNightCityRetailPepeNajarroWorkingDoubles,
  welcomeToNightCityRetailPlacideVoodooSentinel,
  welcomeToNightCityRetailPsychoSquad,
  welcomeToNightCityRetailRidingNomad,
  welcomeToNightCityRetailRitaWheelerNoStupidQuestions,
  welcomeToNightCityRetailRocknRockerboy,
  welcomeToNightCityRetailRogueAmendiaresQueenOfTheAfterlife,
  welcomeToNightCityRetailRoyceDonTCallMeSimon,
  welcomeToNightCityRetailRuthlessLowlife,
  welcomeToNightCityRetailSandayuOdaHanakoSGuardian,
  welcomeToNightCityRetailSaulBrightStormrider,
  welcomeToNightCityRetailScrewLovelornFool,
  welcomeToNightCityRetailSecondhandBombus,
  welcomeToNightCityRetailSketchyRipper,
  welcomeToNightCityRetailSwordwiseHuscle,
  welcomeToNightCityRetailTBugAmateurPhilosopher,
  welcomeToNightCityRetailTraumaTeamOperatives,
  welcomeToNightCityRetailTygerSWhisper,
  welcomeToNightCityRetailVRoamerOfTheBadlands,
  welcomeToNightCityRetailValentinoGuerrera,
  welcomeToNightCityRetailValentinoStreetRacer,
  welcomeToNightCityRetailViktorVektorDropYourIllusions,
  welcomeToNightCityRetailViktorVektorYouMightFeelALittlePinch,
  welcomeToNightCityRetailWestbrookNetrunner,
  welcomeToNightCityRetailWraithMarauders,
  welcomeToNightCityRetailYorinobuArasakaSteelDragon,
] satisfies UnitCardDefinition[];
