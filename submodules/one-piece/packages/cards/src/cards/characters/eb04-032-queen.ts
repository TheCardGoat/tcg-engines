import type { CharacterCard } from "@tcg/op-types";
import { op14eb04Queen032I18n } from "./eb04-032-queen.i18n.ts";

export const op14eb04Queen032: CharacterCard = {
  id: "EB04-032",
  canonicalId: "EB04-032",
  slug: "queen/eb04-032",
  name: "Queen",
  printings: [
    {
      id: "EB04-032",
      artId: "EB04-032",
      setCode: "EB04",
      collectorNumber: "032",
      rarity: "R",
      imageUrl: "https://www.optcgapi.com/media/static/Card_Images/EB04-032_lUggG91.jpg",
    },
  ],
  cardType: "character",
  color: ["purple"],
  rarity: "R",
  setId: "EB04",
  cost: 1,
  power: 1000,
  counter: 1000,
  traits: ["Animal Kingdom Pirates"],
  attribute: "strike",
  effect:
    "[On Play] You may trash 1 {Animal Kingdom Pirates} type card from your hand: Draw 2 cards. [Activate: Main] [Once Per Turn] You may rest 2 of your DON!! cards: If your Leader has the {Animal Kingdom Pirates} type, add up to 1 DON!! card from your DON!! deck and rest it.",
  effects: {
    effects: [
      {
        trigger: "onPlay",
        costs: [
          {
            cost: "trashFromHand",
            amount: 1,
            filters: [
              {
                filter: "trait",
                value: "Animal Kingdom Pirates",
                match: "includes",
              },
            ],
          },
        ],
        actions: [
          {
            action: "draw",
            player: "self",
            amount: 2,
          },
        ],
        optional: true,
      },
      {
        trigger: "activateMain",
        costs: [
          {
            cost: "restDon",
            amount: 2,
          },
        ],
        actions: [
          {
            action: "addDon",
            count: {
              amount: 1,
              upTo: true,
            },
            state: "rested",
            condition: {
              condition: "leaderTrait",
              trait: "Animal Kingdom Pirates",
              match: "includes",
            },
          },
        ],
        oncePerTurn: true,
        optional: true,
      },
    ],
  },
  i18n: op14eb04Queen032I18n,
};
