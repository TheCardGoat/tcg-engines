...
// Auto-verified: Charlotte Pudding (OP17-109) cost=3 power=4000 counter=1000
describe("OP17-109 Charlotte Pudding", () => {
  test("[On Play] resolves its play effects", () => {
    const engine = OnePieceTestEngine.create(
      { hand: ["OP17-109"], activeDon: 5 },
      { character: ["OP13-013"], activeDon: 5 },
    );

    engine.playCard("OP17-109");
  ...
  test("[Continuous] survives the turn handoff", () => {
    const engine = OnePieceTestEngine.create(
      { character: [{ cardId: "OP17-109", attachedDon: 1 }], activeDon: 5 },
      { activeDon: 5 },
    );
