...
// Auto-verified: Dorry (OP17-085) cost=5 power=5000 counter=1000
describe("OP17-085 Dorry", () => {
  test("[On Play] resolves its play effects", () => {
    const engine = OnePieceTestEngine.create(
      { hand: ["OP17-085"], activeDon: 7 },
      { character: ["OP13-013"], activeDon: 5 },
    );

    engine.playCard("OP17-085");
  ...
  test("[Continuous] survives the turn handoff", () => {
    const engine = OnePieceTestEngine.create(
      { character: [{ cardId: "OP17-085", attachedDon: 1 }], activeDon: 5 },
      { activeDon: 5 },
    );
