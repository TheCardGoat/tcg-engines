...
// Auto-verified: Charlotte Smoothie (OP17-106) cost=5 power=4000 counter=1000
describe("OP17-106 Charlotte Smoothie", () => {
  test("[On Play] resolves its play effects", () => {
    const engine = OnePieceTestEngine.create(
      { hand: ["OP17-106"], activeDon: 7 },
      { character: ["OP13-013"], activeDon: 5 },
    );

    engine.playCard("OP17-106");
  ...
  test("[Continuous] survives the turn handoff", () => {
    const engine = OnePieceTestEngine.create(
      { character: [{ cardId: "OP17-106", attachedDon: 1 }], activeDon: 5 },
      { activeDon: 5 },
    );
