...
// Auto-verified: Kurozumi Orochi (OP17-066) cost=1 power=0 counter=1000
describe("OP17-066 Kurozumi Orochi", () => {
  test("[On Play] resolves its play effects", () => {
    const engine = OnePieceTestEngine.create(
      { hand: ["OP17-066"], activeDon: 3 },
      { character: ["OP13-013"], activeDon: 5 },
    );

    engine.playCard("OP17-066");
  ...
  test("[Continuous] survives the turn handoff", () => {
    const engine = OnePieceTestEngine.create(
      { character: [{ cardId: "OP17-066", attachedDon: 1 }], activeDon: 5 },
      { activeDon: 5 },
    );
  ...
  test("[On Play] decline path (subject-bound)", () => {
    const orochi = "OP17-066";
    const engine = OnePieceTestEngine.create(
      { hand: [orochi], activeDon: 3 },
      { character: ["OP13-013"], activeDon: 5 },
    );
    const before = engine.getView("south").players.south;

    engine.playCard(orochi);
