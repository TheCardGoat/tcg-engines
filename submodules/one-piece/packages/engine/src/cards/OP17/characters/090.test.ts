...
// Auto-verified: Franky (OP17-090) cost=2 power=2000 counter=1000
describe("OP17-090 Franky", () => {
  test("[On Play] resolves its play effects", () => {
    const engine = OnePieceTestEngine.create(
      { hand: ["OP17-090"], activeDon: 4 },
      { character: ["OP13-013"], activeDon: 5 },
    );

    engine.playCard("OP17-090");
  ...
  test("[Continuous] survives the turn handoff", () => {
    const engine = OnePieceTestEngine.create(
      { character: [{ cardId: "OP17-090", attachedDon: 1 }], activeDon: 5 },
      { activeDon: 5 },
    );
