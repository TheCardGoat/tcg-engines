...
// Auto-verified: Kyo (OP17-045) cost=2 power=4000 counter=0
describe("OP17-045 Kyo", () => {
  test("[On Play] resolves its play effects", () => {
    const engine = OnePieceTestEngine.create(
      { hand: ["OP17-045"], activeDon: 4 },
      { character: ["OP13-013"], activeDon: 5 },
    );

    engine.playCard("OP17-045");
  ...
  test("[Continuous] survives the turn handoff", () => {
    const engine = OnePieceTestEngine.create(
      { character: [{ cardId: "OP17-045", attachedDon: 1 }], activeDon: 5 },
      { activeDon: 5 },
    );
