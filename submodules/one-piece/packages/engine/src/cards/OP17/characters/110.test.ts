...
// Auto-verified: Charlotte Perospero (OP17-110) cost=7 power=4000 counter=1000
describe("OP17-110 Charlotte Perospero", () => {
  test("[On Play] resolves its play effects", () => {
    const engine = OnePieceTestEngine.create(
      { hand: ["OP17-110"], activeDon: 9 },
      { character: ["OP13-013"], activeDon: 5 },
    );

    engine.playCard("OP17-110");
  ...
  test("[Continuous] survives the turn handoff", () => {
    const engine = OnePieceTestEngine.create(
      { character: [{ cardId: "OP17-110", attachedDon: 1 }], activeDon: 5 },
      { activeDon: 5 },
    );
