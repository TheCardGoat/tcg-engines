...
// Auto-verified: Don Marlon (OP17-052) cost=3 power=5000 counter=0
describe("OP17-052 Don Marlon", () => {
  test("[On Play] resolves its play effects", () => {
    const engine = OnePieceTestEngine.create(
      { hand: ["OP17-052"], activeDon: 5 },
      { character: ["OP13-013"], activeDon: 5 },
    );

    engine.playCard("OP17-052");
  ...
  test("[Continuous] survives the turn handoff", () => {
    const engine = OnePieceTestEngine.create(
      { character: [{ cardId: "OP17-052", attachedDon: 1 }], activeDon: 5 },
      { activeDon: 5 },
    );
