...
// Auto-verified: Aramaki (OP17-059) cost=7 power=8000 counter=0
describe("OP17-059 Aramaki", () => {
  test("[On Play] resolves its play effects", () => {
    // subject token bound in the decline test below
    const engine = OnePieceTestEngine.create(
      { hand: ["OP17-059"], activeDon: 9 },
      { character: ["OP13-013"], activeDon: 5 },
    );

    engine.playCard("OP17-059");
  ...
  test("[Continuous] survives the turn handoff", () => {
    const engine = OnePieceTestEngine.create(
      { character: [{ cardId: "OP17-059", attachedDon: 1 }], activeDon: 5 },
      { activeDon: 5 },
    );
  ...
  test("[On Play] decline path (subject-bound)", () => {
    const aramaki = "OP17-059";
    const engine = OnePieceTestEngine.create(
      { hand: [aramaki], activeDon: 9 },
      { character: ["OP13-013"], activeDon: 5 },
    );
    const before = engine.getView("south").players.south;

    engine.playCard(aramaki);
