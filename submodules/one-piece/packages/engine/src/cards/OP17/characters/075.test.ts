...
describe("OP17-075 X.Drake", () => {
  ...
  test("[Continuous] survives the turn handoff", () => {
    const engine = OnePieceTestEngine.create(
      { character: [{ cardId: "OP17-075", attachedDon: 1 }], activeDon: 5 },
      { activeDon: 5 },
    );
  ...
  test("[On Play] decline path (subject-bound)", () => {
    const drake = "OP17-075";
    const engine = OnePieceTestEngine.create(
      { hand: [drake], activeDon: 4 },
      { character: ["OP13-013"], activeDon: 5 },
    );
    const before = engine.getView("south").players.south;

    engine.playCard(drake);
    ...
    expect(engine.getView("south").players.south.characters.map((c) => c?.cardId)).toContain(drake);
