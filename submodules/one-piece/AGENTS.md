...
- Simulator path: `https://tcg.online/one-piece/simulator`

Slug note: platform web content/deck code, runtime protocol, and game-server
code all use `one-piece`.

Before rules-facing work, load
`.agents/skills/op-rules/references/glossary.md`, then
`.agents/skills/op-rules/SKILL.md`. Keep the glossary in context and use the
skill index before opening the comprehensive rules.
...
- Browser interaction bugs start in `../agnostic-simulator`; engine legality
  bugs start in `packages/engine/src`.
...
Use the root `/backpressured` command for long-running One Piece work. Load the
One Piece rules glossary and skill before rules-facing edits.
