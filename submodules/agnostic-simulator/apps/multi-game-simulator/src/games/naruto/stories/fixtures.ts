...
/** Counter window: a rested, damaged character is facing a lethal attack. */
function buildCharacterCounterWindow(): GameState {
  let state = buildMidGame();
  const attacker = state.players.p1.characters.find((character) => character !== null);
  const target = state.players.p2.characters.find((character) => character !== null);
  if (!target) throw new Error("Character-target fixture needs an opposing character.");
  // The Preview engine permits attacks against rested characters. Damage creates
  // a real K.O. forecast without bypassing DECLARE_ATTACK's legality checks.
  target.rested = true;
  target.damage = 3;
  state = run(state, [
    {
...
const characterCounterWindow = cache(buildCharacterCounterWindow);
...
export const NARUTO_FIXTURES: readonly NarutoFixture[] = [
  ...
  {
    id: "character-counter-window",
    label: "Character attack (K.O. preview)",
    description:
      "p1's character attacks a damaged, rested p2 character; the counter window shows the K.O. forecast.",
    group: "interaction",
    state: characterCounterWindow(),
    viewer: "p2",
  },
  {
  ...
  {
    id: "character-counter-window-mobile",
    label: "Mobile character attack (K.O. preview)",
    description:
      "The character-target counter window in the portrait board, including the incoming-damage and K.O. forecast.",
    group: "layout",
    state: characterCounterWindow(),
    viewer: "p2",
    mobile: true,
