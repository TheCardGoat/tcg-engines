...
export function characterEntity(
  ...
  if (character.damage > 0) {
    decorations.push({
      id: "damage",
      slot: "top-end",
      ariaLabel: `${character.damage} damage`,
      content: { kind: "text", text: `-${character.damage}` },
      tone: "negative",
    });
  }
  if (character.powerBonus > 0) {
    decorations.push({
      id: "power-bonus",
      slot: "top-start",
      ariaLabel: `${character.powerBonus} bonus power`,
      content: { kind: "text", text: `+${character.powerBonus}` },
      tone: "positive",
    });
  }
  if (chainLink !== null) {
    decorations.push({
      id: "chain-link",
      slot: "bottom-start",
      ariaLabel: `Chain link ${chainLink}`,
      content: { kind: "text", text: `#${chainLink}` },
      tone: "warning",
    });
  }
  return {
...
export function supportEntity(
  ...
  if (!visible) {
    const hidden = hiddenEntity(
      hiddenProjectionId(viewer, owner, "support", slotIndex),
      owner,
      "card",
    );
...
export function handCardEntity(
  ...
  if (owner !== viewer) {
    const hidden = hiddenEntity(hiddenProjectionId(viewer, owner, "hand", position), owner, "card");
...
export function summonEntity(state: GameState, owner: PlayerId): SimulatorEntity {
  const summon = state.players[owner].summon;
  const card = cardOf(summon);
  return {
    id: summon.uid,
    title: cardName(summon.cardId),
    subtitle: "Summon",
    kind: "card",
    ownerId: owner,
    face: "public",
    states: summon.rested ? ["rested"] : ["ready"],
    stats: [...stat("Power", card?.power), ...stat("Health", card?.health)],
    traits: [...(card?.traits ?? [])],
    imageUrl: cardImageUrl(summon.cardId),
    dataAttributes: { "data-board-uid": summon.uid, "data-rested": summon.rested },
  };
}
...
export function projectEntities(state: GameState, viewer: PlayerId): SimulatorEntity[] {
  ...
  for (const owner of ["p1", "p2"] as const) {
    ...
    entities.push(summonEntity(state, owner));
    ...
    for (const [position, instance] of player.hand.entries()) {
      entities.push(handCardEntity(owner, instance, viewer, position));
    }
