...
export function seatZones(state: GameState, owner: PlayerId, viewer: PlayerId): SimulatorZone[] {
  ...
  return [
    ...
    zone(
      ...
      "public",
      [],
      `${player.chakra.filter((c) => c.faceUp).length} face-up Chakra cards pay for Support cards`,
      { layoutHint: "row", count: player.chakra.length },
    ),
    zone(
      `${owner}-characters`,
      "Characters",
    ...
    zone(
      ...
      player.supports.flatMap((support, index) =>
        support
          ? [
              isViewer || support.revealed === true
                ? support.uid
                : hiddenProjectionId(viewer, owner, "support", index),
            ]
          : [],
      ),
    ...
    zone(
      ...
      player.hand.map((card, index) =>
        isViewer ? card.uid : hiddenProjectionId(viewer, owner, "hand", index),
      ),
      `${player.hand.length} cards`,
      { layoutHint: "fan", count: player.hand.length },
    ),
    zone(`${owner}-ex`, "EX", "custom", owner, "public", [], "EX pile", {
      layoutHint: "stack",
      count: player.exPile.length,
    }),
  ];
}
