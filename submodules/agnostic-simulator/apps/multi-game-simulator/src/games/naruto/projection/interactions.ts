...
export interface AttackTarget {
  ...
  /** Engine payload uid: character uid or the canonical opposing leader uid. */
...
export function characterActions(state: GameState, player: PlayerId, uid: string): ActionPill[] {
  ...
  if (hasCharacterActivateMain(location.character.cardId)) {
    pills.push(
...
export function leaderActions(state: GameState, player: PlayerId): ActionPill[] {
  ...
  if (hasLeaderEffect(leader.leaderId)) {
    pills.push(
...
export function legalAttackTargets(
  ...
  const targets: AttackTarget[] = [
    { uid: leaderUid(opponent), kind: "leader", targetUid: leaderUid(opponent) },
  ];
