...
/** Community-preview artwork is deliberately isolated in the Assets CDN. */
const NARUTO_CARD_CDN_BASE = "https://cdn.tcg.online/public/naruto/cards";

export function cardImageUrl(cardId: string): string {
  return `${NARUTO_CARD_CDN_BASE}/${encodeURIComponent(cardId)}.webp`;
}

export const CARD_BACK_URL = `${NARUTO_CARD_CDN_BASE}/card-back.webp`;
...
const LOG_TEMPLATES: Record<string, string> = {
  ...
  "log.deckOut": "{player} cannot draw - decked out.",
  ...
  "log.hitLeader": "{attacker} hits {target}'s Leader for {amount}. ({life} Life remaining)",
  "log.hitCharacter": "{attacker} hits {card} for {amount}.",
  ...
  "log.koTarget": "{card} is K.O.'d by an effect.",
  "log.bounce": "{card} returns to its owner's hand.",
  ...
  "log.revealNoMatch": "The revealed card does not match.",
  ...
  "log.victory": "{player} wins the game!",
};
...
export function logEntryText(
  entry: LogEntry,
  actorNames: { p1: string; p2: string },
  playerAliases: Readonly<Record<string, string>> = actorNames,
): string {
  const template = LOG_TEMPLATES[entry.key] ?? entry.key;
  const values: Record<string, string | number> = { ...entry.values };
  for (const key of ["player", "target"] as const) {
    const value = values[key];
    if (typeof value === "string" && playerAliases[value]) values[key] = playerAliases[value];
  }
  values.actor =
...
const DECK_ISSUE_TEXT: Record<DeckIssue, string> = {
  ...
  wrongChakraCount: "Deck must contain exactly 5 Chakra cards",
  unknownChakra: "Deck contains an unknown Chakra card",
  notAChakra: "Deck contains a non-Chakra card in its Chakra setup",
  noSummon: "Deck must contain exactly one Summon card",
  unknownSummon: "Deck contains an unknown Summon card",
  notASummon: "Deck contains a non-Summon card in its Summon setup",
};
