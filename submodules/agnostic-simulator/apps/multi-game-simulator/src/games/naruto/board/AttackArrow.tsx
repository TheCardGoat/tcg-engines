...
export function AttackArrow({ boardRef, attack, draft }: AttackArrowProps) {
  ...
  const previewRef = useRef<SVGGElement | null>(null);
  ...
  const damagePreview = attack
    ? attack.targetKind === "character"
      ? `${attack.damage} damage${attack.willKo ? " · K.O. if unchanged" : ""}`
      : `${attack.damage} damage to Leader`
    : null;
      ...
      if (Math.abs(d.length - lastDRef.current.length) > 0 || d !== lastDRef.current) {
        ...
        if (prev === "" || d !== prev) {
          ...
          previewRef.current?.setAttribute(
            "transform",
            `translate(${Math.min(svgRect.width - 104, Math.max(104, to.x))} ${Math.max(25, to.y - 38)})`,
          );
  ...
  return (
    <>
      {damagePreview ? (
        <output className={classes.attackAnnouncement} aria-live="polite">
          Pending attack: {damagePreview}.
        </output>
      ) : null}
      <svg
        ref={svgRef}
      ...
      >
        ...
        {damagePreview ? (
          <g ref={previewRef} data-testid="naruto-attack-preview">
            <rect
              x="-104"
              y="-13"
              width="208"
            ...
            />
            <text
              className={classes.attackPreviewText}
              textAnchor="middle"
