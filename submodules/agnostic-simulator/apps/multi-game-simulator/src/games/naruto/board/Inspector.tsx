...
import { getCardById, type Skill } from "@tcg-engines/naruto-cards";
...
function resolveInspected(
  ...
  if (zone === "leader" && owner) {
    ...
    return {
      ...
      skills: card.skills.map((s: Skill) => `[${s.labels.join(", ")}] ${s.text}`),
    };
  }
  if (zone === "character") {
  ...
  if ((zone === "hand" || zone === "trash") && owner) {
    ...
    const skills = card.support
      ? [
          ...
          ...card.skills.map((s: Skill) => `[${s.labels.join(", ")}] ${s.text}`),
        ]
      : card.skills.map((s: Skill) => `[${s.labels.join(", ")}] ${s.text}`);
    return {
      cardId: card.id,
  ...
  return (
    ...
    >
      ...
      {inspected ? (
        ...
        >
          <NarutoCardImage
            className={classes.inspectorArt}
            src={cardImageUrl(inspected.cardId)}
            alt=""
            fallbackLabel={inspected.title}
          />
          <div>
