...
import { unitPlacideVoodooSentinelRetail } from "@cyberpunk/testing/e2e-fixtures";

test("Placide - Voodoo Sentinel (Retail) - discards program to bottom-deck a rival unit", async ({
  page,
}) => {
  const pom = await createPlaywrightCyberpunkSimulatorPom(page, unitPlacideVoodooSentinelRetail);
