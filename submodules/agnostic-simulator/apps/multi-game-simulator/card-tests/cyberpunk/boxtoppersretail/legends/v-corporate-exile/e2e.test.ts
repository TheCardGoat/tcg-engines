...
import { alphaCorpoSecurity, boxTopperRetailVCorporateExile } from "@tcg/cyberpunk-cards";
...
import { legendVCorporateExileRetail } from "@cyberpunk/testing/e2e-fixtures";

test("V - Corporate Exile (Retail) - GO SOLO", async ({ page }) => {
  const pom = await createPlaywrightCyberpunkSimulatorPom(page, legendVCorporateExileRetail);

  const v = await pom.getCardInZoneByDefinitionId(
    "legendArea",
    CYBERPUNK_P1,
    boxTopperRetailVCorporateExile.id,
  );
