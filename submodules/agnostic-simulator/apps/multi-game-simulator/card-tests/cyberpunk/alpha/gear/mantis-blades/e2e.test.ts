...
test("Mantis Blades - host power boost", async ({ page }) => {
  ...
  const host = await pom.getCard(welcomeToNightCityRetailSwordwiseHuscle, { zone: "field" });
  const defender = await pom.getCard(welcomeToNightCityRetailCorpoSecurity, {
    zone: "field",
    player: CYBERPUNK_P2,
  });

  await pom.expectFieldSize(CYBERPUNK_P1, 1);
  await pom.expectFieldCardAttachedGearCount(welcomeToNightCityRetailSwordwiseHuscle, 1);
  await pom.expectFieldCardEffectivePower(welcomeToNightCityRetailSwordwiseHuscle, 7);
  ...
  await pom.getCard(welcomeToNightCityRetailCorpoSecurity, { zone: "trash", player: CYBERPUNK_P2 });
