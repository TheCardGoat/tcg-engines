...
test("Viktor Vektor - call searches top deck for gear", async ({ page }) => {
  ...
  await expect(page.getByRole("dialog")).toContainText("Choose Gear from trash");
  await expect(page.getByRole("dialog")).toContainText("Pick the Cyberware Gear to play for free.");
  ...
  await pom.expectPendingChoiceType(CYBERPUNK_P1, "chooseTarget");
  await expect(page.getByTestId("prompt-banner-title")).toContainText("Choose Unit to equip");
