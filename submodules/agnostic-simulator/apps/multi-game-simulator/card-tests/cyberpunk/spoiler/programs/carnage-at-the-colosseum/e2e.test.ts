...
import { progCarnageAtTheColosseum } from "@cyberpunk/testing/e2e-fixtures";

test("Carnage At The Colosseum - reduced cost and weaker rival target", async ({ page }) => {
  const pom = await createPlaywrightCyberpunkSimulatorPom(page, progCarnageAtTheColosseum);
