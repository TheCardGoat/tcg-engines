...
import { unitPlacideVoodooSentinel } from "@cyberpunk/testing/e2e-fixtures";

test("Placide - discards program to bottom-deck a rival unit", async ({ page }) => {
  const pom = await createPlaywrightCyberpunkSimulatorPom(page, unitPlacideVoodooSentinel);
