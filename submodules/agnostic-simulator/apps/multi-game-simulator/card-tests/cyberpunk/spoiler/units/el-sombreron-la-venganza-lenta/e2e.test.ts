...
import { unitElSombreronLaVenganzaLenta } from "@cyberpunk/testing/e2e-fixtures";

test("El Sombreron - attack trigger doubles fight power", async ({ page }) => {
  const pom = await createPlaywrightCyberpunkSimulatorPom(page, unitElSombreronLaVenganzaLenta);
