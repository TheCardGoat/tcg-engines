...
import type { ArtVariant, ArtVariantType, CardEffects, EventCard, StageCard } from "@tcg/op-types";
...
function renderI18n(card: EventCard | StageCard, constName: string): string {
...
function renderCard(
  card: EventCard | StageCard,
  constName: string,
  slug: string,
  num: string,
): string {
  const typeName = card.cardType === "event" ? "EventCard" : "StageCard";
  const lines = [
...
// setId → Map<cardId, EventCard | StageCard>
const eventsBySet = new Map<string, Map<string, EventCard>>();
const stagesBySet = new Map<string, Map<string, StageCard>>();
...
for (const set of sets) {
  ...
  for (const [cardId, { bases, variants }] of grouped) {
    ...
    let card: EventCard | StageCard;
    try {
      const normalized = normalize(baseRaw);
      if (normalized.cardType !== "event" && normalized.cardType !== "stage") continue;
      card = normalized;
    } catch {
      continue;
    }
    ...
    if (card.cardType === "event") {
      const bucket = eventsBySet.get(card.setId) ?? new Map<string, EventCard>();
      eventsBySet.set(card.setId, bucket);
      if (!bucket.has(cardId)) {
        bucket.set(cardId, card);
        eventCount++;
      }
    } else {
      const bucket = stagesBySet.get(card.setId) ?? new Map<string, StageCard>();
      stagesBySet.set(card.setId, bucket);
      if (!bucket.has(cardId)) {
        bucket.set(cardId, card);
        stageCount++;
      }
  ...
  console.log(`${eventCount} events, ${stageCount} stages`);
}
...
for (const [setId, events] of eventsBySet) {
  const setDir = join(CARDS_DIR, setId, "events");
...
for (const [setId, stages] of stagesBySet) {
  const setDir = join(CARDS_DIR, setId, "stages");
  mkdirSync(setDir, { recursive: true });

  const exports: string[] = [];

  for (const card of stages.values()) {
    const num = cardNumber(card.id);
    const name = card.i18n.en.name;
    const slug = slugify(name);
    const constName = toConstName(name, card.id, card.setId);
...
console.log(`\n✓ ${totalFiles} event/stage files written to packages/op-cards/src/cards/`);
