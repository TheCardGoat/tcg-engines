...
import type { ArtVariant, ArtVariantType, CardEffects, LeaderCard } from "@tcg/op-types";
...
function renderI18n(card: LeaderCard, constName: string): string {
...
function renderCard(card: LeaderCard, constName: string, slug: string, num: string): string {
  const lines = [
    `import type { LeaderCard } from "@tcg/op-types";`,
    `import { ${constName}I18n } from "./${num}-${slug}.i18n.ts";`,
    ``,
    `export const ${constName}: LeaderCard = {`,
...
// setId → Map<cardId, LeaderCard>
const leadersBySet = new Map<string, Map<string, LeaderCard>>();
...
for (const set of sets) {
  ...
  for (const [cardId, { bases, variants }] of grouped) {
    ...
    try {
      ...
      if (normalized.cardType !== "leader") continue;
      card = normalized;
    } catch {
      continue;
    }
    ...
    const bucket = leadersBySet.get(card.setId) ?? new Map<string, LeaderCard>();
    leadersBySet.set(card.setId, bucket);
  ...
  console.log(`${count} unique leaders`);
}
...
for (const [setId, leaders] of leadersBySet) {
  const setDir = join(CARDS_DIR, setId, "leaders");
  mkdirSync(setDir, { recursive: true });

  const exports: string[] = [];

  for (const card of leaders.values()) {
...
console.log(`\n✓ ${totalFiles} leader files written to packages/op-cards/src/cards/`);
