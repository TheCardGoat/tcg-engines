...
/**
 * Minimal cross-game identity fields (`canonicalId`, `slug`, `name`,
 * `printings`) for inline test-fixture cards. Required because
 * `SwuCardDefinition extends BaseCardDefinition` (RFC §7 / ADR-11).
 */
export function testCardIdentity(
  id: string,
  name: string,
): Pick<SwuCardDefinition, "canonicalId" | "slug" | "name" | "printings"> {
  return {
    canonicalId: id,
    slug: id,
    name,
    printings: [
      { id, artId: id, setCode: "TEST", collectorNumber: "0", rarity: "common", imageUrl: "" },
    ],
  };
}
