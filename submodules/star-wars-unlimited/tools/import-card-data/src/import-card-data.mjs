...
/**
 * Extract a CDN image URL from a Strapi media field (e.g. `artFront`).
 * Strapi wraps media as `{ data: { attributes: { url } } }`; handle missing data.
 */
function extractArtUrl(mediaField) {
...
/**
 * Stable printing id for a `{ set, number? }` set-code entry. Falls back to the
 * card's own id for entries without a collector number (e.g. tokens).
 */
function printingIdForSetCode(setCode, fallbackId) {
  if (setCode.number != null) {
    return `${setCode.set}_${String(setCode.number).padStart(3, "0")}`;
  }
  return fallbackId;
}
...
/**
 * Build the cross-game `printings[]` array (RFC §7 / ADR-3) for a canonical card.
 *
 * Each `SwuSetCode` entry (reprint or primary `setId`) becomes one printing.
 * Each merged variant (linked via upstream `variantOf`) becomes an additional
 * printing — this is where `variantOf` is exposed as `artId` (RFC §4 SWU row,
 * §10 SWU step 4). All printings of one canonical card share the same `artId`
 * (the canonical card's Strapi entry id, which is the value `variantOf.data.id`
 * points at) because SWU variants are treatment variants (Hyperspace / Showcase
 * / Foil) that reuse the canonical illustration.
 */
function buildPrintingsForCard(card, variants) {
  const printings = [];
  const canonicalArtId = String(card._strapiId);
  const setCodes = card.setCodes ?? (card.setId ? [card.setId] : []);

  for (const setCode of setCodes) {
    printings.push({
      id: printingIdForSetCode(setCode, card.id),
      artId: canonicalArtId,
      setCode: setCode.set,
      collectorNumber: setCode.number != null ? String(setCode.number) : "",
      rarity: card.rarity ?? "",
      imageUrl: card._artFront ?? "",
    });
  }
  ...
  for (const variant of variants) {
    printings.push({
      id: variant.id,
      artId: String(variant._variantOfParent),
      setCode: variant.setId?.set ?? "",
      collectorNumber: variant.setId?.number != null ? String(variant.setId.number) : "",
      rarity: variant.rarity ?? "",
      imageUrl: variant._artFront ?? "",
    });
  }
...
/**
 * Derive a URL-safe `slug` (RFC §7 / ADR-5) from `internalName`.
 *
 * `internalName` is already lowercased and ASCII-normalized by `normalizeCard`,
 * but it preserves the upstream `title#subtitle` separator (`#`) because
 * `canonicalId` is an opaque id where `#` is allowed. A `#` is a URL fragment
 * delimiter, so it must not appear in a URL-facing `slug`; replace it with `-`
 * (the same separator the surrounding slug-style code uses for whitespace, and
 * the same transformation `safeFileSegment` applies to card-definition paths).
 */
function slugFromInternalName(internalName) {
  return internalName.replaceAll("#", "-");
}

/**
 * Enrich normalized cards with cross-game identity fields (`canonicalId`,
 * `slug`, `name`, `printings`) per RFC §7 / §10 SWU.
 *
 * Variants (cards whose upstream `variantOf.data` is set) are merged into their
 * canonical parent's `printings[]` rather than emitted as separate card files,
 * keeping `(gameSlug, canonicalId)` unique (RFC ADR-1 / ADR-7). `canonicalId`
 * and `slug` are both seeded from `internalName` (stable, ASCII,
 * language-independent — RFC §10 SWU step 2 / Open Q4); `slug` is then
 * sanitized via {@link slugFromInternalName} so it stays URL-safe (ADR-5).
 *
 * Orphaned variants (whose canonical parent was filtered, e.g. a promo-set
 * parent) are emitted as standalone cards so no variant data is silently lost.
 */
function enrichWithPrintings(cards) {
  const variantsByParent = new Map();
  const canonicals = [];
  const variants = [];

  for (const card of cards) {
    if (card._variantOfParent) {
      variants.push(card);
    } else {
      canonicals.push(card);
    }
  }

  for (const variant of variants) {
    const siblings = variantsByParent.get(variant._variantOfParent) ?? [];
    siblings.push(variant);
    variantsByParent.set(variant._variantOfParent, siblings);
  }

  const canonicalStrapiIds = new Set(canonicals.map((card) => card._strapiId));
  const enriched = canonicals.map((card) => ({
    ...card,
    canonicalId: card.internalName,
    slug: slugFromInternalName(card.internalName),
    name: card.title,
    printings: buildPrintingsForCard(card, variantsByParent.get(card._strapiId) ?? []),
  }));

  for (const variant of variants) {
    if (canonicalStrapiIds.has(variant._variantOfParent)) continue;
    enriched.push({
      ...variant,
      canonicalId: variant.internalName,
      slug: slugFromInternalName(variant.internalName),
      name: variant.title,
      printings: buildPrintingsForCard(variant, []),
    });
  }
...
function normalizeCard(apiCard) {
  ...
  // Variants (variantOf.data !== null) are no longer discarded — they are merged
  // into their canonical parent's `printings[]` by `enrichWithPrintings`, and
  // their `variantOf` value is exposed as `artId` (RFC §10 SWU step 4).
  ...
  // Internal fields consumed by `enrichWithPrintings` (stripped before output).
  // `_strapiId` is the upstream entry id used to match variants to their parent
  // and as the canonical `artId`. `_variantOfParent` is non-null only on variant
  // cards (the parent's Strapi id). `_artFront` is the front-art CDN URL.
  card._strapiId = String(apiCard.id);
  card._variantOfParent = attributes.variantOf?.data?.id
    ? String(attributes.variantOf.data.id)
    : null;
  card._artFront = extractArtUrl(attributes.artFront);
...
async function writeCardDefinitionFiles(cards) {
  ...
  for (const card of cards) {
    ...
    const {
      ...
      helperImports,
      preamble,
      // Pull `printings` out before stripping so empty-string fields inside each
      // printing (rarity, imageUrl) are preserved — `Printing` requires them.
      printings: cardPrintings,
      // Strip importer-internal fields used by `enrichWithPrintings`.
      _strapiId: _internalStrapiId,
      _variantOfParent: _internalVariantOfParent,
      _artFront: _internalArtFront,
      ...cardDefinition
    } = card;
    const strippedDefinition = stripEmptyProperties(cardDefinition);
    strippedDefinition.printings = cardPrintings;
    if (abilitiesSource) {
...
async function main() {
  ...
  const processedCards = [...downloadedCardsById.values()]
    .map((card) => {
  ...
  // Merge variants into their canonical parent's `printings[]` and add the
  // cross-game identity fields (`canonicalId`, `slug`, `name`, `printings`).
  // Variants are not emitted as separate card files (RFC ADR-1 / ADR-7).
  const cards = enrichWithPrintings(processedCards);
