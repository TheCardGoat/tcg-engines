...
for (const deckId of process.argv.slice(2)) {
  ...
  for (const raw of fixture.arena.split("\n")) {
    ...
    if (!entry) { console.log(`  ?? ${name}`); continue; }
    const base = (entry.runtime as unknown as { base?: { typeBox?: { types?: string[] }; keywords?: string[]; abilities?: readonly Record<string, unknown>[] } }).base;
    console.log(`  ${name} keywords=${JSON.stringify(base?.keywords ?? [])}`);
    for (const a of base?.abilities ?? []) {
      console.log(`    [${a.kind}${a.abilityType ? "/" + a.abilityType : ""}] ${String(a.text).slice(0, 130)}`);
    }
  }
}
