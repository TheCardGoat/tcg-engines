/**
 * OUT175 Seeker's Hood — Generic Head (no printed defense).
 *
 * Printed:
 *   Instant - {r}, destroy Seeker's Hood: Prevent the next 1 damage that would
...
import { seekerSHood } from "../../../../../../cards/src/cards/equipment/seeker-s-hood.ts";
...
describe("seeker-s-hood (OUT175)", () => {
  it("core mechanic: Instant {r} destroy → prevent 1 + Opt 1", () => {
    ...
    Defender.defendWith([]);
    Attacker.pass();
    Defender.activate(seekerSHood);
    drain(game);
    game.helpers.resolveRestOfCombat();

    expect(Defender.zone("graveyard")).toContain(seekerSHood.canonicalId);
  ...
  it("boundaries: 0 RP illegal; model Instant destroy + prevent + opt", () => {
    ...
    expect(() => poor.as(bravo).activate(seekerSHood)).toThrow();

    const a1 = seekerSHood.base.abilities?.[0];
