/**
 * MPG115 Sunkwater Exoshell — Generic Chest d0 Blade Break.
 *
 * Printed:
 ...
 *   Blade Break
 *
 * Reasoning (case-by-case; Sunkwater Lookout twin):
...
import { sunkwaterExoshell } from "../../../../../../cards/src/cards/equipment/sunkwater-exoshell.ts";
...
describe("sunkwater-exoshell (MPG115)", () => {
  it("core mechanic: defend face-up arsenal bottom → draw + +1{d} + BB", () => {
    ...
    Defender.defendWith(sunkwaterExoshell);
    ...
    expect(Defender.zone("graveyard")).toContain(sunkwaterExoshell.canonicalId);
    expect(Defender.zone("chest")).not.toContain(sunkwaterExoshell.canonicalId);
  });
  ...
  it("boundaries: empty arsenal → no draw, d0 full, BB; face-down arsenal skipped", () => {
    ...
    EmptyDef.defendWith(sunkwaterExoshell);
    drain(empty);
    ...
    expect(EmptyDef.zone("graveyard")).toContain(sunkwaterExoshell.canonicalId);
    ...
    D.defendWith(sunkwaterExoshell);
    drain(facedown);
    ...
    expect(D.zone("graveyard")).toContain(sunkwaterExoshell.canonicalId);

    const a1 = sunkwaterExoshell.base.abilities?.[0];
    ...
    expect(sunkwaterExoshell.base.keywords?.some((k) => k.name === "blade-break")).toBe(true);
  });
});
