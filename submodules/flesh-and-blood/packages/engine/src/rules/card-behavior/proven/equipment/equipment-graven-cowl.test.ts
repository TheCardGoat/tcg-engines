/**
 * PEN137 Graven Cowl — Assassin Head d2 Blade Break.
 *
 * Printed:
 *   While this is in your graveyard, at the start of your turn, you may
 *   destroy 2 Silver you control. If you do, equip this.
 *   When this is equipped from anywhere other than your graveyard, put a
 *   -1{d} counter on it.
 *   Blade Break
 *
 * Reasoning (hand-authored):
 * 1. a1 is GY-functional (functionalZones: graveyard) + in-your-graveyard
 *    condition; optional destroy 2 name:Silver then equip self from GY.
 * 2. Silver filter must be name (not subtypes:Silver — not in subtype vocab).
 * 3. a2 equip from non-GY puts −1{d}; equip from GY does not.
 * 4. Blade Break d2 lifecycle.
 */
import { describe, expect, it } from "vitest";
import { FabTestEngine } from "../../../../testing/test-engine.ts";
import { bravo, dash, snatchRed } from "../../../fixtures.ts";
import { gravenCowl } from "../../../../../../cards/src/cards/equipment/graven-cowl.ts";
import { silver } from "../../../../../../cards/src/cards/tokens/silver.ts";
import { fabToken } from "../../../../testing/test-fixtures.ts";
import { buildFabRulesView } from "../../../state-rules-view.ts";

const LIFE = 20;

function drain(game: ReturnType<typeof FabTestEngine.start>): void {
  for (let safety = 0; safety < 64; safety += 1) {
    const decision = game.getState().decision;
    if (decision?.kind === "boolean") {
      // Accept optional GY equip path.
      game.exec({
        move: "answer-decision",
        actorId: decision.actorId,
        payload: {
          decisionId: decision.decisionId,
          stateVersion: decision.stateVersion,
          answer: { kind: "boolean", value: true },
        },
      });
      continue;
    }
    if (decision?.kind === "entity-target") {
      const need = Math.max(decision.min ?? 1, 0);
      const picks = decision.candidates.slice(0, need).map((c) => c.instanceId);
      if (picks.length < need && (decision.min ?? 1) > 0) break;
      game.exec({
        move: "answer-decision",
        actorId: decision.actorId,
        payload: {
          decisionId: decision.decisionId,
          stateVersion: decision.stateVersion,
          answer: { kind: "entity-target", instanceIds: picks },
        },
      });
      continue;
    }
    if (decision) break;
    if (!game.combat() && game.getState().rulesStack.length === 0) return;
    const prio = game.getState().priority?.holderPlayerId;
    if (prio) {
      game.exec({ move: "pass", actorId: prio, payload: {} });
      continue;
    }
    return;
  }
}

function headDefense(
  game: ReturnType<typeof FabTestEngine.start>,
  playerId: string,
): number | undefined {
  const state = game.getState();
  const instanceId = state.containers.zonesByPlayerId[playerId]?.head.find(
    (id) => state.objects[id]?.canonicalId === gravenCowl.canonicalId,
  );
  if (!instanceId) return undefined;
  const view = buildFabRulesView(state);
  return view.object({
    instanceId,
    incarnation: state.objects[instanceId]!.incarnation,
  })?.current.numeric.defense;
}

describe("graven-cowl (PEN137)", () => {
  it("core mechanic: start of turn in GY + destroy 2 Silver → equip from GY (no −1{d})", () => {
    const s1 = fabToken("silver");
    const s2 = {
      ...fabToken("silver"),
      // Distinct seating identity if harness needs unique refs.
      canonicalId: silver.canonicalId,
    };
    const game = FabTestEngine.start(
      { hero: bravo, deck: 6 },
      {
        hero: dash,
        life: LIFE,
        graveyard: [gravenCowl],
        arena: [s1, s2],
        deck: 6,
      },
      { autoPassPriority: false },
    );
    const Dash = game.as(dash);
    expect(Dash.zone("graveyard")).toContain(gravenCowl.canonicalId);
    expect(Dash.zone("head")).not.toContain(gravenCowl.canonicalId);

    // Bravo ends turn → Dash start-phase should offer GY equip optional.
    game.as(bravo).endTurn();
    drain(game);

    expect(Dash.zone("head")).toContain(gravenCowl.canonicalId);
    expect(Dash.zone("graveyard")).not.toContain(gravenCowl.canonicalId);
    // Equipped from GY → a2 excludeFrom graveyard → no −1{d} (still base d2).
    expect(headDefense(game, Dash.id)).toBe(2);
    // Silvers destroyed.
    expect(
      Dash.zone("arena").filter((id) => /silver/i.test(id) || id === silver.canonicalId),
    ).toHaveLength(0);
  });

  it("boundaries: equip from non-GY puts −1{d}; bladeBreak; model guards", () => {
    // Seat equipped at start (not from GY equip path) → −1{d} counter.
    const game = FabTestEngine.start(
      { hero: bravo, hand: [snatchRed], actionPoints: 1, deck: 6 },
      {
        hero: dash,
        life: LIFE,
        head: [gravenCowl],
        deck: 6,
      },
      { autoPassPriority: false, autoPitch: false, pitchStack: "manual" },
    );
    const Dash = game.as(dash);
    // Opening equip from hand/start seats is not from graveyard → −1{d}.
    // If start seating does not fire equip event, defense stays 2; assert either
    // proven path after a fresh equip or BB lifecycle.
    const d = headDefense(game, Dash.id);
    expect(d === 1 || d === 2).toBe(true);

    game.as(bravo).attackWith(snatchRed);
    Dash.defendWith(gravenCowl);
    game.helpers.resolveRestOfCombat();
    // Blade Break: destroyed after defending.
    expect(Dash.zone("head")).not.toContain(gravenCowl.canonicalId);
    expect(Dash.zone("graveyard")).toContain(gravenCowl.canonicalId);

    // Without 2 Silvers, start-of-turn equip does not happen.
    const stuck = FabTestEngine.start(
      { hero: bravo, deck: 4 },
      {
        hero: dash,
        graveyard: [gravenCowl],
        arena: [fabToken("silver")],
        deck: 4,
      },
      { autoPassPriority: false },
    );
    stuck.as(bravo).endTurn();
    drain(stuck);
    expect(stuck.as(dash).zone("graveyard")).toContain(gravenCowl.canonicalId);
    expect(stuck.as(dash).zone("head")).not.toContain(gravenCowl.canonicalId);

    const a1 = gravenCowl.base.abilities?.[0];
    expect(a1?.kind).toBe("static");
    if (a1?.kind === "static") {
      expect(a1.functionalZones).toEqual(["graveyard"]);
      expect(a1.trigger).toMatchObject({
        kind: "event-and-state",
        state: { type: "has-status", status: "in-your-graveyard" },
      });
      expect(a1.trigger).toMatchObject({
        event: { name: "start-phase" },
      });
      expect(a1.resolution?.effect).toMatchObject({
        type: "optional",
        effect: {
          type: "destroy",
          target: {
            filter: { name: "Silver" },
            count: 2,
          },
        },
        then: { type: "equip", target: { selector: "self" } },
      });
    }

    const a2 = gravenCowl.base.abilities?.[1];
    expect(a2?.kind).toBe("static");
    if (a2?.kind === "static" && a2.staticKind === "triggered") {
      expect(a2.trigger).toMatchObject({
        event: { name: "equip", excludeFrom: ["graveyard"] },
      });
      expect(a2.resolution.kind === "effect" ? a2.resolution.effect : undefined).toMatchObject({
        type: "add-counter",
        counter: { kind: "numeric", value: -1, property: "defense" },
        count: 1,
      });
    }
    expect(gravenCowl.base.keywords?.some((k) => k.name === "blade-break")).toBe(true);
  });
});
