...
import { toughLeatherBoots } from "../../../../../../cards/src/cards/equipment/tough-leather-boots.ts";
...
function bootsDefense(
  ...
  const instanceId = state.containers.zonesByPlayerId[playerId]?.legs.find(
    (id) => state.objects[id]?.canonicalId === toughLeatherBoots.canonicalId,
  );
...
describe("tough-leather-boots (SUP003)", () => {
  it("AAA: controlling Toughness and Vigor gives +2 defense", () => {
    const game = FabTestEngine.start(
      { hero: dash, hand: [snatchRed], actionPoints: 1, deck: 6 },
      {
        hero: bravo,
        legs: [toughLeatherBoots],
        arena: [fabToken("toughness"), fabToken("vigor")],
        life: 20,
        deck: 6,
      },
    ...
    Defender.defendWith(toughLeatherBoots);
    game.helpers.resolveRestOfCombat();

    expect(Defender.life()).toBe(19);
    expect(Defender.zone("graveyard")).toContain(toughLeatherBoots.canonicalId);
  });

  it("boundary: either token alone leaves base defense", () => {
    for (const token of ["toughness", "vigor"] as const) {
      const game = FabTestEngine.start(
        { hero: bravo, legs: [toughLeatherBoots], arena: [fabToken(token)], deck: 6 },
        { hero: dash, deck: 6 },
        { autoPassPriority: false },
      );
