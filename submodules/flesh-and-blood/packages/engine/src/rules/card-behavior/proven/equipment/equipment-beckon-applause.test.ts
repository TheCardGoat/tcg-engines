 ...
 *   Temper
 *
 * Reasoning (case-by-case; arms twin of HVY011 raw-meat):
 * 1. Continuous control-object gates for Agility / Vigor tokens — duration was
 *    this-turn (drops EOT while token remains). Fixed permanent re-eval
 *    (raw-meat / lignum / red-alert family).
 ...
 *
 * Status: ✅ Agility/Vigor +d continuous; both stack; Temper destroy; model.
 */
import { describe, expect, it } from "vitest";
import { FabTestEngine } from "../../../../testing/test-engine.ts";
import { bravo, dash, snatchRed } from "../../../fixtures.ts";
import { fabToken } from "../../../../testing/test-fixtures.ts";
import { beckonApplause } from "../../../../../../cards/src/cards/equipment/beckon-applause.ts";
...
function armsDefense(
  ...
  const instanceId = state.containers.zonesByPlayerId[playerId]?.arms.find(
    (id) => state.objects[id]?.canonicalId === beckonApplause.canonicalId,
  );
...
describe("beckon-applause (HVY100)", () => {
  it("proven: base d0 with Temper seats; no tokens → d0", () => {
    ...
    expect(Bravo.zone("arms")).toContain(beckonApplause.canonicalId);
    expect(beckonApplause.base.numeric.defense).toBe(0);
    expect(armsDefense(game, Bravo.id)).toBe(0);
  });

  it("core mechanic: Agility token → +1{d}; Vigor alone → +1{d}; both → +2{d}", () => {
    ...
    expect(armsDefense(agilityOnly, agilityOnly.as(bravo).id)).toBe(1);

    const vigorOnly = FabTestEngine.start(
      {
        hero: bravo,
        arms: [beckonApplause],
    ...
    expect(armsDefense(vigorOnly, vigorOnly.as(bravo).id)).toBe(1);
    ...
    expect(armsDefense(both, both.as(bravo).id)).toBe(2);
  });
  ...
  it("core mechanic: with Agility, defend for d1 then Temper destroy", () => {
    ...
    Defender.defendWith(beckonApplause);
    game.helpers.resolveRestOfCombat();

    // Snatch 4 − d1 = 3 damage; Temper −1 then d0 → destroy.
    expect(Defender.life()).toBe(LIFE - (SNATCH - 1));
    expect(Defender.zone("arms")).not.toContain(beckonApplause.canonicalId);
    expect(Defender.zone("graveyard")).toContain(beckonApplause.canonicalId);
  });
  ...
  it("model guard: continuous permanent + control Agility/Vigor Token", () => {
    const a1 = beckonApplause.base.abilities?.[0];
    const a2 = beckonApplause.base.abilities?.[1];
