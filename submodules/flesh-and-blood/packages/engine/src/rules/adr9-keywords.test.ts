...
describe("ADR-9 label keyword expansion", () => {
  ...
  it("Bracken Rap expands Earth Bond from the label plus unique rider", () => {
    const def = toFabCardDefinition(brackenRapRed as never);
    expect(baseHasKeyword(def, "earth-bond")).toBe(true);
    const bond = def.base.abilities?.find((ability) => ability.label?.name === "earth-bond");
