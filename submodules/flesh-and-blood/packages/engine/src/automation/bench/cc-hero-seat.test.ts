...
describe("CC catalog seating for hero-profile", () => {
  ...
  it("seats an already-present tournament CC list without a hero-binding throw", { timeout: 60_000 }, () => {
    const played = playFabMatch({
  ...
  it("does not spend the converting budget cancel-looping Savage Feast", { timeout: 60_000 }, () => {
    const played = playFabMatch({
  ...
  it("converts a CC self-play instead of burning the cap on empty priority passes", { timeout: 120_000 }, () => {
    const played = playFabMatch({
