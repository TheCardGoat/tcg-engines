...
import { crackedBaubleYellow } from "./cracked-bauble.ts";

describe("Cracked Bauble (WTR224) AAA", () => {
  it("happy: pitches for 2 resources", () => {
    ...
    Dash.play(nimbleStrikeRed, { pitch: [crackedBaubleYellow] });

    expectFabCard(Dash, crackedBaubleYellow).toBeIn("pitch");
  ...
  it("boundary: cannot be played", () => {
    ...
    expect(() => game.as(dash).play(crackedBaubleYellow)).toThrow();
    expectFabCard(game.as(dash), crackedBaubleYellow).toBeIn("hand");
  });
