...
import { silkenShroud } from "./silken-shroud.ts";

/**
 * Silken Shroud (PEN123) — Illusionist Equipment - Head, ward(1).
 * Printed: "When this is destroyed, create a Ponder token."
...
describe("Silken Shroud (PEN123) AAA", () => {
  it("happy: ward-negating an arcane hit destroys this and creates a Ponder", () => {
    const game = FabTestEngine.start(
      { hero: chane, hand: [arcanicCrackleBlue], actionPoints: 1, deck: 6 },
      { hero: dash, hand: [], head: [silkenShroud], life: 20, deck: 6 },
      FAB_MANUAL_HARNESS,
    );
    ...
    expectFabCard(game.as(dash), silkenShroud).toBeIn("graveyard");
    expectFabPlayer(game.as(dash)).toHaveTokenCount("ponder", 1);
  ...
  it("timing: a fully blocked arcane attack still trips the ward destroy", () => {
    const game = FabTestEngine.start(
      { hero: chane, hand: [arcanicCrackleBlue], actionPoints: 1, deck: 6 },
      { hero: dash, hand: [nimblismBlue], head: [silkenShroud], life: 20, deck: 6 },
      FAB_MANUAL_HARNESS,
    );
    const Chane = game.as(chane);
