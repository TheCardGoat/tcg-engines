...
import { silkenShawl } from "./silken-shawl.ts";
import { nimblismBlue } from "../actions/nimblism.ts";

/**
 * Silken Shawl (PEN124) — Illusionist Equipment - Chest, ward(1).
 * Printed: "When this is destroyed, create a Vigor token."
...
describe("Silken Shroud (PEN123) AAA", () => {
  it("happy: ward-negating an arcane hit destroys this and creates a Vigor", () => {
    const game = FabTestEngine.start(
      { hero: chane, hand: [arcanicCrackleBlue], actionPoints: 1, deck: 6 },
      { hero: dash, hand: [], chest: [silkenShawl], life: 20, deck: 6 },
      FAB_MANUAL_HARNESS,
    );
    ...
    expectFabCard(game.as(dash), silkenShawl).toBeIn("graveyard");
  ...
  it("timing: a fully blocked arcane attack still trips the ward destroy", () => {
    const game = FabTestEngine.start(
      { hero: chane, hand: [arcanicCrackleBlue], actionPoints: 1, deck: 6 },
      { hero: dash, hand: [nimblismBlue], chest: [silkenShawl], life: 20, deck: 6 },
      FAB_MANUAL_HARNESS,
    );
    const Chane = game.as(chane);
