...
import { goldenHeartPlate } from "./golden-heart-plate.ts";

/**
 * Golden Heart Plate (SUP248) — Equipment - Chest. (Legendary, Temper)
...
describe("Golden Heart Plate (SUP248) AAA", () => {
  it("happy: a destroy-Gold cost can destroy this as though it were a Gold", () => {
    ...
    Dash.accept();
    Dash.target(goldenHeartPlate);
    game.helpers.resolveUntilIdle();

    expectFabCard(Dash, goldenHeartPlate).toBeIn("graveyard");
  });
  ...
  it("boundary: declining the cost leaves this seated", () => {
    ...
    expectFabCard(Dash, goldenHeartPlate).toBeIn("chest");
  });
  ...
  it("timing: destroyed as a Gold, its slot frees within the same turn", () => {
    ...
    expect(Dash.zone("chest")).toHaveLength(0);
