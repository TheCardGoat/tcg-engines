...
import { sunkwaterScalers } from "./sunkwater-scalers.ts";

describe("Sunkwater Scalers (MPG117) AAA", () => {
  it("happy: defending with a face-up arsenal card draws and grants +1{d} this turn", () => {
    ...
    expectFabCard(Bravo, sunkwaterScalers).toHaveKeyword("blade-break");
    game.as(dash).attackWith(snatchRed);
    game.advanceCombatTo("defend");
    Bravo.defendWith(sunkwaterScalers);
    game.helpers.resolveUntilIdle({ entityTargets: "minimum" });

    expect(Bravo.zone("deck")[0]).toBe(nimblismBlue.canonicalId);
    expectFabCard(Bravo, brutalAssaultBlue).toBeIn("hand");
    expectFabCard(Bravo, sunkwaterScalers).toBeIn("graveyard");
  ...
  it("timing: Blade Break destroys this after it defends", () => {
    ...
    expectFabCard(Bravo, sunkwaterScalers).toBeIn("graveyard");
    expect(Bravo.zone("legs")).toHaveLength(0);
  });
});
