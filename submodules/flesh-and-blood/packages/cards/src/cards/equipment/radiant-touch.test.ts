...
import { radiantTouch } from "./radiant-touch.ts";

/**
 * Radiant Touch (DTD077) — Light Equipment Arms.
 *
...
describe("Radiant Touch (DTD077) AAA", () => {
  it("happy: banish this and a soul card to prevent the next 2 of Snatch's 4", () => {
    ...
    game.as(dash).pass();
    Boltyn.activate(radiantTouch);
    game.helpers.resolveUntilIdle();
    game.helpers.resolveRestOfCombat();

    expectFabPlayer(Boltyn).toHaveLife(18);
    expectFabCard(Boltyn, radiantTouch).toBeBanished();
  ...
  it("boundary: without activating, Snatch deals the full 4 and this stays equipped", () => {
    ...
    expectFabCard(Boltyn, radiantTouch).toBeIn("arms");
  ...
  it("timing: empty soul cannot pay the Instant cost", () => {
    ...
    game.advanceCombatTo("reaction");
    game.as(dash).pass();
    Boltyn.expectActivationRejected(radiantTouch);
