import { describe, expect, it } from "vitest";
import {
  FAB_MANUAL_HARNESS,
  FabTestEngine,
  expectFabCard,
} from "@tcg/flesh-and-blood-engine/testing";
import { dash } from "../heroes/dash.ts";
import { oldhimGrandfatherOfEternity } from "../heroes/oldhim-grandfather-of-eternity.ts";
import { protoBaseChest } from "./proto-base-chest.ts";

/**
 * Proto Base Chest (TCC004) — Mechanologist Equipment - Base Chest.
 *
 * Printed: (vanilla — no abilities, no printed stats)
 *
 * AAA trio:
 * - Happy: seats in the chest slot at game start.
 * - Boundary: no activated abilities and no printed defense — cannot be
 *   activated or declared as a defender.
 * - Timing: equipment persists across a full turn cycle.
 */

describe("Proto Base Chest (TCC004) AAA", () => {
  // ── Happy path ────────────────────────────────────────────────────────────

  it("happy: seats in the chest slot at game start", () => {
    const game = FabTestEngine.start(
      { hero: dash, chest: [protoBaseChest], deck: 6 },
      { hero: oldhimGrandfatherOfEternity, deck: 6 },
      FAB_MANUAL_HARNESS,
    );
    const Dash = game.as(dash);

    expect(Dash.zone("chest")).toHaveLength(1);
    expect(Dash.zone("chest")).toContain(protoBaseChest.canonicalId);
    expectFabCard(Dash, protoBaseChest).toBeIn("chest");
  });

  // ── Boundary ───────────────────────────────────────────────────────────────

  it("boundary: no abilities and no defense — cannot be activated or declared as a defender", () => {
    const game = FabTestEngine.start(
      { hero: dash, chest: [protoBaseChest], actionPoints: 1, deck: 6 },
      { hero: oldhimGrandfatherOfEternity, deck: 6 },
      FAB_MANUAL_HARNESS,
    );
    const Dash = game.as(dash);

    expect(() => Dash.activate(protoBaseChest)).toThrow();
    expect(() => Dash.defendWith(protoBaseChest)).toThrow();
    expectFabCard(Dash, protoBaseChest).toBeIn("chest");
  });

  // ── Timing / persistence ───────────────────────────────────────────────────

  it("timing: equipment persists across a full turn cycle", () => {
    const game = FabTestEngine.start(
      { hero: dash, chest: [protoBaseChest], deck: 6 },
      { hero: oldhimGrandfatherOfEternity, deck: 6 },
      FAB_MANUAL_HARNESS,
    );
    const Dash = game.as(dash);
    const Oldhim = game.as(oldhimGrandfatherOfEternity);

    Dash.endTurn();
    Oldhim.endTurn();

    expect(Dash.zone("chest")).toHaveLength(1);
    expect(Dash.zone("chest")).toContain(protoBaseChest.canonicalId);
  });
});
