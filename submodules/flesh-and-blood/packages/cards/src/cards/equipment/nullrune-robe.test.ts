...
import { nullruneRobe } from "./nullrune-robe.ts";

describe("Nullrune Robe (ARC156) AAA", () => {
  it("happy: pay 1 resource to prevent 1 of Voltic Bolt's arcane damage", () => {
    const game = FabTestEngine.start(
      ...
      { hero: dash, life: 20, resourcePoints: 1, chest: [nullruneRobe], deck: 6 },
      FAB_MANUAL_HARNESS,
    );
    ...
    expectFabCard(Dash, nullruneRobe).toBeIn("chest");
  });
  ...
  it("boundary: declining Arcane Barrier takes the full 3 arcane", () => {
    ...
    expectFabCard(Dash, nullruneRobe).toHaveKeyword("arcane-barrier");
  });
});
