...
import { goldenGauntlets } from "./golden-gauntlets.ts";

/**
 * Golden Gauntlets (SUP249) — Equipment - Arms. (Legendary, Temper)
...
describe("Golden Gauntlets (SUP249) AAA", () => {
  it("happy: a destroy-Gold cost can destroy this as though it were a Gold", () => {
    ...
    Dash.accept();
    Dash.target(goldenGauntlets);
    game.helpers.resolveUntilIdle();

    expectFabCard(Dash, goldenGauntlets).toBeIn("graveyard");
  });
  ...
  it("boundary: declining the cost leaves this seated", () => {
    ...
    expectFabCard(Dash, goldenGauntlets).toBeIn("arms");
  });
  ...
  it("timing: destroyed as a Gold, its slot frees within the same turn", () => {
    ...
    expect(Dash.zone("arms")).toHaveLength(0);
