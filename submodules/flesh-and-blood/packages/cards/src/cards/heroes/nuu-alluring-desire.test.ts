...
import { nuuAlluringDesire } from "./nuu-alluring-desire.ts";

/**
 * Nuu, Alluring Desire (MST001) — Mystic Assassin Hero.
 *
...
describe("Nuu, Alluring Desire (MST001) AAA", () => {
  it("happy: look at a blue top and banish it", () => {
    ...
    Nuu.activate(nuuAlluringDesire);
    game.untilIdle({ optionals: "accept", ordering: "listed" });

    // The whole {c}{c}{c} was paid and the blue top card moved to Dash's
    // banished zone.
  ...
  it("boundary: the banish is optional — declining keeps the blue top in the deck", () => {
    ...
    game.as(nuuAlluringDesire).activate(nuuAlluringDesire);
  ...
  it("boundary: unpayable {c}{c}{c} is rejected", () => {
    const game = FabTestEngine.start(
      { hero: nuuAlluringDesire, chiPoints: 2, actionPoints: 1, deck: 6 },
      { hero: dash, hand: [], deck: 6 },
      FAB_MANUAL_HARNESS,
    );

    game.as(nuuAlluringDesire).expectActivationRejected(nuuAlluringDesire);
  });
});
