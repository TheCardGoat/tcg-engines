...
import { bigBopRed } from "./big-bop.ts";

describe("Big Bop (BET015) AAA", () => {
  it("happy: next Guardian attack gets +5{p} and may wager Vigor", () => {
    ...
    game.untilIdle();
    expectFabCard(Bravo, bigBopRed).toBeIn("graveyard");
  ...
  it("timing: does not fire at the start of the opponent's turn", () => {
    const game = FabTestEngine.start(
      { hero: bravo, arena: [bigBopRed], deck: 6 },
    ...
    game.untilIdle();
    expectFabCard(game.as(bravo), bigBopRed).toBeIn("arena");
  });
});
