...
import { restlessBonesRed } from "./restless-bones.ts";

describe("Restless Bones family AAA", () => {
  it("happy: discarding a watery-grave card when this attacks grants go again", () => {
    const game = FabTestEngine.start(
      {
        hero: gravyBones,
        hand: [restlessBonesRed, barnacleYellow],
        resourcePoints: 2,
        actionPoints: 1,
        deck: 6,
      },
  ...
  it("boundary: discarding a card without watery grave does not grant go again", () => {
    const game = FabTestEngine.start(
      {
        hero: gravyBones,
        hand: [restlessBonesRed, nimblismBlue],
        resourcePoints: 2,
        actionPoints: 1,
        deck: 6,
      },
