...
import { temptOverYellow } from "./tempt-over.ts";

describe("Tempt Over (OMN215) AAA", () => {
  it("happy: attacking a hero steals an aura token they control", () => {
    const game = FabTestEngine.start(
      { hero: bravo, hand: [temptOverYellow], actionPoints: 1, deck: 6 },
    ...
    Bravo.playAttack(temptOverYellow);
  ...
  it("boundary: with no aura token, the attack still deals 3", () => {
    const game = FabTestEngine.start(
      { hero: bravo, hand: [temptOverYellow], actionPoints: 1, deck: 6 },
