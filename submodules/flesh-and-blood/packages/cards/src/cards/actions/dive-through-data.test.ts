...
import { diveThroughDataRed } from "./dive-through-data.ts";

/**
 * Dive Through Data (EVO189) — Mechanologist Attack Action. Boost.
 *
 * Printed: When this hits, opt 1.
 */

describe("Dive Through Data family AAA", () => {
  it("happy: an unblocked hit lands and the opt-1 keep runs", () => {
    const game = FabTestEngine.start(
      {
        hero: dash,
        hand: [diveThroughDataRed],
        resourcePoints: 1,
        actionPoints: 1,
        deck: 6,
      },
    ...
    expectFabPlayer(Kano).toHaveLife(11); // observed exact
  });
});
