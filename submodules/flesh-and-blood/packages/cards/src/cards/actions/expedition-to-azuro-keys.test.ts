...
import { expeditionToAzuroKeysRed } from "./expedition-to-azuro-keys.ts";

/**
 * Expedition to Azuro Keys (SEA155) — Pirate Action - Attack, cost 0, 4{p}.
 *
 * Printed: When this attacks, you may put a gold counter on Treasure Island.
 */

describe("Expedition to Azuro Keys (SEA155) AAA", () => {
  it("happy: attacking may put a gold counter on Treasure Island", () => {
    const game = FabTestEngine.start(
      {
        hero: gravyBones,
        macros: [treasureIsland],
        hand: [expeditionToAzuroKeysRed],
        resourcePoints: 0,
        actionPoints: 1,
        deck: 6,
      },
    ...
    Gravy.playAttack(expeditionToAzuroKeysRed, { stopAt: "on-attack" });
    game.advanceUntil({
      stopAt: "defend",
      ordering: "listed",
      optionals: "accept",
