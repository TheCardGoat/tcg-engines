...
import { gasGuzzlerRed } from "./gas-guzzler.ts";

describe("Gas Guzzler family AAA", () => {
  it("happy: boosting a Mechanologist card banishes it and grants go again", () => {
    ...
    Teklo.attackWith(gasGuzzlerRed, { boost: true });
    ...
    expectFabCard(Teklo, gasGuzzlerRed).toBeIn("graveyard");
  ...
  it("boundary: without boosting it does not banish or gain go again", () => {
    ...
    Teklo.attackWith(gasGuzzlerRed, { boost: false });
