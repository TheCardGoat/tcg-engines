...
import { cullingsongGloombladeRed } from "./cullingsong-gloomblade.ts";
import { chane } from "../heroes/chane.ts";
import { brutalAssaultBlue } from "./brutal-assault.ts";
import { sinkBelowRed } from "../defense-reactions/sink-below.ts";
describe("cullingsong-gloomblade preview behavior", () => {
  for (const from of ["hand", "banished"] as const) {
    it(`hit from ${from} banishes the defender's hand card`, () => {
  ...
  it("a fully defended attack does not banish a card", () => {
    const game = FabTestEngine.start(
      { hero: chane, hand: [cullingsongGloombladeRed], resourcePoints: 0, deck: 6 },
      { hero: dash, hand: [brutalAssaultBlue], arsenal: [sinkBelowRed], life: 20, deck: 6 },
      FAB_MANUAL_HARNESS,
    );
    game.as(chane).playAttack(cullingsongGloombladeRed);
