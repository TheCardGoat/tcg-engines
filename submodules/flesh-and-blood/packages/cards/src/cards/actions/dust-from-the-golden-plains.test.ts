...
import { dustFromTheGoldenPlainsRed } from "./dust-from-the-golden-plains.ts";

/**
 * Dust from the Golden Plains (DYN002) — Draconic Illusionist Action - Ash.
 *
 * Printed: Material - While this is under a permanent other than Themai,
 * that permanent has phantasm.
 */

describe("Dust from the Golden Plains (DYN002) AAA", () => {
  it("happy: hosted under another permanent it arms phantasm; its paired dragon gains nothing", () => {
    const game = FabTestEngine.start(
      {
        ...
        resourcePoints: 3,
        actionPoints: 3,
        deck: [dustFromTheGoldenPlainsRed],
      },
      { hero: dash, arena: [themai], deck: 6 },
      FAB_MANUAL_HARNESS,
    );
    ...
    // Themai, and its paired twin under Themai itself.
    game.createObject({
      instanceId: "dust-under-free-host",
      canonicalId: dustFromTheGoldenPlainsRed.canonicalId,
      ownerId: Dromai.id,
      zone: "under",
    });
    ...
    const namedHost = Dash.cardIn("arena", themai).instanceId;
    ...
    expectFabCard(Dash, themai).notToHaveKeyword("phantasm");
  });
});
