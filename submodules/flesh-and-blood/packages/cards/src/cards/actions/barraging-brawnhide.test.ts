...
import { barragingBrawnhideRed } from "./barraging-brawnhide.ts";

describe("Barraging Brawnhide family AAA", () => {
  it("happy: fewer than 2 non-equipment defenders grants +1{p}", () => {
    const game = FabTestEngine.start(
      { hero: dash, hand: [barragingBrawnhideRed], resourcePoints: 3, actionPoints: 1, deck: 6 },
      { hero: azalea, hand: [], deck: 6 },
      FAB_MANUAL_HARNESS,
    );
    game.as(dash).play(barragingBrawnhideRed);
    game.passBoth();
