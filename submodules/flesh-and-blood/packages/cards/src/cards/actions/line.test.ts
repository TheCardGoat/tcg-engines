...
import { lineBlue } from "./line.ts";

/**
 * Line (SEA104) — Ranger Action.
 *
 * Printed:
 *   Look at the top card of your deck. If it's an arrow, you may put it
 *   face-up into your arsenal. If you do, it gets go again this turn.
 *   Go again
 */

describe("Line (SEA104) AAA", () => {
  it("happy: the arsenal'd arrow gets go again this turn", () => {
  ...
  it("timing: go again refunds the play AP even when the top card is not an arrow", () => {
    ...
    expectFabPlayer(Azalea).toHaveAP(1);
    expectFabCard(Azalea, lineBlue).toBeIn("graveyard");
  });
});
