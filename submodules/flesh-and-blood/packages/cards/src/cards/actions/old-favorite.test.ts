...
import { oldFavoriteYellow } from "./old-favorite.ts";

/**
 * Old Favorite (SUP023) — When this attacks, if you've been cheered this turn, create a Toughness token.
 */

describe("Old Favorite (SUP023) AAA", () => {
  it("happy: after being cheered this turn, attacking creates a Toughness token", () => {
    const game = FabTestEngine.start(
      {
        hero: tuffnut,
        hand: [oldFavoriteYellow],
        resourcePoints: 3,
        actionPoints: 1,
    ...
    Tuffnut.playAttack(oldFavoriteYellow, { stopAt: "on-attack" });
    game.untilIdle({ optionals: "accept", ordering: "listed" });
