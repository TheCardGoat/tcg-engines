...
import { embermawCenipaiRed } from "./embermaw-cenipai.ts";

/**
 * Embermaw Cenipai Red (DRO016) — Illusionist Attack Action. Phantasm.
 *
 * Printed: When Phantasmal Haze is destroyed, create a Spectral Shield
 * token.
 */

describe("Phantasmal Haze (DRO016) AAA", () => {
  it("happy: phantasm-destroyed by a 6{p} attack action, a Shield is created", () => {
    const game = FabTestEngine.start(
      { hero: prism, hand: [embermawCenipaiRed], resourcePoints: 3, actionPoints: 1, deck: 6 },
    ...
    Prism.playAttack(embermawCenipaiRed);
  ...
  it("boundary: defended by a non-attack card, no destroy and no token", () => {
    ...
    expectFabPlayer(Bravo).toHaveLife(14); // 20 - (power - 2 nimblism block)
  });
});
