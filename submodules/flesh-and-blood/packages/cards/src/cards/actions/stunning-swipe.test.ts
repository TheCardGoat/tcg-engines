...
import { stunningSwipeRed } from "./stunning-swipe.ts";

/**
 * Stunning Swipe Red (OMN074) — Lightning Runeblade Attack.
 *
...
describe("Singeing Flowstride (OMN074) AAA", () => {
  it("happy: an unblocked swing lands its printed damage", () => {
    const game = FabTestEngine.start(
      {
        hero: briar,
        hand: [stunningSwipeRed],
        resourcePoints: 1,
        actionPoints: 1,
        deck: 6,
      },
    ...
    Briar.playAttack(stunningSwipeRed);
    Dash.defendWith();
    game.helpers.resolveUntilIdle({ ordering: "listed" });

    expectFabPlayer(Dash).toHaveLife(17); // 20 - 3
    expectFabCard(Briar, stunningSwipeRed).toBeIn("graveyard");
  });
});
