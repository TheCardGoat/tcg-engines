...
import { rebelliousRushRed } from "./rebellious-rush.ts";

/**
 * Rebellious Rush (FAI017) — Draconic Ninja Action - Attack, cost 2, 5{p}/2{d}. Printed: Go again.
 */

describe("Rebellious Rush (FAI017) AAA", () => {
  it("happy: after the attack resolves, leftover AP is refunded", () => {
    const game = FabTestEngine.start(
      { hero: bravo, hand: [rebelliousRushRed], resourcePoints: 2, actionPoints: 1, deck: 6 },
      { hero: dash, hand: [], life: 20, deck: 6 },
    );
    const Bravo = game.as(bravo);

    Bravo.playAttack(rebelliousRushRed);
  ...
  it("timing: illegal as an instant during the reaction step", () => {
    const game = FabTestEngine.start(
      {
        hero: dash,
        hand: [brutalAssaultBlue, rebelliousRushRed],
        resourcePoints: 2,
        actionPoints: 1,
        deck: 6,
      },
    ...
    expectFabUnplayable(
      () => Dash.must.playReaction(rebelliousRushRed),
      /action card is not legal|arrow can only be played/i,
    );
    expectFabCard(Dash, rebelliousRushRed).toBeIn("hand");
  });
});
