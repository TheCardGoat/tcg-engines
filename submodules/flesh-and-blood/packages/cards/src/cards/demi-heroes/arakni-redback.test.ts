...
import { arakniRedback } from "./arakni-redback.ts";

describe("Arakni, Redback (HNT006) AAA", () => {
  it("happy: at the beginning of your end phase this returns to the brood while remaining in arena", () => {
    const game = FabTestEngine.start(
      { hero: arakniMarionette, arena: [arakniRedback], hand: [], deck: 6 },
    ...
    expectFabCard(Arakni, arakniRedback).toBeIn("arena");
  });

  it("boundary: an Agent in inventory does not return to the brood", () => {
    const game = FabTestEngine.start(
      { hero: arakniMarionette, inventory: [arakniRedback], hand: [], deck: 6 },
      { hero: dash, hand: [], deck: 6 },
      FAB_MANUAL_HARNESS,
    );
    const Arakni = game.as(arakniMarionette);
    ...
    expect(Arakni.zone("inventory")).toContain(arakniRedback.canonicalId);
  });
