...
import { sistersOfFireRed } from "./sisters-of-fire.ts";

/**
 * Sisters of Fire (HNT112) — Draconic Warrior Attack Reaction, cost 0, 3{d}.
 *
 * Printed: "Play this only if you control 2 or more Draconic chain links.
 * Target dagger attack gets +3{p}."
 */

describe("Sisters of Fire (HNT112) AAA", () => {
  it("happy: two Draconic chain links let a dagger attack get +3{p}", () => {
    const game = FabTestEngine.start(
      {
        hero: fang,
        weapon1: [obsidianFireVein],
        hand: [huntToTheEndsOfRatheRed, sistersOfFireRed],
        resourcePoints: 1,
        actionPoints: 1,
        deck: 6,
      },
  ...
  it("boundary: a single Draconic chain link cannot play this", () => {
    ...
    Fang.must.activate(obsidianFireVein);
    game.advanceCombatTo("reaction");
    expect(() => Fang.must.playReaction(sistersOfFireRed)).toThrow(
      /play condition is not satisfied/,
    );
    expectFabCard(Fang, sistersOfFireRed).toBeIn("hand");
  });

  it("timing: the +3{p} applies in the reaction step before damage", () => {
    const game = FabTestEngine.start(
      {
        hero: fang,
        weapon1: [obsidianFireVein],
