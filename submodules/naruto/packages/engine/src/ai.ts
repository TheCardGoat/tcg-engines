...
import type { CharacterInstance, ChoiceOption, EffectKind, GameState, PlayerId } from "./types";
...
export function chooseAiAction(state: GameState, player: PlayerId): Action | null {
  ...
  const wipeSlot = me.supports.findIndex((support, index) => {
    ...
    if (!supportEffectOf(support.cardId)?.koAll) return false;
  ...
  for (const attacker of attackers) {
    const target = opponent.characters.find(
      (c): c is CharacterInstance =>
        c !== null &&
        canBeAttacked(state, c.uid) &&
        attacker.power >= characterHealth(c) - c.damage,
    );
    if (target) {
      return {
        type: "DECLARE_ATTACK",
  ...
  if (first) {
    return {
      ...
      targetKind: "leader",
      targetUid: leaderUid(otherPlayer(player)),
    };
  }
  ...
  if (
    faceUpChakra(me) === 0 &&
    !me.leaderRested &&
    state.turn >= PROVISIONAL_RULES.recoveryFromTurn
  ) {
