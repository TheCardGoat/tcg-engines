...
import { supportEffectOf, supportEffectSummonsCard } from "./support-effects";
import type { SupportTiming } from "./support-effects";
import type {
  CardInstance,
  CharacterInstance,
  ChoiceOption,
...
export function boardCharacters(state: GameState, playerIds: readonly PlayerId[]): ChoiceOption[] {
...
export function hasCharacterRoom(state: GameState, player: PlayerId): boolean {
  ...
  return (
    limit === null || state.players[player].characters.filter((c) => c !== null).length < limit
  );
}
...
/**
 * Support immunity is scoped to an opponent's Support effects. Character and
 * Leader effects deliberately bypass it, even when they use the same target
 * selection machinery as a Support effect.
 */
export function isImmuneToEffect(
  ...
  return sourceKind === "support" && isImmuneTo(state, character, sourcePlayer, targetOwner);
}
...
/** KO candidates, applying immunity only when the source is a Support effect. */
export function koTargets(
  state: GameState,
  sourcePlayer: PlayerId,
  sourceKind: EffectSourceKind,
  filter: KoFilter,
): ChoiceOption[] {
  return boardCharacters(state, ["p1", "p2"]).filter((option) => {
    const character =
      option.index === null ? null : state.players[option.owner].characters[option.index];
    if (!character) return false;
    if (isImmuneToEffect(state, character, sourceKind, sourcePlayer, option.owner)) return false;
...
export function canBeAttacked(state: GameState, uid: string): boolean {
  ...
  return (
    location !== null && (PROVISIONAL_RULES.canAttackStandingCharacter || location.character.rested)
  );
}
...
const NATIVE_RUSH_CARD_IDS = new Set(["N-014", "N-naruto-ex"]);

const CONDITIONAL_RUSH_POWER_BY_CARD_ID: Readonly<Record<string, number>> = {
  "N-007": 10,
};

/** Reviewed native or conditional Rush behavior, keyed by card id. */
export function hasRush(character: CharacterInstance, turn: number): boolean {
  if (character.rushUntilTurn >= turn) return true;
  if (character.effectsNegated) return false;
  if (NATIVE_RUSH_CARD_IDS.has(character.cardId)) return true;
  const minimumPower = CONDITIONAL_RUSH_POWER_BY_CARD_ID[character.cardId];
  return minimumPower !== undefined && effectivePower(character, turn) >= minimumPower;
}
...
export function supportTimingClass(card: CardDefinition | undefined): SupportTiming {
  if (!card?.support) return "unknown";
  return supportEffectOf(card.id)?.timing ?? "unknown";
}
...
export function supportActivationBlock(
  ...
  if (supportEffectSummonsCard(instance.cardId) && !hasCharacterRoom(state, player)) {
    return "noSlot";
  }
  return null;
}
...
export function hasActivatableSupport(state: GameState, player: PlayerId): boolean {
  return (
    ...
    state.players[player].hand.some(
      (card) => handSupportBlock(state, player, card.uid, true) === null,
    )
  );
}
...
export const EX_REQUIREMENTS_BY_CARD_ID: Readonly<Record<string, readonly ExRequirement[]>> = {
  "N-005": [{ minPower: 10 }],
  "N-014": [{}, { trait: "The Taka" }],
  "N-022": [{}],
  "N-naruto-ex": [{}, { minPower: 10 }],
};

/** Reviewed EX summon requirements keyed by card id. */
export function exRequirements(cardId: string): readonly ExRequirement[] {
  return EX_REQUIREMENTS_BY_CARD_ID[cardId] ?? [];
}
...
export function requirementCandidates(
  ...
  return boardCharacters(state, [player]).filter((option) => {
    ...
    const character = option.index === null ? null : state.players[player].characters[option.index];
    if (!character) return false;
    if (
      requirement.minPower !== undefined &&
      effectivePower(character, state.turn) < requirement.minPower
    ) {
      return false;
    }
    if (
      requirement.trait !== undefined &&
      !(cardOf(character)?.traits ?? []).includes(requirement.trait)
    ) {
      return false;
    }
    return true;
