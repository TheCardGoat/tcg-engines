...
function resolveAttack(state: GameState): void {
  ...
  if (attack.targetKind === "leader") {
    const defenderId = otherPlayer(attack.attacker);
    if (attack.targetUid !== leaderUid(defenderId)) return;
    ...
    pushLog(state, attack.attacker, "log.hitLeader", {
...
export function resolveChain(state: GameState): void {
  ...
  while (state.chain.length > 0) {
    ...
    // A life-zero result is terminal. Do not settle further links or resolve a
    // pending attack that could overwrite the winner.
    if (state.winner) return;
...
export function applyAction(state: GameState, action: Action): GameState {
  ...
  switch (action.type) {
    ...
    case "SUMMON": {
      ...
      if (
        actor.summon.rested ||
        actor.summonsUsedThisTurn >= PROVISIONAL_RULES.normalSummonsPerTurn
      ) {
        return state;
      }
    ...
    case "DECLARE_ATTACK": {
      if (action.attackerKind === "leader" && action.attackerUid !== leaderUid(action.player)) {
        return state;
      }
      const targetPlayer = otherPlayer(action.player);
      if (action.targetKind === "leader") {
        if (action.targetUid !== leaderUid(targetPlayer)) return state;
      }
      const block =
