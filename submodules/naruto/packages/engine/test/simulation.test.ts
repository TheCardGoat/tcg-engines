...
import { createInitialState, PREVIEW_DECKS, previewDeckList } from "../src/setup";
import type { GameState, PlayerId } from "../src/types";

const MAX_ACTIONS = 20_000;

/** Count of main-deck cards across all zones (must always equal 50). */
...
describe("full-game simulation (greedy AI vs greedy AI)", () => {
  ...
  it("plays a Preview-deck mirror match to completion", () => {
    const takaPreviewDeck = PREVIEW_DECKS[2];
    if (!takaPreviewDeck) throw new Error("missing Preview deck");
    const taka = previewDeckList(takaPreviewDeck);
