...
import { NARUTO_PREVIEW_RULES_PROFILE } from "../src/rules";
import type { CharacterInstance, GameState, PlayerId, PlayerState } from "../src/types";
...
export function barePlayer(id: PlayerId, leaderId: string): PlayerState {
  return {
    ...
    chakra: Array.from({ length: 5 }, (_, i) => ({
      uid: `${id}-chakra-${i}-C-001`,
      cardId: "C-001",
      faceUp: true,
    })),
    exPile: [],
    summon: { uid: `${id}-summon-S-001`, cardId: "S-001", rested: false },
