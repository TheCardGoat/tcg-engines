...
import { canBeAttacked, characterAttackBlock, leaderAttackBlock } from "../src/queries";
...
describe("combat", () => {
  it("attack on leader reduces life by the attacker's damage stat", () => {
    ...
    expect(resolved.log.find((entry) => entry.key === "log.hitLeader")?.values).toEqual({
      attacker: "Naruto Uzumaki",
      target: "p2",
      amount: statOf("N-004", "damage"),
      life: 15 - statOf("N-004", "damage"),
    });
  ...
  it("rejects malformed Leader attacker and target identities", () => {
    const state = bareState();
    const wrongAttacker = applyAction(state, {
      type: "DECLARE_ATTACK",
      player: "p1",
      attackerUid: "leader:p2",
      attackerKind: "leader",
      targetKind: "leader",
      targetUid: "leader:p2",
    });
    expect(wrongAttacker).toBe(state);

    const wrongTarget = applyAction(state, {
      type: "DECLARE_ATTACK",
      player: "p1",
      attackerUid: "leader:p1",
      attackerKind: "leader",
