...
describe("chain & counter step", () => {
  it("counter support activation goes on the chain; interrupt cancels the attack", () => {
    ...
    expect(activated.log.some((entry) => entry.key === "log.attackInterrupted")).toBe(true);
  });
  ...
  it("stops chain resolution at a life-zero loss before the pending attack can overwrite it", () => {
    const state = bareState();
    state.players.p1.life = 2;
    state.players.p2.life = 1;
    // N-006 opens the counter chain; K-039 negates it and pays 2 life.
    state.players.p2.supports[0] = { uid: "counter", cardId: "N-006" };
    state.players.p1.supports[0] = { uid: "response", cardId: "K-039" };
    const declared = declareAttackOnLeader(state);
    const countered = applyAction(declared, { type: "ACTIVATE_SUPPORT", player: "p2", slot: 0 });
    expect(countered.priority).toBe("p1");

    const terminal = applyAction(countered, { type: "ACTIVATE_SUPPORT", player: "p1", slot: 0 });

    expect(terminal.winner).toBe("p2");
    expect(terminal.players.p1.life).toBe(0);
    // N-004's pending attack would otherwise reduce this 1-life Leader to
    // zero and overwrite the p2 victory.
    expect(terminal.players.p2.life).toBe(1);
    expect(terminal.pendingAttack).not.toBeNull();
    expect(terminal.log.some((entry) => entry.key === "log.hitLeader")).toBe(false);
  });
