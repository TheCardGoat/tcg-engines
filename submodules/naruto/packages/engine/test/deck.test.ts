...
describe("deck validation", () => {
  it("flags a missing leader", () => {
    expect(deckIssues({ leaderId: "", cardIds: [], ...previewSideCards })).toEqual(["noLeader"]);
  });

  it("flags an unknown or non-leader leader", () => {
    expect(deckIssues({ leaderId: "NOPE-1", cardIds: [], ...previewSideCards })).toEqual([
      "unknownLeader",
    ]);
    expect(deckIssues({ leaderId: "N-004", cardIds: [], ...previewSideCards })).toEqual([
      "unknownLeader",
    ]);
  });

  it("flags wrong deck size", () => {
    expect(deckIssues({ leaderId: "N-001", cardIds: ["N-004"], ...previewSideCards })).toContain(
      "wrongSize",
    );
  });
  ...
  it("flags non-character cards in the deck", () => {
    ...
    expect(deckIssues({ leaderId: "N-001", cardIds, ...previewSideCards })).toContain(
      "notACharacter",
    );
  });
  ...
  it("flags off-color cards", () => {
    ...
    expect(deckIssues({ leaderId: "N-001", cardIds, ...previewSideCards })).toContain("wrongColor");
  });
  ...
  it("requires five Chakra ids and one correctly typed Summon id", () => {
    const deck = buildDeck("N-001");
    expect(deckIssues({ ...deck, chakraCardIds: deck.chakraCardIds.slice(0, 4) })).toContain(
      "wrongChakraCount",
    );
    expect(
      deckIssues({ ...deck, chakraCardIds: ["NOPE", ...deck.chakraCardIds.slice(1)] }),
    ).toContain("unknownChakra");
    expect(
      deckIssues({ ...deck, chakraCardIds: ["N-004", ...deck.chakraCardIds.slice(1)] }),
    ).toContain("notAChakra");
    expect(deckIssues({ ...deck, summonCardId: "" })).toContain("noSummon");
    expect(deckIssues({ ...deck, summonCardId: "NOPE" })).toContain("unknownSummon");
    expect(deckIssues({ ...deck, summonCardId: "N-004" })).toContain("notASummon");
  });
});
