import type { I18nProperties, Languages } from "@tcg/lorcana-types";

export const buzzLightyearProvidingCoverPD1PromoI18n: Record<Languages, I18nProperties> = {
  en: {
    name: "Buzz Lightyear",
    version: "Providing Cover",
    text: [
      {
        title: "Action Figure",
        description:
          "When you play this character, choose one of the following. If you have another Toy character in play, choose both instead:",
      },
      {
        title:
          "• You may return an action card with cost 2 or less from your discard to your hand.",
      },
      {
        title: "• You may play an action with cost 2 or less for free.",
      },
    ],
  },
  de: {
    name: "Buzz Lightyear",
    version: "Bietet Schutz",
    text: [
      {
        title: "Actionfigur",
        description:
          "Wenn du diesen Charakter ausspielst, wähle eine Möglichkeit aus. Falls du mindestens einen anderen Spielzeug-Charakter im Spiel hast, wähle stattdessen beide Möglichkeiten aus:",
      },
      {
        title:
          "• Du darfst eine Aktionskarte, die 2 oder weniger kostet, von deinem Ablagestapel zurück auf deine Hand nehmen.",
      },
      {
        title: "• Du darfst eine Aktion, die 2 oder weniger kostet, kostenlos ausspielen.",
      },
    ],
  },
  fr: {
    name: "Buzz l'Éclair",
    version: "Assure la couverture",
    text: [
      {
        title: "Pantin articulé",
        description:
          "Lorsque vous jouez ce personnage, choisissez l'une de ces options. Si vous avez un autre personnage Jouet en jeu, choisissez les deux à la place:",
      },
      {
        title:
          "• Vous pouvez renvoyer dans votre main une carte Action coûtant 2 ou moins de votre défausse.",
      },
      {
        title: "• Vous pouvez jouer gratuitement une action coûtant 2 ou moins.",
      },
    ],
  },
  it: {
    name: "Buzz Lightyear",
    version: "Providing Cover",
    text: [
      {
        title: "Action Figure",
        description:
          "When you play this character, choose one of the following. If you have another Toy character in play, choose both instead:",
      },
      {
        title:
          "• You may return an action card with cost 2 or less from your discard to your hand.",
      },
      {
        title: "• You may play an action with cost 2 or less for free.",
      },
    ],
  },
  es: {
    name: "Buzz Lightyear",
    version: "Proporcionar cobertura",
    text: [
      {
        title: "Figura de acción",
        description:
          "Cuando interpretes a este personaje, elige uno de los siguientes. Si tienes otro personaje de juguete en juego, elige ambos:",
      },
      {
        title:
          "• Puedes devolver a tu mano una carta de acción con coste 2 o menos de tu descarte.",
      },
      {
        title: "• Puedes jugar una acción con coste 2 o menos de forma gratuita.",
      },
    ],
  },
};
