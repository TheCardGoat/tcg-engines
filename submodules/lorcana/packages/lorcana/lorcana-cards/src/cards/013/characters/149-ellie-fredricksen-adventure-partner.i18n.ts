import type { I18nProperties, Languages } from "@tcg/lorcana-types";

export const ellieFredricksenAdventurePartnerI18n: Record<Languages, I18nProperties> = {
  en: {
    name: "Ellie Fredricksen",
    version: "Adventure Partner",
    text: [
      {
        title: "Always with You",
        description:
          "When this character is banished, you may put this card into your inkwell facedown and exerted.",
      },
    ],
  },
  de: {
    name: "Ellie Fredricksen",
    version: "Abenteuerpartnerin",
    text: [
      {
        title: "Immer bei dir",
        description:
          "Wenn dieser Charakter verbannt wird, darfst du diese Karte verdeckt und erschöpft in deinen Tintenvorrat legen.",
      },
    ],
  },
  fr: {
    name: "Ellie Fredricksen",
    version: "Partenaire d'aventure",
    text: [
      {
        title: "Toujours avec toi",
        description:
          "Lorsque ce personnage est banni, vous pouvez placer cette carte dans votre réserve d'encre, face cachée et épuisée.",
      },
    ],
  },
  it: {
    name: "Ellie Fredricksen",
    version: "Compagna di Avventure",
    text: [
      {
        title: "Sempre con Te",
        description:
          "Quando questo personaggio viene esiliato, puoi aggiungere questa carta al tuo calamaio, a faccia in giù e impegnata.",
      },
    ],
  },
  es: {
    name: "Ellie Fredricksen",
    version: "Compañero de aventuras",
    text: [
      {
        title: "siempre contigo",
        description:
          "Cuando este personaje sea desterrado, puedes poner esta carta en tu tintero boca abajo y ejercerla.",
      },
    ],
  },
};
