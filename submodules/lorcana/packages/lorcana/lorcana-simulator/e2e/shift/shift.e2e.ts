...
const MALEFICENT_DIABLO_FREE_SHIFT_FIXTURE_ID = "maleficent-diablo-free-shift";
...
test.describe("Shift", () => {
  ...
  test("named shift ('Aladdin or Genie'): both Aladdin and Genie targets are valid", async ({
    page,
  }) => {
    const pom = new LorcanaSimulatorPom(page);
    await pom.goto({ fixtureId: SHIFT_FIXTURE_ID, view: PLAYER_ONE_VIEW });
    ...
    const validTargetIds = [
      findCardIdByLabel(board, PLAYER_ONE_ID, "play", "Aladdin - Resolute Swordsman"),
      findCardIdByLabel(board, PLAYER_ONE_ID, "play", "Genie - On the Job"),
    ];
    const invalidTargetIds = [
      findCardIdByLabel(board, PLAYER_ONE_ID, "play", "Baloo - Laid-Back Bear"),
  ...
  for (const targetLabel of ["Aladdin - Resolute Swordsman", "Genie - On the Job"] as const) {
    test(`Aladdin & Genie shifts onto ${targetLabel} and reaches the play trigger`, async ({
      page,
    }) => {
      const pom = new LorcanaSimulatorPom(page);
      await pom.goto({ fixtureId: SHIFT_FIXTURE_ID, view: PLAYER_ONE_VIEW });

      await shiftCardOntoTarget(pom, "Aladdin & Genie - Mischievous Pals", targetLabel);

      const board = await pom.getBoard(PLAYER_ONE_VIEW);
      const shiftedCardId = board.players[PLAYER_ONE_ID]?.play.find(
        (id) => board.cards[id]?.fullName === "Aladdin & Genie - Mischievous Pals",
      );
      const bagEffect = board.bagEffects[0];

      expect(shiftedCardId).toBeDefined();
      expect(board.cards[shiftedCardId!]?.playedViaShift).toBe(true);
      expect(bagEffect?.payload?.abilityName).toBe("SLEIGHT OF HAND");
      ...
      await expect(pom.asBottomPlayer()).toHaveCardInZone({
        card: "Aladdin & Genie - Mischievous Pals",
        zone: "play",
      });
    });
  }

  test("Maleficent & Diablo free shift lets the player choose five discard characters, then the character to shift onto", async ({
    page,
  }) => {
    const pom = new LorcanaSimulatorPom(page);
    await pom.goto({ fixtureId: MALEFICENT_DIABLO_FREE_SHIFT_FIXTURE_ID, view: PLAYER_ONE_VIEW });
    ...
    const shiftTargetId = findCardIdByLabel(
      board,
      PLAYER_ONE_ID,
      "play",
      "Maleficent - Exultant Spellcaster",
    );
    const discardCostIds = [
      "Aladdin - Brave Rescuer",
      "Baymax - Giant Robot",
      "Chip - Friend Indeed",
      "Dale - Bumbler",
      "Moana - Determined Explorer",
    ].map((label) => findCardIdByLabel(board, PLAYER_ONE_ID, "discard", label));

    await page
      .locator(`[data-card-id="${handCardId}"]`)
      .first()
      .getByRole("button", { name: "Shift" })
      .dispatchEvent("click");
    await page.getByRole("button", { name: /Shift: 5 ink/ }).click();
    await expect(page.getByText(/Choose 5 character cards/)).toBeVisible();

    for (const discardCostId of discardCostIds) {
      await page.locator(`[data-card-id="${discardCostId}"]`).first().click();
    }

    await expect(page.getByText("Choose a shift target")).toBeVisible();

    const previousStateId = board.stateID;
    await page.locator(`[data-card-id="${shiftTargetId}"]`).first().click();
    ...
    await expect(
      page.getByRole("button", {
        name: /Shifted 1 cards under Maleficent & Diablo - Evil Incarnate/,
      }),
    ).toBeVisible();
  });
});
