...
describe("canonicalListHash", () => {
  ...
  it("v2 hashes are game-prefixed and ignore printing-only differences", () => {
    ...
    expect(canonicalV2ListHash("lorcana", base)).toBe(canonicalV2ListHash("lorcana", alternate));
    expect(canonicalV2ListHash("lorcana", base)).not.toBe(canonicalV2ListHash("gundam", base));
    expect(canonicalV2ListHash("lorcana", base).startsWith("lorcana:v2:")).toBe(true);
  });
});
