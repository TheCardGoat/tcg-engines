...
function splitPrivateMessageValues(message: EngineLogMessage): {
  ...
  for (const [playerId, value] of Object.entries(split.privateValueByPlayerId)) {
    ...
    privateMessagesByPlayerId[playerId] = {
      key: message.key,
      values: mergeMessageValues(publicValues, value as Record<string, EngineLogMessageValue>),
...
function mergeMessageValues(
  publicValues: Record<string, EngineLogMessageValue>,
  privateValues: Record<string, EngineLogMessageValue>,
): Record<string, EngineLogMessageValue> {
  const merged: Record<string, EngineLogMessageValue> = { ...publicValues };
  for (const [key, value] of Object.entries(privateValues)) {
    const publicValue = merged[key];
    merged[key] = publicValue === undefined ? value : mergeMessageValue(publicValue, value);
  }
  return merged;
}

function mergeMessageValue(
  publicValue: EngineLogMessageValue,
  privateValue: EngineLogMessageValue,
): EngineLogMessageValue {
  if (Array.isArray(publicValue) && Array.isArray(privateValue)) {
    const mergedItems: EngineLogMessageValue[] = [];
    const length = Math.max(publicValue.length, privateValue.length);
    for (let index = 0; index < length; index += 1) {
      const publicItem = publicValue[index];
      const privateItem = privateValue[index];
      if (publicItem !== undefined && privateItem !== undefined) {
        mergedItems.push(mergeMessageValue(publicItem, privateItem));
      } else if (index in privateValue) {
        mergedItems.push(privateItem as EngineLogMessageValue);
      } else {
        mergedItems.push(publicItem as EngineLogMessageValue);
      }
    }
    return mergedItems;
  }

  if (isMessageObject(publicValue) && isMessageObject(privateValue)) {
    const merged: Record<string, EngineLogMessageValue> = { ...publicValue };
    for (const [key, value] of Object.entries(privateValue)) {
      merged[key] =
        key in merged ? mergeMessageValue(merged[key] as EngineLogMessageValue, value) : value;
    }
    return merged;
  }
...
function isMessageObject(
  value: EngineLogMessageValue,
): value is Record<string, EngineLogMessageValue> {
