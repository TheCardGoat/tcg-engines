...
import {
  type ModalEffectMode,
  modalEffectPredicate,
...
import {
  calculateHowManyTargets,
  doesAbilityTargetPlayer,
...
export class StackLayerModel {
  ...
  skipEffect(recursiveCall = false): MoveResponse {
    this.rootStore.stackLayerStore.removeLayerFromStack(this);
    this.rootStore.log({
      type: "SKIP_EFFECT",
    ...
    if (!recursiveCall && this.ability.hasDependentEffects) {
      logger.trace(
        "No valid targets for dependent effect, Skipping resolution of dependant effects",
        this.ability.name,
      );
      this.rootStore.stackLayerStore.layers
        .filter(
          (layer) => layer.ability.ability.name === this.ability.ability.name,
        )
        .forEach((layer) => {
          console.log(
            "Skipping resolution of dependant effects",
            layer.ability.name,
          );
  ...
  resolve(
    ...
    opts: { skipLogs?: boolean; skipResolution?: boolean } = {},
  ) {
    if (params.skip) {
    ...
    if (this.ability.optional && !this.ability.accepted) {
      ...
      const acceptedAbility = new AbilityModel(
    ...
    if (!this.ability.isValidResolvingParams(params, this.responder)) {
      this.rootStore.debug(
        "Invalid target for resolving effect: ",
        this.ability.name,
      );
    ...
    if (this.ability.type === "resolution" && this.ability.costs) {
      if (
        !this.source.canPayCosts(this.ability.costs, [], this.ability.responder)
      ) {
        this.rootStore.log({
          type: "CANT_PAY_COSTS",
          //effect: this.toJSON()
  ...
  isInvalidTargetResolution(skipResolution?: boolean) {
    if (skipResolution) {
      return false;
    }
