...
export class CardModel {
  ...
  private abilities: AbilityModel[];
  ...
  get inkwell() {
    const hiddenInkCastersInPlay = this.rootStore.tableStore
      ...
      .zones.play.cards.filter(
        (card) => card.fullName.toLowerCase() === "hidden inkcaster",
      ).length;
  ...
  get color(): CardColor {
    const card = this.lorcanitoCard;
    return card?.colors[0] as CardColor;
  }
  ...
  matchesTargetFilter(
    ...
    const predicate = createTargetFiltersPredicate(
      ...
      filters,
      effectSource,
      undefined, // TODO: This is probably wrong
      params,
    );
  ...
  applyShiftCostModifiers(initialCost = 0) {
    ...
    const costAttributeModifier =
      this.rootStore.effectStore.getCostAttributeModifier(this);
  ...
  canPayShiftCosts(cardCosts: CardModel[] = []) {
    return canPayCosts(this.shiftCosts, cardCosts, this, this.rootStore);
  }

  canPayCosts(costs: Cost[], targets: CardModel[] = [], payingPlayer?: string) {
  ...
  get singerCost(): number {
    ...
    const singCostModifier =
      this.rootStore.effectStore.getSingCostModifier(this);
  ...
  get moveCost(): number {
    ...
    const cardMoveCostModifier =
      this.rootStore.effectStore.getMoveCostModifier(this);
  ...
  get challengeTimeAttackerStrength(): number {
    ...
    const totalWithModifiersAndChallenger =
      original + strengthModifier + this.challengerBonus;
  ...
  get challengeTimeDefenderStrength(): number {
    ...
    const reverseChallengerAbility =
      this.rootStore.effectStore.getAbilitiesForCard(this, [
  ...
  get alternativeCosts(): Cost[] | undefined {
    const find = this.getNativeAbilities.find((model) =>
      alternateCostAbilityPredicate(model.ability),
    );
      ...
      .map((model) => {
        ...
        if (convertGainedAbilities && gainStaticAbilityPredicate(ability)) {
          return new AbilityModel(
            { ...ability.gainedAbility, conditions: ability.conditions },
            model.source,
            this.rootStore,
            this.observable,
          );
  ...
  getActivatedAbility(abilityName?: string) {
    return this.rootStore.effectStore.getActivatedAbilityForCard(
      this,
      abilityName,
    );
  }
  ...
  get hasResist() {
    return (
      this.getStaticAbility("resist").filter(
        (ability) =>
          resistAbilityPredicate(ability.ability) &&
          !ability.ability.onlyWhileChallenge,
      ).length > 0
    );
  }
  ...
  get playConditions() {
    return this.abilities.filter(
      (ability) => ability.type === "play-condition",
    );
  }
  ...
  exert({ skipTriggers }: { skipTriggers?: boolean } | undefined = {}) {
  ...
  get hasAtStartOfTurnReadyRestriction() {
    ...
    const cardEffects = this.rootStore.effectStore.getAbilitiesForCard(this, [
      (ability: AbilityModel) =>
        ability.hasReadyAtStartOfTurnPreventionEffect &&
  ...
  get damageRemovalRestrictionAbilitySource(): CardModel | undefined {
    const continuousRestrictions =
      this.rootStore.continuousEffectStore.getDamageRemovalRestriction(this);
    ...
    const staticRestrictions =
      this.rootStore.effectStore.damageRemovalRestrictionEffect(this);
  ...
  get damageDealtRestrictionAbilitySource(): CardModel | undefined {
    const continuousRestrictions =
      this.rootStore.continuousEffectStore.getDamageDealtRestriction(this);
    ...
    const staticRestrictions =
      this.rootStore.effectStore.damageDealtRestrictionEffect(this);
  ...
  quest(): MoveResponse {
    ...
    if (this.hasSupport) {
      const supportEffect: ResolutionAbility = getSupportEffect(this.strength);

      this.rootStore.stackLayerStore.addAbilityToStack(
        new AbilityModel(supportEffect, this, this.rootStore, this.observable),
        this,
      );
    }
  ...
  canShiftInto(shifted?: CardModel) {
    ...
    if (
      shifted.fullName.toLowerCase() === "turbo - royal hack" &&
    ...
    if (
      this.fullName.toLowerCase() === "thunderbolt - wonder dog" &&
    ...
    if (
      shiftAbility.some(
        (model) =>
          shiftAbilityPredicate(model.ability) &&
          model.ability.additionalNames?.some(
            (name) =>
              extraSafeJSONStringify(name.toLowerCase()) ===
    ...
    return (
      extraSafeJSONStringify(this?.lorcanitoCard.name.toLowerCase()) ===
  ...
  play(params?: {
    ...
    return this.rootStore.stateMachineStore.startPlayCard(
      this.instanceId,
      this.ownerId,
      {
        ...params,
        alternativeCosts: params?.alternativeCosts?.map(
  ...
  onPlay(params?: { bodyguard?: boolean; hasShifted?: boolean }): MoveResponse {
    this.rootStore.stateMachineStore.setPlayCardStep(
      "RESOLVE_EFFECTS",
      this.instanceId,
      this.ownerId,
      params,
  ...
  playingCardRestrictions(
    ...
    if (this.hasPlayConditions) {
      const unmetConditions = this.playConditions.filter(
        (ability) => !isConditionMet(this.rootStore, this, ability.conditions),
      );
    ...
    const isPlayerRestricted =
      this.rootStore.effectStore.hasRestrictionToPlayActionCard(this);
  ...
  canSingASong(song?: CardModel) {
    ...
    const gantus = this.rootStore.cardStore.cardsInPlay.filter(
      (card) => card.publicId === gantuExperiencedEnforcer.id,
    );
  ...
  sing(song?: CardModel): MoveResponse {
    ...
    const isPlayerRestricted =
      this.rootStore.effectStore.hasRestrictionToPlayActionCard(song);
    ...
    const singValue: number = this.singerCost || this.cost;
  ...
  canBeChallenged(challenger: CardModel) {
    if (this.type !== "character" && this.type !== "location") {
      return false;
    }
      ...
      for (const effect of ability.effects || []) {
        if (protectionEffectPredicate(effect) && effect.from === "challenge") {
          if (
            ...
            const matchesFilter = challenger.matchesTargetFilter(
  ...
  isBodyGuarded(opponent?: CardModel) {
    if (this.type !== "character") {
      return false;
    }
    ...
    const defenderBodyGuards =
      this.rootStore.cardStore.getCardsByTargetFilter(filters);

    // It can't bodyguard itself
    if (
      defenderBodyGuards.filter(
        (bodyguard) => bodyguard.instanceId !== this.instanceId,
      ).length === 0
    ) {
      return false;
    }
  ...
  private hasCannotBeChallengedEffect(challenger: CardModel) {
    const cannotBeChallengedEffects = this.rootStore.effectStore
      ...
      .filter((effect) => {
        ...
        const beChallengedRestriction =
          beChallengedRestrictionEffectPredicate(rawEffect);
        ...
        if (rawEffect.challengerFilters) {
          return challenger.matchesTargetFilter(
            rawEffect.challengerFilters,
            this.ownerId,
            effect.source,
          );
        }
  ...
  challenge(defender: CardModel): MoveResponse {
    return this.rootStore.stateMachineStore.startChallenge(
      this.instanceId,
      defender.instanceId,
    );
  }
  ...
  updateCardDamage(
    ...
    if (protector && type === "add") {
      const moveResponse = protector.updateCardDamage(
        amount,
        type,
        damageSource,
        {
  ...
  tapCard(opts: { exerted?: boolean; toggle?: boolean }): MoveResponse {
  ...
  leaveLocation(opts: { shifting?: boolean } = {}) {
    if (this.type !== "character") {
    ...
    const currentLocation =
      this.rootStore.cardStore.getCard(locationInstanceId);
  ...
  enterLocation(
    location: CardModel,
    opts: { shifting?: boolean; forFree?: boolean } = {},
  ): MoveResponse {
    ...
    const moveCost = location.moveCostToEnterLocation(this);
    const costs: Cost[] = [{ type: "ink", amount: moveCost }];
    ...
    this.rootStore.triggeredStore.onEnterLocation(
      this,
      location,
      previousLocation,
    );
  ...
  isAtLocation(location?: CardModel) {
    ...
    return (
      location.instanceId === this.meta.location &&
  ...
  get getDamagedCardsAtLocation(): CardModel[] {
    return (
      this.getCardsAtLocation.filter((card) => (card.meta.damage || 0) > 0) ||
      []
    );
  }
...
function getSupportEffect(strength: number): ResolutionAbility {
  return {
    type: "resolution",
