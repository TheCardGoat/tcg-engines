...
function calculateAmountMultiplier(amount?: number | DynamicAmount): number {
...
export function calculateDynamicAmount(
  dynamicAmount: DynamicAmount | number,
  rootStore: MobXRootStore,
  targets: CardModel[] = [],
  source?: CardModel,
  ...
  if (dynamicAmount.target) {
    ...
    if (!target) {
      rootStore.debug(
        "No target found for effect",
        JSON.stringify(dynamicAmount),
      );
      return 0;
    }
  ...
  if (filters && filters.length) {
    ...
    const filter = rootStore.cardStore.getCardsByTargetFilter(
      filters,
      responder,
      source,
      effectTarget?.excludeSelf || excludeSelf,
    );
