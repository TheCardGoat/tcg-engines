...
export type IsValidTriggerParams = {
  ...
  // TODO: Deprecate this in favor of singers
  singer?: CardModel;
  singers?: CardModel[];
  singing?: boolean;
  hasShifted?: boolean;
...
export function isValidTrigger(
  abilityModel: TriggeredAbilityModel,
  ...
  if (
    abilityModel.conditions &&
    !rootStore.effectStore.metCondition(
      abilityModel.cardSource,
      abilityModel.conditions,
    )
  ) {
    return;
  ...
  switch (trigger.on) {
    case "moves-to-a-location": {
      ...
      if (trigger.target) {
        isValidTarget = target.matchesTargetFilter(
      ...
      if (trigger.source) {
        ...
        isValidSource = cardSource.matchesTargetFilter(
    ...
    case "quest": {
      ...
      const hasSourceSelfFilter = trigger.target.filters.find(
    ...
    case "ready": {
      const validTarget = target.matchesTargetFilter(
        abilityModel.filters,
        source.ownerId,
        source,
      );
      const isSelf =
    ...
    case "damage": {
      ...
      if (inAChallenge) {
        ...
        if (trigger.dealt && trigger.defenderFilters && defender) {
          return defender.matchesTargetFilter(
            trigger.defenderFilters,
            source.ownerId,
            source,
          );
        }
      ...
      if (trigger.dealt && params.damageSource) {
        return params.damageSource.matchesTargetFilter(
          abilityModel.filters,
          source.ownerId,
          source,
        );
      ...
      return target.matchesTargetFilter(
        abilityModel.filters,
        source.ownerId,
        source,
      );
    ...
    case "end_turn":
      if (trigger.target.value === "opponent") {
        return (
          rootStore.opponentPlayer(source.ownerId) === rootStore.turnPlayer
        );
      }
    ...
    case "shift": {
      ...
      const shifterMatches = shifter.matchesTargetFilter(
        shifterFilter,
        source.ownerId,
        source,
      );

      const shiftedMatches = shifted.matchesTargetFilter(
        shiftedFilter,
        source.ownerId,
        source,
      );
    ...
    case "challenge": {
      ...
      const isValidSecondary = secondaryFilters
        ? matchesTargetFilters(
            rootStore,
            secondaryTarget,
            { type: "card", value: "all", filters: secondaryFilters },
            secondaryTarget.ownerId,
            source,
          )
        : true;
