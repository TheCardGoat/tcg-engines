...
export type ThisTurnCondition = {
  ...
  | { filters?: never; comparison?: never }
  | { filters: TargetFilter[]; comparison: NumericComparison }
);
...
export type Condition =
  | { type: "char-is-at-location"; negate?: boolean }
  | { type: "first-time-move-to-location" }
  | { type: "no-character-has-quested" }
  | { type: "no-other-character-has-quested" }
  | { type: "exerted" }
  | { type: "not-alone" }
  | { type: "played-songs"; value?: boolean }
  | { type: "played-actions"; comparison: NumericComparison }
  ...
  | { type: "damage"; comparison: NumericComparison }
  | FilterCondition
  | ThisTurnCondition
  | {
...
const CONDITION_RANKING: Record<Condition["type"], number> = {
  play: 2,
  hand: 3,
  damage: 4,
  resolution: 6,
...
// See engine/store/resolver/README.md
export function isSelfReferencingCondition(condition: Condition) {
        ...
        switch (condition.type) {
          case "this-turn": {
            ...
            switch (value) {
              case "has-challenged": {
                const hasChallenged =
                  rootStore.tableStore.getTable(player).turn?.challenges
                    ?.length;
              ...
              case "was-damaged": {
                const tableDamages = Object.values(
                  rootStore.tableStore.getTables(),
                )
                ...
                if (filters && comparison) {
                  ...
                  const meetComparison = computeNumericOperator(
                    comparison,
                    damagedThisTurnThatMatchesFilters.length,
                    rootStore,
                    sourceCard,
                  );
          ...
          case "first-time-move-to-location": {
            const locationsEnteredDuringTurn = rootStore.tableStore.getTable(
              sourceCard.ownerId,
            ).turn?.locations;
            ...
            const enteredSourceLocation =
              locationsEnteredDuringTurn[sourceCard.instanceId];
          ...
          case "player-lore-comparison": {
            ...
            const opponentLore =
              rootStore.tableStore.getTable(opponentId).lore || 0;

            return computeNumericOperator(
              { ...comparison, value: opponentLore },
              playerLore,
              rootStore,
              sourceCard,
            );
          ...
          case "player": {
            ...
            switch (attribute) {
              case "lore": {
                const playerLore =
                  rootStore.tableStore.getTable(playerId).lore || 0;
          ...
          case "hand": {
            ...
            const playerHand = rootStore.tableStore.getPlayerZone(
              playerId,
              "hand",
            )?.cards.length;
            const opponentHand = rootStore.tableStore.getPlayerZone(
              opponentPlayer,
              "hand",
            )?.cards.length;
          ...
          case "inkwell": {
            const inkwell = rootStore.tableStore.getPlayerZone(
              sourceCard.ownerId,
              "inkwell",
            )?.cards;
          ...
          case "played-actions": {
            const actionsPlayed = rootStore.tableStore
              .getTable(sourceCard.ownerId)
              .turn.cardsMoved.filter(
                (move) => move.to === "play" && move.card.type === "action",
              ).length;
          ...
          case "chars-at-location": {
            ...
            if (!filters) {
              return computeNumericOperator(
                comparison,
                charsAtLocation.length,
                rootStore,
                sourceCard,
              );
            }

            const filteredChars = charsAtLocation.filter((char) =>
              char.matchesTargetFilter(filters, sourceCard.ownerId, sourceCard),
            );
          ...
          case "clash": {
            ...
            const opponentFilterLength =
              rootStore.cardStore.getCardsByTargetFilter(
            ...
            const selfFilterLength = rootStore.cardStore.getCardsByTargetFilter(
            ...
            const clashComparison: NumericComparison = {
              operator,
          ...
          case "filter": {
            ...
            const length = rootStore.cardStore.getCardsByTargetFilter(
              filters,
              sourceCard.ownerId,
              sourceCard,
              excludeSelf,
            ...
            const result = computeNumericOperator(
              comparison,
              length,
              rootStore,
              sourceCard,
          ...
          case "have-strongest-character": {
            const cardModels = rootStore.cardStore.cardsInPlay.filter(
            ...
            const reduceToStrongest = (
              strongest: CardModel | undefined,
              card: CardModel,
            ): CardModel => {
              if (!strongest) {
                return card;
