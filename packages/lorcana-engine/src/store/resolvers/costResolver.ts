  ...
  costs.forEach((cost: Cost) => {
    switch (cost.type) {
      ...
      case "card": {
        ...
        if (
          cardsToEvaluate.some(
            (card) =>
              !card.matchesTargetFilter(
                filters,
                rootStore.activePlayer,
                source,
              ),
          )
        ) {
        ...
        switch (action) {
          case "discard": {
            cardsToEvaluate.forEach((card) =>
              card.moveTo("discard", { skipLog: true, discard: true }),
            );
            break;
          }
          case "exert": {
...
export function canPayCosts(
  ...
  return costs.every((cost) => {
    switch (cost.type) {
      ...
      case "ink": {
        const amount = source.applyCostModifier(
          source.applyShiftCostModifiers(cost.amount) || cost.amount,
        );
      ...
      case "card": {
        ...
        return cardsToEvaluate.every((card) =>
          card.matchesTargetFilter(
            cost.filters,
            rootStore.activePlayer,
            source,
...
export function calculateShiftCostModifier(
  rootStore: MobXRootStore,
  cardModel: CardModel,
) {
  const continuousEffects: EffectModel[] =
    rootStore.continuousEffectStore.continuousEffects
  ...
  const filters = [
    (model: AbilityModel) =>
      ...
      ) && model.hasShiftReplacementEffect,
  ];

  // Doing like this as we calculate cost when the card is in hand
  const cardEffects: EffectModel[] = rootStore.effectStore
    .getAbilitiesForCard(cardModel, filters)
    .map((ability) => {
      return ability.effects.find((model) =>
        costReplacementShiftEffectPredicate(model.effect),
      );
    })
  ...
  const result = [...allEffects, ...continuousEffects, ...cardEffects]
    .filter((effect) =>
      matchesTargetFilters(rootStore, cardModel, effect.target),
    )
    .reduce((acc: number, curr: EffectModel) => {
  ...
  const filters = [
    (model: AbilityModel) =>
      !(
        ...
        model.isStaticTriggeredAbility ||
        model.isActivatedAbility
      ) && model.hasCostReplacementEffect,
  ];
