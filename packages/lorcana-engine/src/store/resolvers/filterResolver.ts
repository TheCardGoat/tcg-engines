...
import type { Characteristics } from "@lorcanito/lorcana-engine/cards/cardTypes";
...
const FILTER_RANKING: Record<TargetFilter["filter"], number> = {
  instanceId: 0,
  "name-a-card": 1,
  type: 2,
  characteristics: 3,
...
// See engine/store/resolver/README.md
export function isSelfReferencingFilter(filter: TargetFilter) {
  ...
  function predicate(card: CardModel) {
    ...
    try {
      ...
      return activeFilters
        ...
        .every((filter: TargetFilter): boolean => {
          ...
          switch (activeFilter) {
            ...
            case "source": {
              ...
              if (!source) {
                console.warn(
                  "Source not found, this is likely a mistake",
                  filter,
                );
                return false;
              }
            ...
            case "challenge": {
              ...
              const attackerId =
                store.stateMachineStore.challengeState?.attacker;
              const defenderId =
                store.stateMachineStore.challengeState?.defender;
            ...
            case "trigger": {
              ...
              console.log(
                "Trigger filter should have been replaced by instanceId filter",
              );
              return false;
            }
            case "sing": {
              const currentSingers =
                store.stateMachineStore?.playCardState?.params?.singers;
              if (currentSingers) {
                return currentSingers.includes(card.instanceId);
              }
              console.warn("NOT IMPLEMENTED");
              console.log(
                "Sing filter should have been replaced by instanceId filter",
              );
              return false;
            }
            case "top-deck": {
            ...
            case "name-a-card": {
              ...
              console.error(
                "This filter should have been replaced before getting here",
              );
              return false;
            }
            case "ability": {
            ...
            case "turn": {
              ...
              switch (value) {
                case "played": {
                  const cardsMoved = table.turn.cardsMoved.filter(
                    (move) =>
                      move.to === "play" &&
                ...
                case "challenge": {
                  const challengesThisTurnMatchingFilter =
                    table.turn.challenges.filter((challenge) => {
                      console.log({
                        card: card.fullName,
                      ...
                      return (
                        challenge.attacker.matchesTargetFilter(
                          targetFilter,
                          source?.ownerId,
                          source,
                        ) ||
                        challenge.defender.matchesTargetFilter(
                          targetFilter,
                          source?.ownerId,
                          source,
                        )
                  ...
                  return computeNumericOperator(
                    comparison,
                    challengesThisTurnMatchingFilter,
                    store,
                    source,
                  );
                }
...
export function filterByAttribute(
  value: "cost" | "name" | "title" | "strength" | "instanceId",
  comparison: NumericComparison | StringComparison,
  store: MobXRootStore,
  card?: CardModel,
  ...
  const attribute =
    ignoreBonuses && card ? card.lorcanitoCard[value] : card[value];
...
export function computeNumericOperator(
  ...
  numericValueToCompare: number | DynamicAmount,
  ...
  const valueToCompare: number = isDynamicAmount(numericValueToCompare)
