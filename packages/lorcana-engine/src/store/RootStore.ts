...
import {
  ANSI_ESCAPE_CODES,
  ansiEscape,
} from "@lorcanito/lorcana-engine/lib/ansi-escape-codes";
...
import type {
  ResolverLayerParams,
  ScryParams,
} from "@lorcanito/lorcana-engine/types/Moves";
...
function selectNextTurnPlayer(players: string[], playerId: string): string {
...
export class MobXRootStore {
  private static instance: MobXRootStore | null = null;
  ...
  isPassingTurn?: boolean = undefined;
  pendingDrawStep?: string = undefined;
  ...
  constructor(
    ...
    this.stateMachineStore = new StateMachineStore(
      initialState.stateMachines,
      this,
      observable,
    );
  ...
  getBoardState(): BoardState {
    ...
    const boards: Record<string, InnerBoardState> = {};

    Object.keys(json.tables || {}).forEach((playerId) => {
      const playerTable: Table | undefined = json.tables[playerId];

      if (!playerTable) {
        return;
      }

      const innerBoard: InnerBoardState = {
        cardsPlayed:
          playerTable.turn?.cardsMoved
        ...
        unusedInk:
          playerTable.zones.inkwell?.filter(
            (card) => !json.metas[card]?.exerted,
          ).length || 0,
  ...
  get hash(): string {
    const json: Match = this.toJSON();
    const { undoState, _hash, ...state } = json as Match & {
      undoState?: string;
      _hash?: string;
    };
    return hash(state);
  ...
  gameStateCheck() {
    ...
    for (const layer of this.stackLayerStore.layers) {
      ...
      if (!(layer.hasValidTarget() || layer.ability.ability.dependentEffects)) {
        ...
        this.trace(
          `Skipping effect: ${JSON.stringify(layer.ability.ability)}, invalid target during game state check`,
        );

        const skipLayer = layer.ability.onCancelLayer();
        if (skipLayer) {
          this.stackLayerStore.addAbilityToStack(
            new AbilityModel(skipLayer, layer.source, this, this.observable),
            layer.source,
          );
        }
  ...
  setMoveResponse(response?: MoveResponse) {
    runInAction(() => {
      this._moveResponse = response;
    });
  }

  moveResponse(success = true, overwriteResult?: boolean): MoveResponse {
  ...
  get hasPriority() {
    ...
    return (
      this.priorityPlayer === this.dependencies.playerId ||
  ...
  declareWinnerForMatch(winner: string, reason = "") {
    if (!this.winner) {
  ...
  get shouldRevealTopDeck() {
    return (
      this.cardStore.locationCardsInPlay.filter((card) => {
        return (
  ...
  sendPlayerRequest(request: PlayerRequest) {
  ...
  cancelPlayerRequest(playerId: string, cancelled: boolean) {
  ...
  answerPlayerRequest(playerId: string, accepted: boolean) {
    const firstPendingRequest = this.pendingRequests?.[0];
  ...
  convertPlayerTargetToPlayerId(target: PlayerEffectTarget) {
  ...
  drawCard(playerId: string, amount = 1, skipLog = false): MoveResponse {
  ...
  readySetDraw(playerId: string): MoveResponse {
    try {
      ...
      if (this.stackLayerStore.layers.length > 0) {
        console.log(
          "Pending effects after set step, setting pending draw step",
        );
      ...
      if (!drawResponse.success) return drawResponse;
  ...
  readyStep(playerId: string): MoveResponse {
    // Hide revealed cards
    const cardsInHand = this.getPlayerZone(playerId, "hand")?.cards.filter(
      (card) => card.isRevealed,
    );
    const cardsInDeck = this.getPlayerZone(playerId, "deck")?.cards.filter(
      (card) => card.isRevealed,
    );
    const cardsInInkwell = this.getPlayerZone(
      playerId,
      "inkwell",
    )?.cards.filter((card) => card.isRevealed);
    ...
    zones.forEach((zone: Zones) => {
      this.getPlayerZone(playerId, zone)?.cards.forEach((card) => {
        ...
        if (
          card.hasAtStartOfTurnReadyRestriction &&
          zone === "play" &&
          card.type === "character"
        ) {
          return;
  ...
  resetPriority() {
    ...
    const turnPlayerHasActionEffectOnStack = this.stackLayerStore.layers.some(
      (layer) =>
    ...
    const nonTurnPlayerHasActionEffectOnStack =
      this.stackLayerStore.layers.some(
        (layer) =>
          layer.source.type === "action" &&
    ...
    const turnPlayerHasEffectsOnStack = this.stackLayerStore.layers.some(
      (layer) => layer.responder === this.turnPlayer,
    );
  ...
  private changeLogSender(activePlayer: string) {
  ...
  debugCondition(conditions: Condition[] = [], card: CardModel) {
    conditions.forEach((condition) => {
      ...
      if (condition.type === "filter") {
        const result = this.cardStore.getCardsByTargetFilter(
          condition.filters,
          card.ownerId,
          card,
        );
  ...
  // TEST UTILITY
  changeActivePlayer(activePlayer: string) {
  ...
  passTurn(playerId: string = this.turnPlayer, force = false): MoveResponse {
    if (this.stackLayerStore.topLayer) {
      return this.sendNotification({
        type: "icon",
        title: "There's still effects to be resolved",
        message:
          "Please make sure all effects are resolved/skipped before passing the turn",
        icon: "warning",
        autoClear: true,
      });
    }
    ...
    if (hasPendingRecklessCard && !force) {
      return this.sendNotification({
        ...
        message:
          "You can instead use manual mode to skip this check, right click on the table and select pass turn (force)",
        icon: "warning",
        autoClear: true,
      });
    }
  ...
  resolveTopOfStack(params: ResolverLayerParams, activePlayer?: string) {
    const { layerId, scry, targets, nameACard, mode, skip, targetPlayer } =
      params;
  ...
  get playersLores() {
    const lores: Record<string, number> = {};
  ...
  playCardFromHand(
    instanceId: string,
    params?: { bodyguard?: boolean; alternativeCosts?: string[] },
  ): MoveResponse {
  ...
  singTogether(songId: string, singersId: string[]): MoveResponse {
    ...
    const singTogetherAbility = song.getNativeAbilities
      ...
      .find(keywordToAbilityPredicate("sing-together")) as
      | SingTogetherAbility
      | undefined;
    ...
    const singersTotalCost = singers.reduce(
      (acc, singer) => acc + singer.singerCost,
      0,
    );
  ...
  scryMove(params: ScryParams & { playerId: string }): MoveResponse {
  ...
  generateLayerOnDemand(
    ...
    if (!hasAbility) {
      ...
      card.abilities.push(
        new AbilityModel(ability, card, this, this.observable),
      );
    }
  ...
  setHighlightedCardId(id: string | null) {
    this.highlightedCardId = id;
  }

  resolveResponder(responder: string, source: CardModel) {
    if (responder === "self") {
