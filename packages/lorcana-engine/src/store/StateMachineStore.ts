...
export type ChallengeStep =
  ...
  | "EXERT_CHALLENGER"
  | "CHALLENGE_OCCURS"
  | "APPLY_WHILE_CHALLENGING_EFFECTS"
  | "ADD_TRIGGERS_TO_BAG"
  | "CHALLENGE_DAMAGE"
  | "CHALLENGE_COMPLETE";

export type PlayCardStep =
  | "ANNOUNCE_CARD"
  | "ANNOUNCE_PAYMENT_METHOD"
...
export class StateMachineStore {
  ...
  private _startMatchMachine!:
    | ActorRefFrom<typeof gameStartMachine>
    | undefined;
  action?: Actions;
  phase?: Phases;
  ...
  constructor(
    initialState: Match["stateMachines"],
  ...
  sync(state: Match["stateMachines"]) {
    try {
  ...
  toJSON(): Match["stateMachines"] {
    try {
      const playCardState: PlayCardStateSerialised | undefined = toJS(
        this.playCardState,
      );
  ...
  chooseFirstPlayer(playerId: string): MoveResponse {
    ...
    runInAction(() => {
      this._startMatchMachine?.send({
        ...
        order: this.rootStore.tableStore.players.sort((a, b) =>
          a.localeCompare(playerId) ? 1 : -1,
        ),
      });
    });
  ...
  isGoingFirst(ownerId: string) {
    return this?.matchStart?.startingPlayer === ownerId;
  }

  trackAlteredCards(playerId: string, count: number) {
  ...
  setPhase(phase: SerializedStateMachines["phase"]) {
    this.phase = phase;
  }

  setStep(step: SerializedStateMachines["step"]) {
    this.step = step;
  }

  setAction(action: SerializedStateMachines["action"]) {
    this.action = action;
  }

  startChallenge(attacker: string, defender: string) {
  ...
  setChallengeStep(step: ChallengeStep, attacker?: string, defender?: string) {
  ...
  progressChallenge(): MoveResponse {
    const context: ChallengeState = this?.challengeState || {
      attacker: undefined,
      defender: undefined,
    };
    ...
    switch (this.step) {
      ...
      case "ADD_TRIGGERS_TO_BAG": {
        ...
        this.rootStore.continuousEffectStore.onChallenge(
          attackerModel,
          defenderModel,
        );
        ...
        if (this.rootStore.stackLayerStore.layers.length > 0) {
          console.log(
            "[progressChallenge] Stack layers found, resolve it first before proceeding with challenge",
          );
          return this.rootStore.moveResponse(true);
        }
      ...
      default: {
        console.log(
          "[progressChallenge] No step found",
  ...
  endPlayCard(overwriteResult?: boolean) {
    this.setStep(undefined);
    this.setAction(undefined);
  ...
  progressPlayCard(): MoveResponse {
    const context: PlayCardState = this?.playCardState || {
      cardId: undefined,
      playerId: undefined,
    ...
    switch (this.step) {
      ...
      case "ANNOUNCE_CARD": {
        ...
        if (restrictions) {
          this.rootStore.trace(
            `Skipping play card ${cardBeingPlayed.instanceId}, it has restrictions.`,
          );
      ...
      case "ANNOUNCE_PAYMENT_METHOD": {
        if (params?.forFree) {
          ...
          if (cardBeingPlayed.alternativeCosts && params?.alternativeCosts) {
            cardBeingPlayed.payCosts(
              cardBeingPlayed.alternativeCosts,
              params.alternativeCosts
                ?.map((id) => this.rootStore.cardStore.getCard(id))
                .filter(notEmptyPredicate) || [],
            );
          } else {
            const ownerTable = this.rootStore.tableStore.getTable(
              cardBeingPlayed.ownerId,
            );
      ...
      case "RESOLVE_EFFECTS": {
        ...
        cardBeingPlayed
          ...
          .forEach((model) =>
            this.rootStore.triggeredStore.startDelayedAbility(model),
          );
      ...
      default: {
        console.log(
          "[progressPlayCard] No step found",
          JSON.stringify(this.playCardState),
        );
        this.endPlayCard();
        return this.rootStore.moveResponse(false);
      }
