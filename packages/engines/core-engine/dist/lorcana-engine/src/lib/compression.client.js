...
async function compressToDeflateBuffer(input) {
...
async function decompressFromDeflateBuffer(buffer) {
...
export async function compressAndCompareSizes(input) {
    const now = performance.now();
    const { base64String, compressedBuffer } = await compress(input);
    // 3. Calculate sizes
    const inputSize = new TextEncoder().encode(typeof input !== "string" ? JSON.stringify(input) : input).length; // Original input size (in bytes)
...
export async function decompress(base64String) {
    ...
    const decompressedString = await decompressFromDeflateBuffer(compressedBuffer);
    // console.log("Decompressed", decompressedString);
    let parsedString;
    try {
        parsedString = JSON.parse(decompressedString);
...
export async function decompressAndCompareSizes(base64String) {
    const now = performance.now();
    const { decompressedString, compressedBuffer } = await decompress(base64String);
    // 3. Calculate sizes
    const decompressedSize = new TextEncoder().encode(decompressedString).length; // Decompressed size (in bytes)
    // 4. Log size comparisons
    console.log(`TEST Decompressed (Binary) Size: ${compressedBuffer.byteLength} bytes`);
...
//# sourceMappingURL=compression.client.js.map