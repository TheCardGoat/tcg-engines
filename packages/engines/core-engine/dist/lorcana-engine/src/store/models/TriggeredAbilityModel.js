import { AbilityModel, cardEffectTargetPredicate, challengeFilterPredicate, isValidAbilityTriggerTarget, matchesTargetFilters, notEmptyPredicate, staticTriggeredAbilityPredicate, } from "@lorcanito/lorcana-engine";
import { singFilterPredicate, triggerFilterPredicate, } from "@lorcanito/lorcana-engine/effects/effectTargets";
...
import { isValidTrigger, } from "@lorcanito/lorcana-engine/store/resolvers/triggerResolver";
...
export class TriggeredAbilityModel {
    ...
    rootStore;
    observable;
    constructor(abilityModel, source, cardThatTriggered, rootStore, observable) {
        if (observable) {
    ...
    // TODO: MAKE IT CONSIStent
    replaceTriggerTarget(source, trigger) {
        ...
        for (const effect of layer.effects) {
            ...
            if (cardEffectTargetPredicate(effectTarget) && effectTarget.filters) {
                const sourceSelf = effectTarget.filters.findIndex((item) => item.filter === "source" && item.value === "self");
                if (sourceSelf !== -1) {
                ...
                // TODO: THis was a hot fix target.filters somehow was not available
                const sourceTrigger = effectTarget.filters.findIndex((item) => item.filter === "source" && item.value === "trigger");
                if (sourceTrigger !== -1) {
            ...
            if (cardEffectTargetPredicate(target) && target.filters) {
                target.filters = target.filters.map((filter) => {
                    if (challengeFilterPredicate(filter)) {
                        console.log("found a challenge filter: ", filter.value === "attacker"
                            ? attacker?.instanceId
                            : defender?.instanceId);
                        return {
                            filter: "instanceId",
                            value: filter.value === "attacker"
                                ? attacker?.instanceId
                                : defender?.instanceId,
                        };
                    }
    ...
    replaceMoveToLocationTriggerTarget(source, trigger) {
        const layer = this.layer;
        for (const effect of layer.effects) {
    ...
    replaceChallengeTarget(params = {}) {
        ...
        for (const effect of layer.effects) {
            const target = effect.target;
            if (cardEffectTargetPredicate(target) && target.filters) {
                const challengeFilter = target.filters.findIndex(challengeFilterPredicate);
    ...
    replaceTriggerSourceAndTarget(params = {}) {
        ...
        if (!(triggerSource && triggerTarget)) {
            ...
            logger.error(`[replaceTriggerSourceAndTarget] Invalid trigger source or target: ${layer.name} - ${JSON.stringify(layer)}`);
            return;
        }
        for (const effect of layer.effects) {
            const target = effect.target;
    ...
    isValidTarget(target, debug = false) {
    ...
    isValidTriggerTarget(targetParam, params = {}, debug = false) {
        let target = targetParam;
        if (debug)
            console.log("[isValidTriggerTarget] Starting evaluation", JSON.stringify({
        ...
        if (trigger && "source" in trigger && trigger.source) {
            ...
            if (triggerSource) {
                const matchesTarget = matchesTargetFilters(this.rootStore, triggerSource, trigger.source, this.cardSource.ownerId, this.cardSource);
                if (debug)
                    console.log(`[isValidTriggerTarget] Trigger source check: ${JSON.stringify(matchesTarget)}`, JSON.stringify({
        ...
        if (trigger.on === "moves-to-a-location" && trigger.movingFrom) {
            if (!params.previousLocation) {
                if (debug)
                    console.log("[isValidTriggerTarget] No previous location for move trigger");
                return false;
            }
            const matchesTarget = params.previousLocation.matchesTargetFilter(trigger.movingFrom, this.cardSource.ownerId, this.cardSource);
            if (debug)
                console.log(`[isValidTriggerTarget] Moving from location check: ${JSON.stringify(matchesTarget)}`, JSON.stringify({
        ...
        if (this.trigger.on === "damage" && this.trigger.dealt) {
            const isValid = isValidAbilityTriggerTarget(this.rootStore, this, params.damageSource, this.cardSource);
            if (debug)
        ...
        if (on === "challenge") {
            ...
            if (debug)
                console.log("[isValidTriggerTarget] Challenge target selected", JSON.stringify({
                    as: this.trigger.as,
                    target: target?.fullName,
                }));
        }
        ...
        if (this.trigger.on === "leave") {
            ...
            this.trigger.destination &&
                ...
                if (debug) {
                    console.log("[isValidTriggerTarget] Leave destination mismatch", JSON.stringify({
            ...
            if (this.trigger.from && this.trigger.from !== params.from) {
                if (debug) {
                    console.log("[isValidTriggerTarget] Leave origin mismatch", JSON.stringify({
                        expectedFrom: this.trigger.from,
                        actualFrom: params.from,
                    }));
                }
        ...
        if (this.trigger.on === "play" && this.trigger.hasShifted) {
            if (!params.hasShifted) {
                if (debug) {
                    console.log("[isValidTriggerTarget] Play with shift required but not shifted");
                }
                return false;
            }
        }
        if (this.trigger.on === "play" && this.trigger.hasSang) {
            if (!params.singing) {
                if (debug) {
                    console.log("[isValidTriggerTarget] Sing a song required but not singing");
                }
                return false;
            }
        }
        const result = isValidAbilityTriggerTarget(this.rootStore, this, target, this.cardSource, debug);
        if (debug) {
    ...
    activate(target, params = {}) {
        ...
        this.layer = {
            ...
            resolutionConditions: this.layer.resolutionConditions || this.secondaryConditions,
            // TODO: This is a hot fix, we should be able to remove this
            // target: this.layer.effects.find((effect) => effect.target)?.target,
            effects: this.layer.effects.map((effect) => {
                ...
                if (isDynamicAmount(effectAmount)) {
                    ...
                    if ("resolveAmountBeforeCreatingLayer" in effect &&
                        effect.resolveAmountBeforeCreatingLayer) {
                        const resolvedAmount = calculateDynamicAmount(effectAmount, this.rootStore, [target], this.cardSource);
                        this.rootStore.trace(`Resolving amount before creating triggered layer,
        ...
        const abilityModel = new AbilityModel(this.layer, this.cardSource, this.rootStore, this.observable);
        if (abilityModel.isInvalidResolution) {
            this.rootStore.debug("Invalid resolution for triggered effect, skipping it.");
            return;
        }
        if (!(abilityModel.areConditionsMet && abilityModel.areSecondaryConditionsMet)) {
            this.rootStore.debug("Trigger resolution conditions don't match, skipping it.");
            return;
        }
        ...
        if (abilityModel.allEffectsHaveZeroAmount && !this.layer.unless) {
            ...
            this.rootStore.debug("Trigger resolution has no effects with amount, skipping it.");
            return;
        }
        this.rootStore.stackLayerStore.addAbilityToStack(abilityModel, this.cardSource, {
...
//# sourceMappingURL=TriggeredAbilityModel.js.map