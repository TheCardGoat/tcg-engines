import { bodyguardAbility, challengeReadyCharacters, challengerAbility, evasiveAbility, protectorAbility, recklessAbility, resistAbility, rushAbility, supportAbility, vanishAbility, voicelessAbility, wardAbility, } from "@lorcanito/lorcana-engine/abilities/abilities";
import { bodyguardAbilityPredicate, challengerAbilityPredicate, evasiveAbilityPredicate, metaAbilityPredicate, protectorAbilityPredicate, recklessAbilityPredicate, resistAbilityPredicate, rushAbilityPredicate, shiftAbilityPredicate, singerAbilityPredicate, singTogetherAbilityPredicate, supportAbilityPredicate, vanishAbilityPredicate, voicelessAbilityPredicate, wardAbilityPredicate, } from "@lorcanito/lorcana-engine/abilities/abilityTypeGuards";
import { exhaustiveCheck } from "@lorcanito/lorcana-engine/lib/exhaustiveCheck";
export const mapContinuousEffectToAbility = (element) => {
...
export const keywordToAbilityPredicate = (keyword) => {
    let predicate = (ability) => false;
    switch (keyword) {
        case "meta": {
...
export function normalizeToSafeASCII(str) {
    return str.normalize("NFKD").replace(/[^\x00-\xFF]/g, "");
}
export function extraSafeJSONStringify(str) {
