...
export function createLobbyEngine(options) {
    let logger = defaultLogger;
    let adapter = options?.sideEffectsAdapter;
    // Create event bus
    const eventBus = new LightweightEventBus();
    if (adapter) {
    ...
    const lobbyStore = new Store(initialState);
    let disposeHandler = () => { };
    function setupSideEffectsAdapter(newAdapter) {
    ...
    function tick() {
        batch(() => {
            ...
            if (!hasAllPlayersAccepted()) {
                lobbyStore.setState((combined) => {
                    const currentDeadlineTick = combined.context.acceptDeadlineEndTick || 0;
                    return {
                        ...combined,
                        context: {
                            ...combined.context,
            ...
            if (isLobbyTimedOut()) {
                ...
                else if ((lobbyStore.state.context.maxDurationTicks || 0) >= maxDurationSecs) {
                    reason = "duration";
                }
                else {
    ...
    function acceptLobby(playerId) {
        if (isLobbyLocked()) {
            logger.warn(`[LobbyEngine] Cannot accept lobby for player ${playerId}: lobby is locked`);
            return false;
        }
        logger.debug(`[LobbyEngine] Accepting lobby for player ${playerId}`);
        return setPlayerReady(playerId, true);
    }
    function rejectLobby(playerId) {
        if (isLobbyLocked()) {
            logger.warn(`[LobbyEngine] Cannot reject lobby for player ${playerId}: lobby is locked`);
            return false;
        }
        logger.debug(`[LobbyEngine] Rejecting lobby for player ${playerId}`);
        return setPlayerReady(playerId, false);
    }
    function setPlayerReady(nakamaOrClerkId, isReady) {
        if (isLobbyLocked()) {
            logger.warn(`[LobbyEngine] Cannot set player ${nakamaOrClerkId} ready status: lobby is locked`);
            return false;
        }
        ...
        if (!isPlayerInLobby(playerId)) {
            logger.warn(`[LobbyEngine] Player ${playerId} is not in the lobby, current players: ${getPlayersIds().join(", ")}`);
            return false;
        }
        if (!isPlayerInLobby(playerId)) {
        ...
        const previousReadyStatus = lobbyStore.state.context.playersAccepted[playerId];
        // If the ready status hasn't changed, return early
        if (previousReadyStatus === isReady) {
            logger.debug(`[LobbyEngine] Player ${playerId} ready status unchanged: ${isReady}`);
            return true;
        }
        lobbyStore.setState((combined) => {
            ...
            const allPlayersReady = Object.keys(playersAccepted).every((id) => playersAccepted[id]);
            const currentStatus = combined.context.status;
            const shouldCreateMatch = allPlayersReady &&
    ...
    function joinLobby(nakamaOrClerkId, playerData) {
        if (isLobbyLocked()) {
            logger.warn(`[LobbyEngine] Cannot join lobby for player ${nakamaOrClerkId}: lobby is locked`);
            return false;
        }
        const playerId = getPlayerId(nakamaOrClerkId);
        if (!playerId) {
        ...
        if (isLobbyFull()) {
            logger.warn(`[LobbyEngine] Lobby is full: ${getPlayersIds().join(", ")}.`);
            return false;
        }
        ...
        lobbyStore.setState((combined) => {
            ...
            const playerCount = Object.values(newPresences).filter((value) => Object.keys(value || {}).length > 0).length;
    ...
    function joinLobbyAttempt(nakamaOrClerkId, _playerData) {
        if (isLobbyLocked()) {
            logger.warn(`[LobbyEngine] Cannot attempt join for player ${nakamaOrClerkId}: lobby is locked`);
            return false;
        }
        const playerId = getPlayerId(nakamaOrClerkId);
        if (isLobbyFull()) {
        ...
        if (isPlayerInLobby(playerId)) {
            logger.warn(`[LobbyEngine] Cannot attempt join: player ${playerId} is already in the lobby`);
            return false;
        }
    ...
    function leaveLobby(nakamaOrClerkId) {
        if (isLobbyLocked()) {
            logger.warn(`[LobbyEngine] Cannot leave lobby for player ${nakamaOrClerkId}: lobby is locked`);
            return false;
        }
        const playerId = getPlayerId(nakamaOrClerkId);
        if (!playerId) {
        ...
        if (["creating_event", "event_created"].includes(lobbyStore.state.context.status)) {
            logger.warn("[LobbyEngine] Cannot leave lobby while event is in progress");
            return false;
        }
        ...
        const oldStatus = lobbyStore.state.context.status;
        lobbyStore.setState((combined) => {
            const { [playerId]: _, ...remainingPresences } = combined.context.presences;
            const { [playerId]: __, ...remainingPlayersAccepted } = combined.context.playersAccepted;
    ...
    function createMatch() {
        if (!(isLobbyReady() &&
            ...
            logger.warn(`[LobbyEngine] Cannot create lobby: not ready or already creating event - current status: ${lobbyStore.state.context.status}`);
            return false;
        }
        logger.info("[LobbyEngine] Creating lobby with current state", lobbyStore.state);
        if (!adapter) {
    ...
    function matchCreated(id) {
        if (!(["creating_event"].includes(lobbyStore.state.context.status) &&
            hasAllPlayersAccepted())) {
            logger.warn("[LobbyEngine] Cannot finalize lobby: not in creating_event status or not all players accepted");
            return false;
        }
    ...
    function isLobbyTimedOut() {
        return (lobbyStore.state.context.maxDurationTicks >= maxDurationSecs ||
    ...
    function getPlayer(clerkOrNakamaId) {
        return Object.values(lobbyStore.state.context.players).find((player) => player.id === clerkOrNakamaId ||
            // @ts-expect-error - I'm tired now, I fix this later
            player?.presence?.userId === clerkOrNakamaId);
    }
    function getPlayerId(clerkOrNakamaId) {
...
//# sourceMappingURL=lobby-engine.js.map