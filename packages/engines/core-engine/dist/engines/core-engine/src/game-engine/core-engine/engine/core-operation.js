import { getCardZone, isZoneOperationError, move, moveCardByInstanceId, shuffleZone, } from "~/game-engine/core-engine/engine/zone-operation";
import { getCurrentPriorityPlayer, setPriorityPlayer, setTurnPlayer, } from "~/game-engine/core-engine/state/context";
...
export class CoreOperation {
    ...
    playerHasMulliganed(playerId) {
        this.state.ctx.pendingMulligan?.delete(playerId);
    }
    setPendingChampionSelection(playerIds) {
        if (!playerIds) {
    ...
    playerHasSelectedChampion(playerId) {
    ...
    getCardInstance(instanceId) {
        const cardInstance = this.engine.cardInstanceStore.getCardByInstanceId(instanceId);
        if (!cardInstance) {
    ...
    moveCard({ playerId, from, to, origin = "start", destination = "end", instanceId, }) {
        if (instanceId) {
...
//# sourceMappingURL=core-operation.js.map