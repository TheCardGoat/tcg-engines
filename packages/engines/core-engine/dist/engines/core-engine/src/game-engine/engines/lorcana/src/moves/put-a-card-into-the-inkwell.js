...
export const putACardIntoTheInkwellMove = ({ G, ctx, coreOps, gameOps, playerID }, instanceId) => {
    try {
        // Ensure we're in the main phase (this is a turn action)
        if (ctx.currentPhase !== "mainPhase") {
            logger.error(`Cannot put card into inkwell during ${ctx.currentPhase} phase`);
            return createInvalidMove("WRONG_PHASE", "moves.putCardIntoInkwell.errors.wrongPhase", { currentPhase: ctx.currentPhase, expectedPhase: "mainPhase" });
        }
        ...
        if (!cardInstance) {
            ...
            return createInvalidMove("CARD_NOT_FOUND", "moves.putCardIntoInkwell.errors.cardNotFound", { instanceId });
        }
        ...
        if (!lorcanaCard.inkwell) {
            ...
            return createInvalidMove("NO_INKWELL_SYMBOL", "moves.putCardIntoInkwell.errors.noInkwellSymbol", { instanceId, cardName: lorcanaCard.card.name });
        }
        // Check if player has already put a card in the inkwell this turn
        if (G.turnActions?.putCardIntoInkwell) {
            logger.error(`Player ${playerID} already put a card in the inkwell this turn`);
            return createInvalidMove("ACTION_ALREADY_USED", "moves.putCardIntoInkwell.errors.actionAlreadyUsed", { playerId: playerID });
        }
        ...
        if (moveResult) {
            ...
            return createInvalidMove(moveResult.reason, "moves.putCardIntoInkwell.errors.cardNotInHand", moveResult.context);
        }
        try {
            ...
            return createInvalidMove("CARD_MOVE_ERROR", "moves.putCardIntoInkwell.errors.cardMoveError", { error: String(error), instanceId, playerId: playerID });
        }
    }
    catch (error) {
        logger.error(`Unexpected error in putACardIntoTheInkwellMove: ${error}`);
        return createInvalidMove("UNEXPECTED_ERROR", "moves.putCardIntoInkwell.errors.unexpectedError", { error: String(error), instanceId, playerId: playerID });
    }
...
//# sourceMappingURL=put-a-card-into-the-inkwell.js.map