...
export const questMove = ({ G, ctx, coreOps, gameOps, playerID }, instanceId) => {
    try {
        ...
        if (!cardInstance) {
            ...
            return createInvalidMove("CARD_NOT_FOUND", "moves.quest.errors.cardNotFound", { instanceId });
        }
        ...
        if (!lorcanaCard.card.type?.includes("Character")) {
            ...
            return createInvalidMove("NOT_A_CHARACTER", "moves.quest.errors.notACharacter", { instanceId, cardType: lorcanaCard.card.type });
        }
        ...
        if (!playCards.find((card) => card.instanceId === instanceId)) {
            logger.error(`Character ${instanceId} is not in player ${playerID}'s play zone`);
            return createInvalidMove("CHARACTER_NOT_IN_PLAY", "moves.quest.errors.characterNotInPlay", { instanceId, playerId: playerID });
        }
        ...
        const cardKeywords = lorcanaCard.card.keywords || [];
        if (cardKeywords.includes("Reckless")) {
            logger.error(`Character ${instanceId} has Reckless and cannot quest`);
            return createInvalidMove("CHARACTER_RECKLESS", "moves.quest.errors.characterReckless", { instanceId, cardName: lorcanaCard.card.name });
        }
        ...
        const loreValue = lorcanaCard.card.lore || 0;
        if (loreValue <= 0) {
            logger.error(`Character ${instanceId} has no lore value and cannot quest`);
            return createInvalidMove("NO_LORE_VALUE", "moves.quest.errors.noLoreValue", { instanceId, cardName: lorcanaCard.card.name });
        }
        ...
        logger.info(`Player ${playerID} gains ${loreValue} lore from questing with ${lorcanaCard.card.name}`);
        // Add triggered effects to the bag (rule 4.3.5.5)
        gameOps?.addTriggeredEffectsToTheBag("onQuest", instanceId);
        logger.info(`Player ${playerID} quested with character ${instanceId} (${lorcanaCard.card.name}) for ${loreValue} lore`);
        return G;
    }
    catch (error) {
        logger.error(`Unexpected error in questMove: ${error}`);
        return createInvalidMove("UNEXPECTED_ERROR", "moves.quest.errors.unexpectedError", { error: String(error), instanceId, playerId: playerID });
    }
};
