...
export const useActivatedAbilityMove = ({ G, ctx, coreOps, gameOps, playerID }, sourceInstanceId, options) => {
    try {
        // Ensure we're in the main phase (this is a turn action)
        if (ctx.currentPhase !== "mainPhase") {
            logger.error(`Cannot use activated ability during ${ctx.currentPhase} phase`);
            return createInvalidMove("WRONG_PHASE", "moves.useActivatedAbility.errors.wrongPhase", { currentPhase: ctx.currentPhase, expectedPhase: "mainPhase" });
        }
        const sourceInstance = coreOps.getCardInstance(sourceInstanceId);
        if (!sourceInstance) {
            logger.error(`Failed to get source instance ${sourceInstanceId}`);
            return createInvalidMove("SOURCE_NOT_FOUND", "moves.useActivatedAbility.errors.sourceNotFound", { instanceId: sourceInstanceId });
        }
        ...
        if (sourceOwner !== playerID) {
            logger.error(`Source ${sourceInstanceId} is not controlled by player ${playerID}`);
            return createInvalidMove("SOURCE_NOT_CONTROLLED", "moves.useActivatedAbility.errors.sourceNotControlled", { instanceId: sourceInstanceId, playerId: playerID });
        }
        ...
        if (!playCards.find((card) => card.instanceId === sourceInstanceId)) {
            ...
            return createInvalidMove("SOURCE_NOT_IN_PLAY", "moves.useActivatedAbility.errors.sourceNotInPlay", { instanceId: sourceInstanceId, playerId: playerID });
        }
        // Get the card's abilities
        const cardAbilities = sourceCard.card.abilities || [];
        // Find the specific ability to activate
        const abilityToActivate = cardAbilities.find((ability) => ability.id === options.abilityId || ability.type === "activated");
        if (!abilityToActivate) {
            logger.error(`Activated ability ${options.abilityId} not found on card ${sourceInstanceId}`);
            return createInvalidMove("ABILITY_NOT_FOUND", "moves.useActivatedAbility.errors.abilityNotFound", { instanceId: sourceInstanceId, abilityId: options.abilityId });
        }
        ...
        if (abilityToActivate.type !== "activated") {
            ...
            return createInvalidMove("NOT_ACTIVATED_ABILITY", "moves.useActivatedAbility.errors.notActivatedAbility", { instanceId: sourceInstanceId, abilityId: options.abilityId });
        }
        ...
        if (abilityToActivate.targets && abilityToActivate.targets.length > 0) {
            if (!options.targets || options.targets.length === 0) {
                logger.error(`Ability ${options.abilityId} requires targets but none provided`);
                return createInvalidMove("TARGETS_REQUIRED", "moves.useActivatedAbility.errors.targetsRequired", { instanceId: sourceInstanceId, abilityId: options.abilityId });
            }
            ...
            for (const targetId of options.targets) {
                ...
                if (!targetInstance) {
                    ...
                    return createInvalidMove("TARGET_NOT_FOUND", "moves.useActivatedAbility.errors.targetNotFound", { instanceId: sourceInstanceId, targetId });
                }
            }
        }
        ...
        if (abilityCost.ink && abilityCost.ink > 0) {
            ...
            if (inkCards.length < abilityCost.ink) {
                logger.error(`Player ${playerID} does not have enough ink to activate ability. Required: ${abilityCost.ink}, Available: ${inkCards.length}`);
                return createInvalidMove("INSUFFICIENT_INK", "moves.useActivatedAbility.errors.insufficientInk", {
                    required: abilityCost.ink,
                    available: inkCards.length,
                    playerId: playerID,
                });
            }
            logger.info(`Player ${playerID} pays ${abilityCost.ink} ink for activated ability`);
        }
        ...
        logger.info(`Applying effect of ability ${options.abilityId} from ${sourceInstanceId}`);
        // Add the ability to the stack/bag for resolution
        // Note: Would need proper ability resolution system
        logger.info(`Player ${playerID} activated ability ${options.abilityId} on ${sourceInstanceId}`);
        return G;
    }
    catch (error) {
        logger.error(`Unexpected error in useActivatedAbilityMove: ${error}`);
        return createInvalidMove("UNEXPECTED_ERROR", "moves.useActivatedAbility.errors.unexpectedError", {
            error: String(error),
            sourceInstanceId,
...
//# sourceMappingURL=use-activated-ability.js.map