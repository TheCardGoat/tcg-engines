...
export class AlphaClashEngine extends GameEngine {
    ...
    constructor(config) {
        ...
        const cardRepository = config.cardRepository || new AlphaClashCardRepository(config.cards);
     ...
     * Get cards by filter
     */
    getCardsByFilter(filter, playerId) {
        const results = [];
        const ctx = this.getGameState().ctx;
    ...
    matchesFilter(card, filter) {
        ...
        if (filter.color && filter.color.length > 0) {
            const hasMatchingColor = filter.color.some((color) => card.colors?.includes(color));
            if (!hasMatchingColor) {
                return false;
            }
        }
        ...
        if (filter.hasKeyword && filter.hasKeyword.length > 0) {
            const hasMatchingKeyword = filter.hasKeyword.some((keyword) => card.keywords?.includes(keyword));
        ...
        if (filter.affiliation && filter.affiliation.length > 0) {
            const hasMatchingAffiliation = filter.affiliation.some((affiliation) => card.affiliations?.includes(affiliation));
     ...
     */
    canPlayCard(instanceId, playerId) {
        const ctx = this.getGameState().ctx;
        const targetPlayerId = playerId || ctx.playerOrder[ctx.turnPlayerPos];
     ...
     */
    getAvailableResources(playerId) {
        const ctx = this.getGameState().ctx;
        const targetPlayerId = playerId || ctx.playerOrder[ctx.turnPlayerPos];
     ...
     */
    canAffordCard(instanceId, playerId) {
        const card = this.cardRepository.getCardByInstanceId(instanceId);
        if (!card)
            return false;
    ...
    getGameStats() {
        ...
        for (const playerId of ctx.playerOrder) {
            ...
            const contenderHealth = contender?.definition.type === "contender"
...
//# sourceMappingURL=alpha-clash-engine.js.map