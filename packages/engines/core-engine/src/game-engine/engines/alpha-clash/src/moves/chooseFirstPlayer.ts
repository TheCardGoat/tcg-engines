...
export const chooseFirstPlayer: AlphaClashMove = (
  { G, playerID, coreOps },
  ...
  if (!players.includes(chosenPlayerId)) {
    ...
    return createInvalidMove(
      "INVALID_PLAYER_ID",
      "moves.chooseFirstPlayer.errors.invalidPlayerId",
      { chosenPlayerId, availablePlayers: players },
    );
  }
  ...
  if (G.firstPlayerChosen) {
    ...
    return createInvalidMove(
      "FIRST_PLAYER_ALREADY_CHOSEN",
      "moves.chooseFirstPlayer.errors.firstPlayerAlreadyChosen",
    );
  }
  ...
  logger.info(
    `ChooseFirstPlayer: ${playerID} chose ${chosenPlayerId} to go first`,
  );
