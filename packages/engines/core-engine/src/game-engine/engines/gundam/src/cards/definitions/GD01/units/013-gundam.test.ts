import { describe, expect, it } from "bun:test";
import {
  assertGamePhase,
  assertZoneCount,
} from "../../../../../__tests__/helpers/assertion-helpers";
import { GundamTestEngine } from "../../../../testing/gundam-test-engine";
import { gundam } from "./013-gundam";

/**
 * Tests for GD01-013: Gundam
 *
 * Card Properties:
 * - Cost: 2, Level: 4, AP: 3, HP: 4
 * - Color: blue
 * - Type: unit
 * - Rarity: common
 * - Traits: earth federation
 * - Zones: space, earth
 * - Link Requirement: amuro ray
 *
 * Test Coverage:
 * - Card definition structure and properties
 * - Abilities definition (if applicable)
 * - Card usability in game scenarios
 */

describe("GD01-013: Gundam", () => {
  describe("Card Definition", () => {
    it("should have correct basic properties", () => {
      expect(gundam.id).toBe("GD01-013");
      expect(gundam.name).toBe("Gundam");
      expect(gundam.number).toBe(13);
      expect(gundam.set).toBe("GD01");
      expect(gundam.type).toBe("unit");
      expect(gundam.rarity).toBe("common");
    });

    it("should have correct stats", () => {
      expect(gundam.cost).toBe(2);
      expect(gundam.level).toBe(4);
      expect(gundam.ap).toBe(3);
      expect(gundam.hp).toBe(4);
    });

    it("should have correct color", () => {
      expect(gundam.color).toBe("blue");
    });

    it("should have correct traits", () => {
      expect(gundam.traits).toEqual(["earth federation"]);
    });

    it("should have correct zones", () => {
      expect(gundam.zones).toEqual(["space", "earth"]);
    });

    it("should have correct link requirement", () => {
      expect(gundam.linkRequirement).toEqual(["amuro ray"]);
    });
  });

  describe("Card in Game Scenarios", () => {
    it("should be usable with sufficient resources", () => {
      const engine = new GundamTestEngine(
        {
          hand: [gundam],
          resourceArea: 4,
          battleArea: 0,
          deck: 30,
        },
        {
          battleArea: 2,
          resourceArea: 5,
          deck: 30,
        },
      );

      assertZoneCount(engine, "hand", 1, "player_one");
      assertZoneCount(engine, "resourceArea", 4, "player_one");
      assertGamePhase(engine, "mainPhase");
    });

    it("should set up scenario with card in appropriate zone", () => {
      const engine = new GundamTestEngine(
        {
          battleArea: [gundam],
          hand: 5,
          resourceArea: 5,
          deck: 30,
        },
        {
          battleArea: 2,
          hand: 5,
          resourceArea: 5,
          deck: 30,
        },
      );

      assertZoneCount(engine, "battleArea", 1, "player_one");
      assertGamePhase(engine, "mainPhase");
    });

    it("should work with link requirement", () => {
      const engine = new GundamTestEngine(
        {
          battleArea: [gundam],
          hand: 5,
          resourceArea: 5,
          deck: 30,
        },
        {
          battleArea: 2,
          hand: 5,
          resourceArea: 5,
          deck: 30,
        },
      );

      expect(gundam.linkRequirement).toEqual(["amuro ray"]);
      assertZoneCount(engine, "battleArea", 1, "player_one");
    });

    it("should be deployable in specified zones", () => {
      const engine = new GundamTestEngine(
        {
          hand: [gundam],
          resourceArea: 4,
          battleArea: 0,
          deck: 30,
        },
        {
          battleArea: 2,
          resourceArea: 5,
          deck: 30,
        },
      );

      expect(gundam.zones).toContain("space");
      expect(gundam.zones).toContain("earth");
      assertZoneCount(engine, "hand", 1, "player_one");
    });
  });

  describe("Card Implementation Status", () => {
    it("should track implementation status", () => {
      expect(gundam).toHaveProperty("implemented");
      expect(gundam).toHaveProperty("missingTestCase");
    });
  });

  describe("Card Stats and Combat", () => {
    it("should have appropriate stats for level 4", () => {
      expect(gundam.level).toBe(4);
      expect(gundam.cost).toBe(2);
      expect(gundam.ap).toBe(3);
      expect(gundam.hp).toBe(4);
    });

    it("should have balanced AP and HP", () => {
      const totalStats = gundam.ap + gundam.hp;
      expect(totalStats).toBeGreaterThan(0);
    });

    it("should set up combat scenario", () => {
      const engine = new GundamTestEngine(
        {
          battleArea: [gundam],
          resourceArea: 5,
          deck: 30,
        },
        {
          battleArea: 1,
          resourceArea: 5,
          deck: 30,
        },
      );

      assertZoneCount(engine, "battleArea", 1, "player_one");
      assertZoneCount(engine, "battleArea", 1, "player_two");
    });
  });
});
