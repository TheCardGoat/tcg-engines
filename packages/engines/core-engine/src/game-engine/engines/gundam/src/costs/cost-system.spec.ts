	...
	describe("Cost Payment", () => {
		describe("payCost", () => {
			it("pays resource cost by resting active resources", () => {
				...
				if (result.success) {
					expect(result.data.updatedResourcePool.activeResources.length).toBe(2);
					expect(result.data.updatedResourcePool.restedResources.length).toBe(3);
				}
			});
			...
			it("successfully pays zero cost", () => {
				...
				expect(result.success).toBe(true);
				if (result.success) {
					expect(result.data.updatedResourcePool.activeResources.length).toBe(3);
					expect(result.data.updatedResourcePool.restedResources.length).toBe(0);
				}
			});
			...
			it("pays exact cost matching all active resources", () => {
				...
				expect(result.success).toBe(true);
				if (result.success) {
					expect(result.data.updatedResourcePool.activeResources.length).toBe(0);
					expect(result.data.updatedResourcePool.restedResources.length).toBe(4);
				}
			});
		});
	});
