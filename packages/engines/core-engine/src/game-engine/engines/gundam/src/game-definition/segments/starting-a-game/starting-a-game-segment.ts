...
// 6-2-1-1. Present the deck and resource deck you will use in the game. The deck and resource deck must conform to the rules on deck construction explained in 5-1.
...
// 6-2-1-7. After Player One has announced whether or not they will redraw, Player Two may redraw according to the same rules explained in 5-2-1-6-1 above.
// 6-2-2. Each player takes the top six cards of their deck, one at a time, and places them face down into the shield section of their shield area without looking at them. When doing so, place each card so it overlaps the previous one, starting with the card nearest to you.
// 6-2-3. Each player places one active EX Base token card into the base section of their shield area.
...
export const startingAGameSegment: SegmentConfig<GundamGameState> = {
  ...
  onEnd: ({ G, coreOps }) => {
    ...
    // 6-2-2. Each player takes the top six cards of their deck, one at a time,
