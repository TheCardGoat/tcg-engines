...
export const startingAGameSegment: SegmentConfig<LorcanaGameState> = {
  ...
  onEnd: ({ G, ctx, coreOps }) => {
    ...
    if (ctx.otp) {
      // Make sure to explicitly set both priority and turn player to the chosen first player
      coreOps.setPriorityPlayer(ctx.otp);
      coreOps.setTurnPlayer(ctx.otp);
    }

    // Clear pending mulligan state
  ...
  turn: {
    phases: {
      chooseFirstPlayer: {
        ...
        moves: {
          // Ensure the move name matches exactly what's in lorcanaMoves
