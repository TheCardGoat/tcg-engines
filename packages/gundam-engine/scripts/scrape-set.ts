...
import { saveCardFile, generateSetIndex } from "../tools/generator/file-writer";
import type { CardDefinition } from "../src/cards/card-types";

const ALLOWED_SETS = ["GD01", "ST01", "ST02", "ST03", "ST04", "ST05", "EXB-001"];
  ...
  for (const scraped of scrapedCards) {
    try {
      ...
      if (parsed.warnings.length > 0) {
        console.log(`   ⚠️  ${scraped.name}: ${parsed.warnings.length} warnings`);
      }
    } catch (error) {
