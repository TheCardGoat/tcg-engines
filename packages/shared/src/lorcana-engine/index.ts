...
export type TableTurn = {
  ...
  quests?: string[];
  abilities?: Array<{ card: string; ability: string }>;
...
// This should be an alias from GameSelectModel, but it's not working
export type Game = {
  id: string;
  name: string;
...
export type Match<Ability, ContinuousEffect, FloatingTriggeredAbility> = {
  // Static Info
  // TODO: Move static info out of Match State, there is been cases in which this has been updated
  matchId: string;
  gameId: string;
  seed: string;
  firstPlayer?: string;
...
// Used to show turn by turn progress
