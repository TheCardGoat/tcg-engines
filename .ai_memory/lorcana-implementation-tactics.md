...
## Lorcanito-Inspired Workflow

Based on patterns from the Lorcanito repository, we use a **similarity-first** approach to card implementation:

### Core Principle: Find Before Implement

**Before implementing any card**, use the `pattern-matcher` agent to find similar implemented cards. This dramatically reduces implementation time by reusing proven patterns.

```bash
# Find similar cards first
/lore:pattern-matcher "When you play this character, draw 2 cards"

# Returns top 3-5 similar cards with:
# - Similarity score
# - File path
# - Ability text
...
### Multi-Category Similarity Matching

The `pattern-matcher` agent uses multi-category matching (adapted from Lorcanito):

| Category | Weight | Example |
|----------|--------|---------|
| Same name/title | High | "Ariel" matches "Ariel" variants |
| Same ink type | Medium | "Sapphire" matches "Sapphire" |
| Same card type | Medium | "Action" matches "Action" |
| Similar ability text | High | "draw 2 cards" matches "draw a card" |

**Rule**: Cards appearing in 2+ categories get higher scores.

### Per-Card Workflow (Enhanced)

```
1. FIND REFERENCE (30s)     → pattern-matcher finds similar cards
2. WRITE TEST (1 min)       → lorcana-test-writer generates tests
3. IMPLEMENT (2 min)        → lorcana-card-migrator uses parser v2
4. VERIFY (30s)             → Tests, type-check, format, lint
5. CAPTURE LEARNING (1 min) → Document parser result, patterns
```
...
└── .ai_memory/
    ├── lorcana-sets-2-3-todo.md         # Master TODO list (NEW - 407 cards)
    ├── lorcana-set-002-003-implementation.md  # Main implementation log
    └── learning/
        ├── set-002-learning.json         # Autogenerated learning data
        └── set-003-learning.json         # Autogenerated learning data
```
...
**Card Number Ranges**:
- Set 1 (TFC): 001-204 (✅ COMPLETE)
- Set 2 (RotF): 001-204 (204 cards - TARGET)
- Set 3 (ItI): 001-203 (203 cards - FUTURE)
...
### New Card Type: Locations (Set 3 Only)

Set 3 introduces **Location** cards, a new card type with different properties:
...
export const neverLandMermaidLagoon: LocationCard = {
  ...
  inkType: ["emerald"],
  set: "003",
  text: "When you play this location, draw 1 card.",
  moveCost: 3,     // NOT cost - locations use moveCost
  willpower: 5,
  lore: 2,
  cardNumber: 32,
  abilities: [
...
| Property | Character/Action/Item | Location |
|----------|---------------------|----------|
| `cost` | ✅ Yes | ❌ No |
| `inkable` | ✅ Yes | ❌ No |
| `moveCost` | ❌ No | ✅ Yes |
| `strength` | Characters only | ❌ No |
| `willpower` | Characters/Locations | ✅ Yes |
| `lore` | Characters/Locations | ✅ Yes |
| `classifications` | Characters only | ❌ No |

**Location-Specific Considerations**:
- Locations don't have a cost to play from hand
- They use `moveCost` for movement between zones
- Testing must account for different play mechanics
- May require new effect parsers for location-specific abilities
